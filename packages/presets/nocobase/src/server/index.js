...
export class PresetNocoBase extends Plugin {
    ...
    async allPlugins() {
        ...
        return (await Promise.all(builtInPlugins.map(async (pkgOrName) => {
        ...
        }))).concat(await Promise.all(localPlugins.map(async (plugin) => {
    ...
    async getPluginToBeUpgraded() {
        ...
        const builtInPlugins = await this.getBuiltInPlugins();
        const localPlugins = await this.getLocalPlugins();
        const plugins = await Promise.all(builtInPlugins.map(async (pkgOrName) => {
            const { name } = await PluginManager.parseName(pkgOrName);
            const packageJson = await this.getPackageJson(pkgOrName);
            return {
                name,
    ...
    async updateOrCreatePlugins() {
        ...
        await this.db.sequelize.transaction((transaction) => {
            return Promise.all(plugins.map((values) => repository.updateOrCreate({
                transaction,
                values,
                filterKeys: ['name'],
            })));
