 ...
 */
export function getDepPkgPath(packageName, cwd) {
  try {
...
export function getPackageFilePath(packageName, filePath) {
  const packageDir = getPackageDir(packageName);
  return path.join(packageDir, filePath);
}
export function getPackageFilePathWithExistCheck(packageName, filePath) {
...
export function getExposeUrl(packageName, filePath) {
  return `${process.env.PLUGIN_STATICS_PATH}${packageName}/${filePath}`;
}
export function getExposeReadmeUrl(packageName, lang) {
  let READMEPath = null;
...
export function getExposeChangelogUrl(packageName) {
 ...
 */
export function getPackageNameByExposeUrl(pathname) {
...
export function getPackageDirByExposeUrl(pathname) {
  return getPackageDir(getPackageNameByExposeUrl(pathname));
}
//# sourceMappingURL=clientStaticUtils.js.map
