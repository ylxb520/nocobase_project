 ...
 */
export async function dataTemplate(ctx, next) {
...
const traverseHasMany = (arr, { collection, exclude = [], include = [] }) => {
  if (!arr) {
    return arr;
  }
  return arr.map((item) => traverseJSON(item, { collection, exclude, include, isHasManyField: true }));
};
const traverseBelongsToMany = (arr, { collection, exclude = [], through }) => {
  if (!arr) {
    return arr;
  }
