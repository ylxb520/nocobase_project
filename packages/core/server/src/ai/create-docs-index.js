...
async function collectModuleGroups(packageName) {
    ...
    if (await fs.pathExists(rootMetaPath)) {
        try {
            const rootMeta = (await fs.readJson(rootMetaPath));
    ...
    const moduleMetaFiles = metaEntries.length === 0
        ? await fg(['*/meta.json'], {
            cwd: docsDir,
            onlyFiles: true,
            absolute: true,
    ...
    const groups = [];
    const entriesToProcess = metaEntries.length > 0
        ? metaEntries.map((entry) => ({ metaFile: rootMetaPath, entry }))
        : moduleMetaFiles.map((metaFile) => ({ metaFile, entry: undefined }));
    for (const item of entriesToProcess) {
        ...
        for (const metaEntry of perFileEntries) {
            ...
            const docEntries = await Promise.all(files.map(async (file) => {
        ...
        for (const group of groups) {
            ...
            if (group.description) {
                ...
                if (existingDesc && existingDesc !== group.description) {
                    ...
                    list.push(`${group.packageName}: description mismatch (existing="${existingDesc}", new="${group.description}")`);
    ...
    for (const [moduleName, groups] of moduleGroups.entries()) {
        ...
        for (const group of sortedGroups) {
            for (const entry of group.entries) {
                ...
                if (existing) {
                    moduleConflicts.push(`Duplicate path "${entry.relativePath}" from ${entry.packageName} and ${existing.packageName}`);
                    continue;
                }
                ...
                if (entry.isIndex) {
                    processedContent = applyReferencesToIndex(processedContent, entry.absolutePath, group.directoryChildren, group.docMap);
                }
                if (splitRefs.length) {
                    processedContent = applySplitReferences(processedContent, splitRefs.map((split) => ({
...
function splitExamples(body) {
    ...
    for (const match of codeMatches) {
        ...
        if (currentH3 || currentH2) {
            ...
            const nextHeadingIndex = headings.filter((item) => item.index > anchor.index && item.level <= anchor.level).slice(0, 1)[0]?.index ??
                body.length;
...
export async function createDocsIndex(app, options = {}) {
...
//# sourceMappingURL=create-docs-index.js.map