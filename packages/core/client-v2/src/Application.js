...
export class Application {
  ...
  apiClient;
  components = {
    AppNotFound: () => React.createElement('div', null, 'Not Found'),
    AppError: () => React.createElement('div', null, this.error?.message),
    AppSpin: () => React.createElement('div', null, 'Loading'),
    AppMaintaining: () => React.createElement('div', null, 'Maintaining'),
    AppMaintainingDialog: () => React.createElement('div', null, 'Maintaining Dialog'),
  };
  pluginManager;
  ...
  getComponent(Component, isShowError = true) {
    const showError = (msg) => isShowError && console.error(msg);
    if (!Component) {
    ...
    if (typeof Component === 'string') {
      const res = _.get(this.components, Component);
      if (!res) {
  ...
  getRootComponent() {
    const Root = () =>
      React.createElement(
        ...
        React.createElement(FlowModelRenderer, { fallback: this.renderComponent('AppSpin'), model: this.model }),
      );
    return Root;
  }
...
class ApplicationModel extends FlowModel {
  ...
  renderContent() {
    ...
    return React.createElement(Router, { BaseLayout: Providers });
  }
}
