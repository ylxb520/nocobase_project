...
describe('app', () => {
  ...
  it('should render custom "Not Found" component', async () => {
    ...
    const app = createMockClient({
      ...
      components: { AppNotFound: () => React.createElement('div', null, 'Not Found2') },
    });
    await renderApp(app);
  ...
  it('should support app provider functionality', async () => {
    class PluginHelloClient extends Plugin {
      async load() {
        this.app.use(() => React.createElement('div', null, 'Hello Provider'));
      }
    }
  ...
  it('should support router functionality', async () => {
    class PluginHelloClient extends Plugin {
      async load() {
        this.router.add('root', { path: '/', Component: () => React.createElement('div', null, 'Hello Route') });
      }
    }
    const app = createMockClient({ plugins: [PluginHelloClient] });
    await renderApp(app);
  ...
  it('should handle WebSocket maintaining and running states', async () => {
    ...
    class PluginHelloClient extends Plugin {
      async load() {
        this.router.add('root', { path: '/', Component: () => React.createElement('div', null, 'Hello') });
