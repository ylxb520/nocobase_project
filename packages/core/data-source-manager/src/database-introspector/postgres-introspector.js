...
ORDER BY tc.constraint_name, kcu.column_name;
  ...
  async getArrayColumnElementType(tableInfo, columnNames) {
    const schema = tableInfo.schema || 'public';
...
WHERE c.table_schema = :schema AND c.table_name = :tableName AND c.column_name in (:columnNames);
  ...
  async appendArrayColumnElementType(tableInfo, columnsInfo) {
    const arrayColumns = {};
    ...
    const udtNameMap = _.keyBy(rows, 'column_name');
