...
describe('associated field order', () => {
  ...
  it('should sortBy through table field', async () => {
    ...
    const postImageRepository = db.getRepository('posts.images', p1.get('id'));
    await postImageRepository.add([[t2.get('id'), { sort: 2 }]]);
    await postImageRepository.add([[t1.get('id'), { sort: 1 }]]);
...
//# sourceMappingURL=sort-by.test.js.map
