 ...
 */
import { createMockDatabase, Repository } from '@nocobase/database';
...
describe('repository.update', () => {
  ...
  it('update with filterByTk', async () => {
    ...
    const user2 = await User.model.create({
      name: 'user2',
    });
    const hook = vi.fn();
...
describe('repository.relatedQuery', () => {
  ...
  it('create', async () => {
    ...
    const userPostRepository = await User.repository.relation('posts').of(user.get('id'));
