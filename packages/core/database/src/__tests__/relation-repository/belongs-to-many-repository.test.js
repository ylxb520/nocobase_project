 ...
 */
import { BelongsToManyRepository, createMockDatabase } from '@nocobase/database';
...
pgOnly()('belongs to many with targetCollection', () => {
  ...
  it('should update child collection', async () => {
    ...
    const repository = Org.repository.relation('users').of(org1.get('id'));
...
describe('belongs to many with collection that has no id key', () => {
  ...
  it('should set relation', async () => {
    ...
    const a1bsRepository = await A.repository.relation('bs').of('a1');
...
describe('belongs to many with target key', function () {
  ...
  test('destroy by target key', async () => {
    ...
    const PostTagRepository = new BelongsToManyRepository(Post, 'tags', p1.get('title'));
    await PostTagRepository.set([t1.get('name'), t2.get('name')]);
...
//# sourceMappingURL=belongs-to-many-repository.test.js.map
