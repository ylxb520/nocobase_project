...
describe('collection sync', () => {
  ...
  test('sync fields', async () => {
    ...
    const tableFields = await collection.model.queryInterface.describeTable(`${db.getTablePrefix()}users`);
  ...
  test('sync with association not exists', async () => {
    ...
    const tableFields = await model.queryInterface.describeTable(`${db.getTablePrefix()}posts`);
  ...
  test('sync with association', async () => {
    ...
    await collection.sync();
    if (db.options.underscored) {
      const tableFields = await model.queryInterface.describeTable(`${db.getTablePrefix()}posts_tags`);
      expect(tableFields['post_id']).toBeDefined();
      expect(tableFields['tag_id']).toBeDefined();
    } else {
      const tableFields = await model.queryInterface.describeTable(`${db.getTablePrefix()}postsTags`);
