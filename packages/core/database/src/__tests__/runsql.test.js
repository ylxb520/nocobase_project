...
describe('runSQL', function () {
  ...
  describe('error handling', () => {
    test('should throw error for invalid SQL syntax', async () => {
      await expect(db.runSQL('SELECT * FORM test')).rejects.toThrow();
    });
    ...
    test('should throw error for mismatched parameter binding', async () => {
      await expect(db.runSQL('SELECT * FROM test WHERE num = $invalidParam', { bind: { num: 1 } })).rejects.toThrow();
    });
  });
  describe('edge cases', () => {
...
//# sourceMappingURL=runsql.test.js.map
