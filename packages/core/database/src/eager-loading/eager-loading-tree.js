    ...
    const loadRecursive = async (node, ids = []) => {
      ...
      if (!node.parent) {
        ...
        if (associationType === 'BelongsToArray') {
          ...
          let where = { [targetKey]: Array.from(new Set(flatten(targetKeyValues))) };
          if (node.where) {
            where = {
              [Op.and]: [where, node.where],
            };
        ...
        if (associationType === 'BelongsToArray') {
          ...
          node.parent.instances.forEach((parentInstance) => {
            ...
            parentInstance.setDataValue(
              association.as,
              targetKeys?.map((targetKey) => instanceMap[targetKey]).filter(Boolean),
            );
          });
        }
        if (associationType == 'BelongsTo') {
...
//# sourceMappingURL=eager-loading-tree.js.map
