...
export function getTableName(collectionName, options) {
...
export function patchSequelizeQueryInterface(db) {
...
export function percent2float(value) {
  if (!value.endsWith('%')) {
    return NaN;
  }
...
export function getKeysByPrefix(keys, prefix) {
  return keys.filter((key) => key.startsWith(`${prefix}.`)).map((key) => key.substring(prefix.length + 1));
}
export function extractTypeFromDefinition(rawType) {
...
export function processIncludes(includes, model, parentAs = '') {
