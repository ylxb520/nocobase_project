...
const serverGlobalFiles = ['src/**', '!src/client/**', ...globExcludeFiles];
const clientGlobalFiles = ['src/**', '!src/server/**', ...globExcludeFiles];
...
export function deleteServerFiles(cwd, log) {
...
export function writeExternalPackageVersion(cwd, log) {
...
export async function buildServerDeps(cwd, serverFiles, log) {
  ...
  for (const dep of Object.keys(deps)) {
    ...
    await ncc(dep, nccConfig).then(({ code, assets }) => {
      // emit dist file
...
export async function buildPluginServer(cwd, userConfig, sourcemap, log) {
...
export async function buildProPluginServer(cwd, userConfig, sourcemap, log) {
...
export async function buildPluginClient(cwd, userConfig, sourcemap, log, isCommercial = false) {
...
export async function buildPlugin(cwd, userConfig, sourcemap, log) {
  if (
