 ...
 * @internal
 */
export const colorFormat = winston.format((info) => {
 ...
 * @internal
 */
export const stripColorFormat = winston.format((info) => {
  Object.entries(info).forEach(([k, v]) => {
    if (typeof v !== 'string') {
      return;
    }
 ...
 */
export const logfmtFormat = winston.format.printf((info) =>
  Object.entries(info)
    .map(([k, v]) => {
      if (typeof v === 'object') {
        try {
 ...
 * @internal
 */
export const consoleFormat = winston.format.printf((info) => {
 ...
 * @internal
 */
export const escapeFormat = winston.format((info) => {
  let { message } = info;
