...
describe('FlowViewer zIndex with usePage', () => {
  let engine;
  function TestApp({ onReady }) {
    const [page, pageHolder] = usePage();
    React.useEffect(() => {
      const wrappedTypes = {
        embed: {
          open: (props, ctx) => {
    ...
    return React.createElement(React.Fragment, null, pageHolder);
  }
  const Wrapper = ({ onReady }) =>
    React.createElement(
      ...
      React.createElement(
        App,
        null,
        React.createElement(FlowEngineProvider, { engine: engine }, React.createElement(TestApp, { onReady: onReady })),
      ),
    );
  beforeEach(() => {
  ...
  it('open page1 -> open page2 -> close page2 -> open page3: internal zIndex counter should be correct', async () => {
    ...
    let page1;
    await act(async () => {
      page1 = getViewer().embed({ content: React.createElement('div', { 'data-testid': 'page1' }, 'Page 1') });
    });
    ...
    let page2;
    await act(async () => {
      page2 = getViewer().embed({ content: React.createElement('div', { 'data-testid': 'page2' }, 'Page 2') });
    });
    ...
    let page3;
    await act(async () => {
      page3 = getViewer().embed({ content: React.createElement('div', { 'data-testid': 'page3' }, 'Page 3') });
    });
  ...
  it('replaces previous embed view when using global #nocobase-embed-container target', async () => {
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            FlowEngineProvider,
            { engine: engine },
            React.createElement(TestApp, { onReady: onReady }),
          ),
        ),
      );
    ...
    await act(async () => {
      api.open({ target, content: React.createElement('div', { 'data-testid': 'page1' }, 'Page 1') }, engine.context);
    });
    await waitFor(() => expect(screen.getByTestId('page1')).toBeInTheDocument());
    // Opening page2 into the global embed container should destroy page1 (replace behavior).
    await act(async () => {
      api.open({ target, content: React.createElement('div', { 'data-testid': 'page2' }, 'Page 2') }, engine.context);
    });
    await waitFor(() => expect(screen.getByTestId('page2')).toBeInTheDocument());
...
//# sourceMappingURL=FlowView.usePage.test.js.map
