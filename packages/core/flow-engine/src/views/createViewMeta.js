 ...
 */
export function createPopupMeta(ctx, anchorView) {
  const t = (k) => ctx.t(k);
  const isPopupView = (view) => {
  ...
  const hasPopupNow = () => isPopupView(anchorView ?? ctx.view);
  // 统一解析锚定视图下的 RecordRef，避免在设置弹窗等二级视图中被误导
  const resolveRecordRef = async (flowCtx) => {
  ...
  const getParentRecordRef = async (level, flowCtx) => {
    try {
      ...
      let model = useCtx.engine?.getModel(parent.viewUid, true);
      if (!model) {
        try {
  ...
  const createParentFactory = (level) => {
    const factory = () => {
      const meta = {
        type: 'object',
    ...
    const meta = {
      ...
      buildVariablesParams: async (c) => {
        ...
        try {
          ...
          const assoc = inputArgs?.associationName;
          const dsKey = inputArgs?.dataSourceKey || 'main';
      ...
      properties: async () => {
        ...
        try {
          ...
          const srcId = inputArgs?.sourceId;
          let assoc = inputArgs?.associationName;
          let dsKey = inputArgs?.dataSourceKey || 'main';
          ...
          if (!assoc || typeof assoc !== 'string' || !assoc.includes('.')) {
            ...
            if (last?.viewUid) {
              let model = ctx?.engine?.getModel(last.viewUid, true);
              if (!model) {
...
export async function buildPopupRuntime(ctx, view) {
 ...
 */
export function registerPopupVariable(ctx, view) {
...
//# sourceMappingURL=createViewMeta.js.map
