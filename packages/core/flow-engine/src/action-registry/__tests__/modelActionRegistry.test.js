...
describe('ModelActionRegistry (class-level)', () => {
  it('warns and overrides when registering duplicate actions on same class', () => {
    ...
    const m = engine.createModel({ use: 'X' });
  ...
  it('merges inheritance and respects override', () => {
    ...
    const a = engine.createModel({ use: 'A' });
    const b = engine.createModel({ use: 'B' });
  ...
  it('lazy invalidation along chain when ancestor updates', () => {
    ...
    const m5 = engine.createModel({ use: 'M5' });
  ...
  it('child registration does not affect parent set', () => {
    ...
    const p = engine.createModel({ use: 'P' });
    const c = engine.createModel({ use: 'C' });
    const childH = vi.fn();
...
//# sourceMappingURL=modelActionRegistry.test.js.map
