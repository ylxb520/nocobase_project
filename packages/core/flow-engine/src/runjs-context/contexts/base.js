...
export function defineBaseContextMeta() {
  FlowRunJSContext.define({
    ...
    methods: {
      ...
      t:
        'Internationalization function for translating text. Parameters: (key: string, variables?: object) => string. Example: `ctx.t("Hello {{name}}", { name: "World" })`',
      initResource: {
        description:
  ...
  FlowRunJSContext.define(
    {
      ...
      methods: {
        ...
        t:
          '国际化函数，用于翻译文案。参数：(key: string, variables?: object) => string。示例：`ctx.t("你好 {{name}}", { name: "世界" })`',
        initResource: {
          description:
