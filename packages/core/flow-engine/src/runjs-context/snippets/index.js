 ...
 */
export function registerRunJSSnippet(ref, loader, options) {
...
function normalizeScenes(def, key) {
  if (Array.isArray(def?.scenes) && def.scenes.length) {
    return def.scenes.map((scene) => String(scene).trim()).filter((scene) => scene.length > 0);
  }
  const parts = key.split('/');
...
function resolveLocaleMeta(def, locale) {
...
export async function getSnippetBody(ref) {
...
export async function listSnippetsForContext(ctxClassName, version = 'v1', locale) {
  const entries = [];
  const allowedContextNames = new Set();
  if (typeof ctxClassName === 'string' && ctxClassName) allowedContextNames.add(ctxClassName);
  try {
    const resolvedCtor = RunJSContextRegistry['resolve'](version, ctxClassName);
  ...
  await Promise.all(
    Object.entries(snippets).map(async ([key, loader]) => {
      ...
      try {
        const mod = await loader();
        const def = mod?.default || {};
        const body = def?.content ?? mod?.content;
        ...
        if (Array.isArray(def?.contexts) && def.contexts.length) {
          ...
          if (ctxClassName === '*') {
            ...
            ok = ctxNames.includes('*') || ctxNames.some((name) => allowedContextNames.has(name));
          }
        }
