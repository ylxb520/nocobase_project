...
export class FlowExecutor {
  ...
  async runFlow(model, flowKey, inputArgs, runId, eventName) {
    ...
    let eventStep = model.getEvent(typeof flow.on === 'string' ? flow.on : flow.on?.eventName);
    if (eventStep) {
    ...
    const stepDefs = eventStep ? { eventStep: eventStep, ...flow.steps } : flow.steps;
    ...
    for (const [stepKey, step] of Object.entries(stepDefs)) {
      ...
      if (step.use) {
        ...
        runtimeCtx = await createEphemeralContext(flowContext, {
          ...actionDefinition,
          ...step,
        });
        ...
        runtimeCtx = await createEphemeralContext(flowContext, step);
  ...
  async dispatchEvent(model, eventName, inputArgs, options) {
    ...
    try {
      ...
      // 进入错误钩子并记录
      try {
        await model.onDispatchEventError?.(eventName, options, inputArgs, err);
      } finally {
    ...
    const flowsToRun = isRouterReplayClick
      ? flows.filter((flow) => {
          ...
          const type = reg?.constructor?._type;
      ...
      if (sequential) {
        ...
        const ordered = flowsWithIndex
          ...
          .sort((a, b) => {
            ...
            const typeA = regA?.constructor?._type;
            const typeB = regB?.constructor?._type;
        ...
        ordered.forEach((flow, indexInOrdered) => {
          const on = flow.on;
          const onObj = typeof on === 'object' ? on : undefined;
          if (!onObj) return;
        ...
        for (const [whenKey, list] of scheduleGroups.entries()) {
          ...
          for (const it of sorted) {
            const when = it.shouldSkipOnAborted
              ? Object.assign((event) => event.type === whenKey && event.aborted !== true, {
                  __eventType: whenKey,
                })
              : whenKey;
    ...
    try {
      ...
      // 进入错误钩子并记录
      try {
        await model.onDispatchEventError?.(eventName, options, inputArgs, error);
      } catch (_) {
