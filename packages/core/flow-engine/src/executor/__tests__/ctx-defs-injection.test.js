...
describe('defineProperties/defineMethods injection', () => {
  ...
  it('engine.context.runAction applies defs before defaultParams and handler', async () => {
    ...
    engine.registerActions({
      test: {
        ...
        defineMethods: { add: (x) => x + 1 },
        defaultParams: (ctx) => ({ a: ctx.foo, b: ctx.add(2) }),
        handler,
      },
    });
    const result = await engine.context.runAction('test');
  ...
  it('model.context.runAction applies defs before defaultParams and handler', async () => {
    ...
    const h = vi.fn((_ctx, params) => ({ ...params, y: _ctx.mul(3) }));
    M.registerAction({
      name: 'mAction',
      defineProperties: async () => ({ al: { value: 7 } }),
      defineMethods: async () => ({ mul: (x) => x * 2 }),
      defaultParams: (ctx) => ({ a: ctx.al }),
      handler: h,
    });
    const m = engine.createModel({ use: 'M' });
    const res = await m.context.runAction('mAction', { x: 3 });
    expect(h).toHaveBeenCalledTimes(1);
    expect(res).toEqual({ a: 7, x: 3, y: 6 });
  });
  function createModelWithFlows(uid, flows) {
    ...
    const flows = {
      f: {
        steps: {
          s: {
            use: 'act',
            defaultParams: (ctx) => ({ p: ctx.base, q: ctx.inc(4) }),
          },
        },
      },
    };
  ...
  it('FlowExecutor: inline step injects defs before defaultParams/handler', async () => {
    const stepHandler = vi.fn((_ctx, params) => ({ ...params, v: _ctx.twice(5) }));
    const flows = {
      f: {
        steps: {
          s: {
            defineProperties: { k: { value: 8 } },
            defineMethods: { twice: (x) => 2 * x },
            defaultParams: (ctx) => ({ m: ctx.k }),
            handler: stepHandler,
          },
        },
      },
  ...
  it('FlowExecutor: step defs are isolated per step', async () => {
    const flows = {
      f: {
        steps: {
          a: {
            defineMethods: { only: (x) => x + 1 },
            handler: (_ctx, _params) => ({ ok: true }),
          },
          b: {
            // 这里期望无法访问到上一步注入的方法，从而抛出错误
            defaultParams: (ctx) => ({ v: ctx.only(1) }),
      ...
      f: {
        steps: {
          s: {
            ...
            defineMethods: { id: (x) => x + 100 },
            defaultParams: (ctx) => ({ a: ctx.foo, b: ctx.id(1) }),
          },
        },
      },
    };
    ...
    const flows = {
      f: {
        steps: {
          a: {
            handler: (ctx) => {
              ctx.defineProperty('mark', { value: 7 });
              ctx.defineMethod('inc', (x) => x + 1);
              return { ok: true };
            },
          },
          b: {
            // 无注入，直接读取上一步通过 defineProperty/defineMethod 挂在到父级的能力
            defaultParams: (ctx) => ({ v: ctx.mark, w: ctx.inc(2) }),
            handler: (_ctx, params) => params,
          },
        },
      },
    };
    const m = engine.createModel({ use: FlowModel.name, flowRegistry: flows });
    const res = await engine.executor.runFlow(m, 'f');
    expect(res).toEqual({ a: { ok: true }, b: { v: 7, w: 3 } });
  });
});
//# sourceMappingURL=ctx-defs-injection.test.js.map
