...
export class ModelOperationScheduler {
  engine;
  itemsByTargetUid = new Map();
  itemIdsByFromUid = new Map();
  itemsById = new Map();
  unbindHandlers = [];
  subscribedEventNames = new Set();
  constructor(engine) {
    this.engine = engine;
  ...
  bindEngineLifecycle() {
    ...
    const onGenericBeforeStart = async (e) => {
    ...
    const onGenericBeforeEnd = async (e) => {
  ...
  ensureEventSubscriptionIfNeeded(when) {
    const eventType = typeof when === 'string' ? when : typeof when === 'function' ? when.__eventType : undefined;
    if (!eventType) return;
...
//# sourceMappingURL=ModelOperationScheduler.js.map
