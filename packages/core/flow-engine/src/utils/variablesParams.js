...
import { buildServerContextParams } from '../utils/serverContextParams';
...
export function inferRecordRef(ctx) {
...
export function inferViewRecordRef(ctx) {
  const view = ctx.view;
  if (!view) return undefined;
  const inputArgs = view?.inputArgs || {};
  const collection = inputArgs?.collectionName || view?.record?.collection;
  const dataSourceKey = inputArgs?.dataSourceKey || view?.record?.dataSourceKey || 'main';
  const filterByTk = inputArgs?.filterByTk || view?.record?.filterByTk;
...
export function getViewRecordFromParent(flowContext, viewContext) {
...
export function createViewRecordResolveOnServer(ctx, getLocalRecord) {
  return (p) => {
 ...
 */
export function createRecordResolveOnServerWithLocal(collectionAccessor, valueAccessor) {
...
export function inferParentRecordRef(ctx) {
  const resource = ctx.resource;
 ...
 */
export async function buildRecordMeta(collectionAccessor, title, paramsBuilder) {
  const base = await createCollectionContextMeta(collectionAccessor, title, true)();
  if (!base) return null;
  return {
    ...base,
    buildVariablesParams: (ctx) => (paramsBuilder ? paramsBuilder(ctx) : inferRecordRef(ctx)),
  };
}
/**
 ...
 */
export function createRecordMetaFactory(collectionAccessor, title, paramsBuilder) {
  const factory = async () => buildRecordMeta(collectionAccessor, title, paramsBuilder);
  factory.title = title;
  return factory;
}
/**
 ...
 */
export function createCurrentRecordMetaFactory(ctx, collectionAccessor, options) {
 ...
 */
export async function collectContextParamsForTemplate(ctx, template) {
...
//# sourceMappingURL=variablesParams.js.map
