...
async function prefetchEsmModule(url, options) {
  ...
  if (!u) return;
  const g = globalThis;
  g.__nocobaseImportAsyncPrefetchCache = g.__nocobaseImportAsyncPrefetchCache || new Map();
  ...
  const task = (async () => {
    ...
    try {
      if (typeof document !== 'undefined' && document?.createElement) {
        const href = new URL(u, globalThis?.location?.href || undefined).toString();
 ...
 */
export async function runjsRequireAsync(requirejs, url) {
 ...
 */
export async function runjsImportAsync(url) {
  if (!url || typeof url !== 'string') {
    throw new Error('invalid url');
  }
  const u = url.trim();
  const shouldPatchAmd = !!globalThis?.define?.amd;
  // 尽量缩小全局副作用窗口：
...
function getEsmCdnBaseUrl() {
  ...
  const w = typeof window !== 'undefined' ? window : g?.window;
 ...
 */
export async function runjsImportModule(ctx, specifier, options) {
  ...
  const g = globalThis;
  g.__nocobaseImportAsyncCache = g.__nocobaseImportAsyncCache || new Map();
  ...
  if (parsed.name === 'react' && !subpath) {
    setRunJSLibOverride(ctx, 'React', mod);
