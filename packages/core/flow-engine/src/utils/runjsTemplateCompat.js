...
const PREPARE_RUNJS_CODE_CACHE = {
  withTemplates: new Map(),
  withoutTemplates: new Map(),
};
...
function extractUsedCtxLibKeys(code) {
  ...
  const scanTemplateLiteral = (start) => {
    let i = start;
    while (i < code.length) {
      const ch = code[i];
      const next = code[i + 1];
 ...
 *   的字符串替换行为。
 */
export function preprocessRunJsTemplates(code, options = {}) {
  ...
  const placeholderVars = new Map();
  const preambleLines = [];
  let needsStringifyHelper = false;
  const usedNames = new Set();
  const pickUniqueName = (base) => {
  ...
  const getPlaceholderVar = (placeholder) => {
 ...
 */
export async function prepareRunJsCode(code, options = {}) {
...
//# sourceMappingURL=runjsTemplateCompat.js.map
