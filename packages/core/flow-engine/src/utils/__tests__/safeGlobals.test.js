...
describe('safeGlobals', () => {
  it('createSafeWindow exposes only allowed globals and extras', () => {
    const win = createSafeWindow({ foo: 123 });
  ...
  it('createSafeDocument exposes whitelisted methods and extras', () => {
    const doc = createSafeDocument({ bar: true });
  ...
  it('createSafeNavigator exposes limited props and guards others', () => {
    ...
    // blocked properties
    expect(() => nav.geolocation).toThrow(/not allowed/);
    expect(() => nav.userAgent).toThrow(/not allowed/);
  });
});
//# sourceMappingURL=safeGlobals.test.js.map
