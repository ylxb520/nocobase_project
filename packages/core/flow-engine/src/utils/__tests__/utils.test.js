...
const createMockFlowModel = (overrides = {}) => {
  ...
  flowEngine.translate = vi.fn((key, options) => {
...
describe('Utils', () => {
  ...
  describe('getT()', () => {
    ...
    describe('fallback mechanism', () => {
      test('should return fallback function when no flowEngine', () => {
        const modelWithoutEngine = { flowEngine: null };
  ...
  describe('isInheritedFrom()', () => {
    ...
    let UnrelatedClass;
    beforeEach(() => {
      BaseClass = class extends FlowModel {};
      MiddleClass = class MiddleClass extends BaseClass {};
      DerivedClass = class DerivedClass extends MiddleClass {};
      UnrelatedClass = class extends FlowModel {};
    });
    ...
    describe('multi-level inheritance', () => {
      ...
      test('should handle deep inheritance chains', () => {
        ...
        expect(isInheritedFrom(Level3, BaseClass)).toBe(true);
        expect(isInheritedFrom(Level3, MiddleClass)).toBe(true);
        expect(isInheritedFrom(Level3, DerivedClass)).toBe(true);
        expect(isInheritedFrom(Level3, Level2)).toBe(true);
      });
    });
    describe('prototype chain validation', () => {
      test('should traverse prototype chain correctly', () => {
        // Create a complex inheritance chain
        const A = class extends FlowModel {};
      ...
      test('should handle null prototype correctly', () => {
        const NullProtoClass = function () {};
        Object.setPrototypeOf(NullProtoClass.prototype, null);
        expect(() => {
  ...
  describe('resolveDefaultOptions()', () => {
    ...
    describe('function options resolution', () => {
      ...
      test('should handle function accessing model properties', async () => {
        const optionsFn = vi.fn((parentContext) => ({
          use: 'DynamicModel',
  ...
  describe('shouldHideStepInSettings()', () => {
    ...
    test('evaluates function step.hideInSettings with FlowRuntimeContext', async () => {
      ...
      mockStep.hideInSettings = hideFn;
      const result = await shouldHideStepInSettings(mockModel, mockFlow, mockStep);
      expect(hideFn).toHaveBeenCalledTimes(1);
      const ctx = hideFn.mock.calls[0][0];
      expect(ctx).toBeInstanceOf(FlowRuntimeContext);
      expect(ctx.model).toBe(mockModel);
      expect(ctx.flowKey).toBe('testFlow');
      expect(ctx.mode).toBe('settings');
      expect(ctx.currentStep).toBeInstanceOf(ContextPathProxy);
      expect(String(ctx.currentStep)).toBe('{{ctx.currentStep}}');
      expect(result).toBe(true);
    });
  ...
  describe('resolveStepDisabledInSettings()', () => {
    ...
    test('returns disabled=false when step is falsy', async () => {
      const result = await resolveStepDisabledInSettings(mockModel, mockFlow, null);
    ...
    test('evaluates function disabled settings with FlowRuntimeContext', async () => {
      ...
      mockStep.disabledInSettings = disabledFn;
      mockStep.disabledReasonInSettings = reasonFn;
      const result = await resolveStepDisabledInSettings(mockModel, mockFlow, mockStep);
      expect(disabledFn).toHaveBeenCalledTimes(1);
      expect(reasonFn).toHaveBeenCalledTimes(1);
      const disabledCtx = disabledFn.mock.calls[0][0];
      const reasonCtx = reasonFn.mock.calls[0][0];
      expect(disabledCtx).toBeInstanceOf(FlowRuntimeContext);
      expect(reasonCtx).toBeInstanceOf(FlowRuntimeContext);
      expect(disabledCtx.currentStep).toBeInstanceOf(ContextPathProxy);
      expect(String(disabledCtx.currentStep)).toBe('{{ctx.currentStep}}');
    ...
    test('returns disabled=false when function disabledInSettings throws', async () => {
      const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      mockStep.disabledInSettings = vi.fn().mockRejectedValue(new Error('boom'));
      const result = await resolveStepDisabledInSettings(mockModel, mockFlow, mockStep);
      expect(consoleSpy).toHaveBeenCalled();
      expect(result).toEqual({ disabled: false });
      consoleSpy.mockRestore();
