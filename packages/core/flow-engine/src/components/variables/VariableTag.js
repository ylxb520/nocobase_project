...
const VariableTagComponent = ({ value, onClear, className, style, metaTreeNode, metaTree }) => {
  const { resolvedMetaTree } = useResolvedMetaTree(metaTree);
  const ctx = useFlowContext();
  const { data: displayedValue } = useRequest(
    async () => {
      const resolveLabelFromPath = async (rawPath) => {
  ...
  const customTagRender = (props) => {
    ...
    return React.createElement(
      Tooltip,
      { title: fullText, placement: 'top', getPopupContainer: () => document.body },
      React.createElement(
        Tag,
        {
          color: 'blue',
