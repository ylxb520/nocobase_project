...
const renderWithCtx = (ui) => {
  ...
  return render(React.createElement(FlowContextProvider, { context: engine.context }, ui));
};
describe('VariableTag', () => {
  it('should render with variable value and display parsed path', async () => {
    renderWithCtx(React.createElement(VariableTag, { value: '{{ ctx.User.Name }}' }));
    await waitFor(
      () => {
  ...
  it('should render with allowClear when onClear is provided', async () => {
    ...
    const { container } = renderWithCtx(
      React.createElement(VariableTag, { value: '{{ ctx.User.Email }}', onClear: onClear }),
    );
    await waitFor(
      () => {
  ...
  it('should not show close button when onClear is not provided', async () => {
    const { container } = renderWithCtx(React.createElement(VariableTag, { value: '{{ ctx.User.Name }}' }));
    await waitFor(
      () => {
  ...
  it('should have blue color by default', async () => {
    renderWithCtx(React.createElement(VariableTag, { value: '{{ ctx.Test }}' }));
    await waitFor(
      () => {
  ...
  it('should have proper styling for tag appearance', async () => {
    renderWithCtx(React.createElement(VariableTag, { value: '{{ ctx.Styled.Tag }}' }));
    await waitFor(
      () => {
  ...
  it('should render empty value', async () => {
    const { container } = render(React.createElement(VariableTag, { value: '' }));
    await waitFor(
      () => {
        const selectElement = container.querySelector('.ant-select');
        expect(selectElement).toBeInTheDocument();
  ...
  it('should handle undefined value gracefully', async () => {
    const { container } = render(React.createElement(VariableTag, null));
    await waitFor(
      () => {
        const selectElement = container.querySelector('.ant-select');
        expect(selectElement).toBeInTheDocument();
  ...
  it('should render Select component with proper structure', async () => {
    const { container } = render(React.createElement(VariableTag, { value: '{{ ctx.Test }}' }));
    await waitFor(
      () => {
  ...
  it('should show clear button when onClear is provided', async () => {
    const onClear = vi.fn();
    const { container } = renderWithCtx(
      React.createElement(VariableTag, { value: '{{ ctx.Test }}', onClear: onClear }),
    );
    await waitFor(
      () => {
        expect(screen.getByText('Test')).toBeInTheDocument();
  ...
  it('should have correct container styling when disabled', async () => {
    const { container } = render(React.createElement(VariableTag, { value: '{{ ctx.Test }}', onClear: undefined }));
    await waitFor(
      () => {
        expect(screen.getByText('Test')).toBeInTheDocument();
      },
  ...
  it('should have correct container styling when enabled', async () => {
    const onClear = vi.fn();
    const { container } = render(React.createElement(VariableTag, { value: '{{ ctx.Test }}', onClear: onClear }));
    await waitFor(
      () => {
        expect(screen.getByText('Test')).toBeInTheDocument();
      },
  ...
  it('should have proper accessibility attributes for Select component', async () => {
    const { container } = renderWithCtx(React.createElement(VariableTag, { value: '{{ ctx.Test }}' }));
    await waitFor(
      () => {
        expect(screen.getByText('Test')).toBeInTheDocument();
      },
  ...
  it('should fallback to title when parentTitles is not provided', async () => {
    ...
    renderWithCtx(
      React.createElement(VariableTag, { value: '{{ ctx.user.profile.firstName }}', metaTreeNode: mockMetaTreeNode }),
    );
  ...
  it('should render long text without ellipsis (visual overflow only)', async () => {
    ...
    renderWithCtx(React.createElement(VariableTag, { value: '{{ ctx.long.path }}', metaTreeNode: mockMetaTreeNode }));
    await waitFor(
      () => {
  ...
  it('should handle parentTitles correctly', async () => {
    ...
    renderWithCtx(
      React.createElement(VariableTag, { value: '{{ ctx.parent.child.field }}', metaTreeNode: mockMetaTreeNode }),
    );
    await waitFor(
      () => {
...
//# sourceMappingURL=VariableTag.test.js.map
