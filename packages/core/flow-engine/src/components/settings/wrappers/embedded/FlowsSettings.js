 ...
 */
const FlowsSettings = (props) => {
  if (isModelByIdProps(props)) {
    return React.createElement(FlowsSettingsWithModelById, { ...props });
  } else {
    return React.createElement(FlowsSettingsWithModel, { ...props });
  }
};
// 使用传入的model
const FlowsSettingsWithModel = observer(({ model, expandAll = false, children }) => {
  if (!model) {
    return React.createElement(Alert, { message: '\u63D0\u4F9B\u7684\u6A21\u578B\u65E0\u6548', type: 'error' });
  }
  const settingsContent = React.createElement(FlowsSettingsContent, { model: model, expandAll: expandAll });
  // 如果有children，作为wrapper模式
  if (children) {
    return React.createElement('div', null, settingsContent, children);
  }
  // 如果没有children，返回设置内容
  return settingsContent;
});
// 通过useModelById hook获取model
const FlowsSettingsWithModelById = observer(({ uid, modelClassName, expandAll = false, children }) => {
...
//# sourceMappingURL=FlowsSettings.js.map
