...
const FlowSettings = (props) => {
  if (isModelByIdProps(props)) {
    return React.createElement(FlowSettingsWithModelById, { ...props });
  } else {
    return React.createElement(FlowSettingsWithModel, { ...props });
  }
};
// 使用传入的model
const FlowSettingsWithModel = observer(({ model, flowKey }) => {
  if (!model) {
    return React.createElement(Alert, { message: '\u63D0\u4F9B\u7684\u6A21\u578B\u65E0\u6548', type: 'error' });
  }
  return React.createElement(FlowSettingsContent, { model: model, flowKey: flowKey });
});
// 通过useModelById hook获取model
const FlowSettingsWithModelById = observer(({ uid, flowKey, modelClassName }) => {
...
const FlowSettingsContent = observer(({ model, flowKey }) => {
  const [form] = Form.useForm();
  // 获取流程定义
  const flows = model.getFlows();
  const flow = flows.get(flowKey);
  const [configurableSteps, setConfigurableSteps] = useState([]);
  ...
  if (!flow) {
    return React.createElement(Alert, { message: `未找到Key为 ${flowKey} 的流程`, type: 'error' });
  }
      ...
      return Object.entries(uiSchema).map(([fieldKey, schema]) => {
        ...
        const renderField = () => {
          switch (schema['x-component']) {
            ...
            case 'Switch':
              return React.createElement(Switch, { ...(schema['x-component-props'] || {}) });
            case 'Input.TextArea':
  ...
  return React.createElement(
    Form,
    { form: form, layout: 'vertical', onValuesChange: handleValuesChange },
    renderFormFields(),
  );
});
export { FlowSettings };
//# sourceMappingURL=FlowSettings.js.map
