 ...
 */
const FlowsContextMenu = (props) => {
  if (isModelByIdProps(props)) {
    return React.createElement(FlowsContextMenuWithModelById, { ...props });
  } else {
    return React.createElement(FlowsContextMenuWithModel, { ...props });
  }
};
// 使用传入的model
const FlowsContextMenuWithModel = observer(
  ({ model, children, enabled = true, position = 'right', showDeleteButton = true }) => {
          ...
          flowsArray.map(async (flow) => {
            const configurableSteps = await Promise.all(
              Object.entries(flow.steps).map(async ([stepKey, stepDefinition]) => {
                ...
                const disabledState = await resolveStepDisabledInSettings(model, flow, actionStep);
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('div', { style: { display: 'inline-block', width: '100%' } }, children),
      ),
    );
  },
);
// 通过useModelById hook获取model
const FlowsContextMenuWithModelById = observer(
  ({ uid, modelClassName, children, enabled = true, position = 'right', showDeleteButton = true }) => {
    ...
    return React.createElement(
      ...
      { model: model, enabled: enabled, position: position, showDeleteButton: showDeleteButton },
      children,
    );
  },
);
