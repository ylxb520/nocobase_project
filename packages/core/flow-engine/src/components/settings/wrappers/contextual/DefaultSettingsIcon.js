 ...
 */
const findSubModelByKey = (model, subModelKey) => {
  // Input validation
...
const MenuLabelItem = ({ title, uiMode, itemProps }) => {
  ...
  const content = (() => {
    ...
    return React.createElement(Component, { title: title, ...itemProps });
  })();
  if (!disabled) {
    return content;
  }
  return React.createElement(
    ...
    React.createElement(
      Tooltip,
      { title: disabledReason, placement: 'right', destroyTooltipOnHide: true },
      React.createElement(QuestionCircleOutlined, { style: { color: disabledIconColor } }),
    ),
  );
};
export const DefaultSettingsIcon = ({
  ...
  const handleOpenChange = useCallback((nextOpen, info) => {
  ...
  const getModelConfigurableFlowsAndSteps = useCallback(async (targetModel, modelKey) => {
    try {
        ...
        flowsArray.map(async (flow) => {
          const configurableSteps = await Promise.all(
            Object.entries(flow.steps).map(async ([stepKey, stepDefinition]) => {
              ...
              const disabledState = await resolveStepDisabledInSettings(targetModel, flow, actionStep);
              let uiMode = await resolveUiMode(actionStep.uiMode, targetModel.context);
              // 检查是否有uiSchema（静态或动态）
              ...
              if (actionStep.use) {
                try {
                  ...
                  uiMode = await resolveUiMode(action?.uiMode || uiMode, targetModel.context);
  ...
  const menuItems = useMemo(() => {
    const items = [];
    const keyCounter = new Map(); // 跟踪重复的key
    // 生成唯一key的辅助函数
    const generateUniqueKey = (baseKey) => {
      ...
      if (flattenSubMenus) {
        ...
        configurableFlowsAndSteps.forEach(({ flow, steps, modelKey }) => {
          ...
          steps.forEach((stepInfo) => {
            ...
            items.push({
              key: uniqueKey,
              label: React.createElement(MenuLabelItem, {
                ...
                items.push({
                  ...
                  label: stepInfo.disabled
                    ? React.createElement(
                        ...
                        React.createElement(
                          Tooltip,
                          { title: stepInfo.disabledReason, placement: 'right', destroyTooltipOnHide: true },
                          React.createElement(QuestionCircleOutlined, { style: { color: disabledIconColor } }),
                        ),
                      )
                    : stepInfo.title,
...
//# sourceMappingURL=DefaultSettingsIcon.js.map
