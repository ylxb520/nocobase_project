...
function attachDefineChildren(Base, def) {
...
describe('subModel/utils', () => {
  describe('buildSubModelGroups', () => {
    ...
    it('invokes buildSubModelItems when meta.children is false', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-children-false' });
      ...
      expect(group.children.map((item) => item.key)).toEqual(['Derived']);
    });
    it('excludes subclasses already contributed by previous base classes', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-exclude-groups' });
      const ctx = parent.context;
      ...
      expect(second.children.map((item) => item.key)).toEqual(['SecondLeaf']);
    });
  });
  describe('buildSubModelItem', () => {
    ...
    it('maps meta fields and wraps children createModelOptions', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-child-group' });
      const ctx = parent.context;
      ...
      const childrenFactory = item?.children;
      ...
      const merged = await leaf.createModelOptions?.(ctx, { fromTest: true });
    ...
    it('falls back to use=current class name when meta createModelOptions omitted', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-default-create-options' });
  ...
  describe('buildSubModelItems', () => {
    it('sorts by meta.sort and respects exclusion list', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-sorting' });
      const ctx = parent.context;
    ...
    it('excludes subclasses by constructor reference', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-exclude-ctor' });
      const ctx = parent.context;
  ...
  describe('buildSubModelGroups label and resolution', () => {
    it('falls back to class key when meta label missing', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-label-fallback' });
      const ctx = parent.context;
    ...
    it('buildItems unwraps first group children', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-build-items' });
      const ctx = parent.context;
    ...
    it('buildItems returns empty array when no groups available', async () => {
      ...
      const parent = engine.createModel({ use: 'Parent', uid: 'parent-empty-build-items' });
      const ctx = parent.context;
  ...
  describe('mergeSubModelItems', () => {
    it('merges multiple arrays and inserts dividers when requested', async () => {
      const sourceA = [{ key: 'a', label: 'A' }];
      const sourceB = [{ key: 'b', label: 'B' }];
    ...
    it('returns original source when only one valid entry', () => {
      const original = [{ key: 'solo', label: 'Solo' }];
