...
describe('buildWrapperFieldChildren', () => {
  ...
  it('toggleable checks step params for fieldPath (using real FlowModel)', () => {
    ...
    const titleModel = engine.createModel({ use: 'FlowModel' });
    titleModel.setStepParams('fieldSettings', 'init', { fieldPath: 'title' });
    const statusModel = engine.createModel({ use: 'FlowModel' });
    statusModel.setStepParams('fieldSettings', 'init', { fieldPath: 'status' });
    expect(titleItem.toggleable(titleModel)).toBe(true);
    expect(titleItem.toggleable(statusModel)).toBe(false);
    expect(statusItem.toggleable(statusModel)).toBe(true);
  });
...
//# sourceMappingURL=buildWrapperFieldChildren.test.js.map
