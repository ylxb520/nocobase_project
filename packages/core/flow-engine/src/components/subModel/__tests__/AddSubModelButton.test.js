...
import { mergeSubModelItems, transformItems } from '../AddSubModelButton';
...
describe('AddSubModelButton - preset settings open on add', () => {
  test('calls openFlowSettings with preset=true for subModel with preset steps', async () => {
    ...
    const parent = engine.createModel({ use: 'ParentModel', uid: 'parent' });
...
describe('AddSubModelButton - async group children (nested)', () => {
  ...
  it('renders group and nested async group leaf items', async () => {
    ...
    const parent = engine.createModel({ use: 'Parent', uid: 'p1' });
    const items = async () => [
      {
        key: 'async-group',
        label: 'Async Group',
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            App,
            null,
            React.createElement(AddSubModelButton, { model: parent, subModelKey: 'items', items: items }, 'Open Menu'),
          ),
        ),
      ),
    );
...
describe('transformItems - searchable flags', () => {
  it('preserves searchable + placeholder on non-group submenu items', async () => {
    ...
    class Parent extends FlowModel {}
    engine.registerModels({ Parent });
    const parent = engine.createModel({ use: 'Parent' });
    const items = [
      {
        key: 'submenu',
        label: 'Pick',
    ...
    const factory = transformItems(items, parent, 'items', 'array');
    const resolved = await (typeof factory === 'function' ? factory() : factory);
    expect(resolved).toHaveLength(1);
    const submenu = resolved[0];
...
describe('transformItems - hide', () => {
  it('filters items by hide flag/function recursively', async () => {
    ...
    class Parent extends FlowModel {}
    engine.registerModels({ Parent });
    const parent = engine.createModel({ use: 'Parent', uid: 'p-hide' });
    const definition = [
      {
        key: 'keep',
        label: 'Keep',
    ...
    const resolved = await (typeof factory === 'function' ? factory() : factory);
    expect(resolved.map((i) => i.key)).toEqual(['keep', 'group']);
    const group = resolved.find((i) => i.key === 'group');
    expect(group).toBeTruthy();
    expect(Array.isArray(group.children)).toBe(true);
    expect(group.children.map((i) => i.key)).toEqual(['g-keep']);
  });
  it('removes group when all children are hidden (even with async hide)', async () => {
    ...
    class Parent extends FlowModel {}
    engine.registerModels({ Parent });
    const parent = engine.createModel({ use: 'Parent', uid: 'p-empty-group' });
    let show = false;
    const definition = [
      {
        key: 'group',
    ...
    expect(second.map((i) => i.key)).toEqual(['group']);
  });
  it('supports async hide functions and disables cache', async () => {
    ...
    class Parent extends FlowModel {}
    engine.registerModels({ Parent });
    const parent = engine.createModel({ use: 'Parent', uid: 'p-async-hide' });
    let shouldHide = true;
    const definition = [
      {
        key: 'async-hide',
    ...
    shouldHide = false;
    const second = await factory();
    expect(second.map((i) => i.key)).toEqual(['async-hide']);
  ...
  it('shows items when hide function throws (conservative fallback)', async () => {
    ...
    class Parent extends FlowModel {}
    engine.registerModels({ Parent });
    const parent = engine.createModel({ use: 'Parent', uid: 'p-hide-throws' });
    ...
    try {
      ...
      expect(resolved.map((i) => i.key)).toEqual(['throw']);
    } finally {
...
describe('transformItems - toggleable items', () => {
  ...
  it('marks toggleable item as active when matching sub model exists', async () => {
    ...
    const parent = engine.createModel({ use: 'ToggleParent', uid: 'toggle-parent-on' });
    const child = engine.createModel({ use: 'ToggleChild', uid: 'toggle-child-on' });
    ...
    const { getByRole } = render(React.createElement(React.Fragment, null, toggleItem.label));
    ...
    expect((parent.subModels.items || []).length).toBe(0);
  });
  it('infers useModel from createModelOptions when toggleable is enabled', async () => {
    const engine = setupEngine();
    const parent = engine.createModel({ use: 'ToggleParent', uid: 'toggle-parent-infer' });
    const child = engine.createModel({ use: 'ToggleChild', uid: 'toggle-child-infer' });
    parent.addSubModel('items', child);
    const definition = [
      {
        key: 'toggle-child',
  ...
  it('keeps toggleable item off when sub model missing', async () => {
    const engine = setupEngine();
    const parent = engine.createModel({ use: 'ToggleParent', uid: 'toggle-parent-off' });
    const definition = [
      {
        key: 'toggle-child',
        label: 'Toggle Child',
  ...
  it('respects keepDropdownOpen override on toggleable items', async () => {
    const engine = setupEngine();
    const parent = engine.createModel({ use: 'ToggleParent', uid: 'toggle-parent-keep' });
    const definition = [
      {
  ...
  it('removes object sub model via default remove handler when toggleDetector provided', async () => {
    ...
    const parent = engine.createModel({ use: 'ObjectParent', uid: 'object-parent' });
    const child = engine.createModel({ use: 'ObjectChild', uid: 'object-child' });
                ...
                items: [
                  {
                    key: 'toggle-object',
                    label: 'Toggle Object',
                    toggleDetector: async (ctx) => Boolean(ctx.model.subModels.single),
    ...
    await waitFor(() => expect(parent.subModels.single).toBeUndefined());
  });
});
describe('mergeSubModelItems', () => {
  it('merges multiple sources with divider insertion', async () => {
    const staticItems = [{ key: 'static', label: 'Static' }];
...
describe('transformItems - caching behaviour', () => {
  ...
  const setupEngine = () => {
    ...
    const parent = engine.createModel({ use: 'CacheParent', uid: 'cache-parent' });
    return { engine, parent };
  };
  it('reuses cached result when no toggleable items exist', async () => {
    const { parent } = setupEngine();
    const definition = [{ key: 'basic', label: 'Basic', createModelOptions: { use: 'CacheChild' } }];
    const factory = transformItems(definition, parent, 'items', 'array');
...
describe('AddSubModelButton - refreshTargets linkage', () => {
  it('clicking an item with refreshTargets triggers toggle recomputation on target branch', async () => {
    ...
    const parent = engine.createModel({ use: 'Parent', uid: 'p-refreshTargets' });
    const items = [
      {
        key: 'js',
        label: 'JS Group',
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              AddSubModelButton,
              { model: parent, subModelKey: 'subs', items: items, keepDropdownOpen: true },
              'Open',
            ),
          ),
        ),
...
describe('AddSubModelButton - base class menu groups', () => {
  ...
  it('renders async children provided by subModelBaseClasses', async () => {
    ...
    const parent = engine.createModel({ use: 'Parent', uid: 'parent-async-group' });
    render(
      React.createElement(
        FlowEngineProvider,
        { engine: engine },
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              AddSubModelButton,
              { model: parent, subModelKey: 'items', subModelBaseClasses: ['AsyncGroup'] },
              'Add Item',
            ),
          ),
        ),
  ...
  it('skips base class groups whose children resolve to empty', async () => {
    ...
    const parent = engine.createModel({ use: 'Parent', uid: 'parent-empty-group' });
    render(
      React.createElement(
        FlowEngineProvider,
        { engine: engine },
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              AddSubModelButton,
              { model: parent, subModelKey: 'items', subModelBaseClasses: ['EmptyGroup', 'NonEmptyGroup'] },
              'Open Menu',
            ),
          ),
        ),
  ...
  it('renders submenu base class with children and respects meta.sort', async () => {
    ...
    const parent = engine.createModel({ use: 'Parent', uid: 'parent-submenu' });
    render(
      React.createElement(
        FlowEngineProvider,
        { engine: engine },
  ...
  it('merges explicit items with base class and grouped sources', async () => {
    ...
    const parent = engine.createModel({ use: 'Parent', uid: 'parent-merged-sources' });
    render(
      React.createElement(
        FlowEngineProvider,
        { engine: engine },
...
describe('AddSubModelButton - toggle interactions', () => {
  it('removes existing toggleable sub model and triggers callbacks', async () => {
    ...
    engine.registerModels({ ToggleParent, ToggleChild });
    const parent = engine.createModel({ use: 'ToggleParent', uid: 'toggle-parent-remove' });
    const existing = engine.createModel({ use: 'ToggleChild', uid: 'toggle-child-existing' });
  ...
  it('creates toggleable sub model and runs lifecycle callbacks', async () => {
    ...
    engine.registerModels({ ToggleParent, ToggleChild });
    const parent = engine.createModel({ use: 'ToggleParent', uid: 'toggle-parent-add' });
    ...
    const subModels = parent.subModels.items || [];
...
describe('AddSubModelButton toggleable behavior', () => {
  ...
  function setup() {
    ...
    vi.spyOn(engine.flowSettings, 'open').mockResolvedValue(false);
    const parent = engine.createModel({ use: FlowModel });
    ...
    const ui = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            AddSubModelButton,
            { model: parent, items: items, subModelType: 'array', subModelKey: 'subs' },
            'Open',
          ),
        ),
      ),
  ...
  test('keeps dropdown open and preserves loaded children on toggle add/remove', async () => {
    ...
    const repo = engine.modelRepository;
    // toggle ON (add model)
        ...
        children: [
          {
            ...
            toggleable: (m) => m.getStepParams('fieldSettings', 'init')?.fieldPath === 'f1',
            useModel: 'ToggleModel',
            createModelOptions: {
              use: 'ToggleModel',
              stepParams: {
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            App,
            null,
            React.createElement(AddSubModelButton, { model: parent, items: items, subModelKey: 'subs' }, 'Open'),
          ),
        ),
      ),
    );
...
//# sourceMappingURL=AddSubModelButton.test.js.map
