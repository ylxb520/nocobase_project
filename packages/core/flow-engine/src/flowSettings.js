  ...
  async open(options) {
    ...
    for (const fk of targetFlowKeys) {
      ...
      for (const sk of Object.keys(flow.steps || {})) {
        ...
        flowRuntimeContext.defineMethod('getStepFormValues', (flowKey, stepKey) => {
    ...
    if (modeType === 'embed') {
      const target = document.querySelector(`#${GLOBAL_EMBED_CONTAINER_ID}`);
    ...
    // 为每个步骤创建独立的表单实例，互不干扰
    const forms = new Map();
    const keyOf = (e) => `${e.flowKey}::${e.stepKey}`;
    entries.forEach((e) => {
    ...
    openView({
      ...
      content: (currentView, viewCtx) => {
        viewCtx?.defineMethod('getStepFormValues', (flowKey, stepKey) => {
          return forms.get(keyOf({ flowKey, stepKey }))?.values;
        });
        // 渲染单个 step 表单（无 JSX）：FormProvider + SchemaField
        const renderStepForm = (entry) => {
        ...
        const renderStepsContainer = () => {
          ...
          return React.createElement(Tabs, { items, defaultActiveKey });
        };
...
function FormProviderWithForm({ children, form, initialValues, onFormValuesChange: _onFormValuesChange }) {
  const formInstanceRef = React.useRef(form);
  ...
  return React.createElement(FormProvider, { form: formInstanceRef.current }, children);
}
