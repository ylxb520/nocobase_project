...
const createBasicFlowDefinition = (overrides = {}) => ({
  key: 'testFlow',
  steps: {
    step1: {
...
const createAutoFlowDefinition = (overrides = {}) => ({
  key: 'autoFlow',
  sort: 1,
  steps: {
    autoStep: {
...
const createEventFlowDefinition = (eventName, overrides = {}) => ({
  key: `${eventName}Flow`,
  on: { eventName },
  steps: {
    eventStep: {
...
const createErrorFlowDefinition = (errorMessage = 'Test error', overrides = {}) => ({
  key: 'errorFlow',
  steps: {
    errorStep: {
...
describe('FlowModel', () => {
  ...
  describe('Constructor & Initialization', () => {
    ...
    test('should initialize with default values when options are minimal', () => {
      const model = new FlowModel({ flowEngine });
      ...
      describe('lifecycle hooks', () => {
        ...
        beforeEach(() => {
          ...
          TestFlowModelWithHooks = class extends TestFlowModel {
            async onDispatchEventStart(eventName, _opts, inputArgs) {
              if (eventName === 'beforeRender') beforeHookSpy(inputArgs);
            }
            async onDispatchEventEnd(eventName, _opts, inputArgs, results) {
              if (eventName === 'beforeRender') afterHookSpy(results, inputArgs);
            }
            async onDispatchEventError(eventName, _opts, inputArgs, error) {
        ...
        test("should allow onDispatchEventStart('beforeRender') to terminate flow via ctx.exit()", async () => {
          ...
          const TestFlowModelWithExitHooks = class extends TestFlowModel {
            ...
            async onDispatchEventEnd(eventName, _o, _i, _r) {
              if (eventName === 'beforeRender') afterHookSpy(_r, _i);
            }
            async onDispatchEventError(eventName, _o, i, e) {
    ...
    describe('dispatchEvent', () => {
      ...
      describe('debounce functionality', () => {
        test('should use debounced dispatch when debounce option is true', async () => {
          ...
          TestFlowModel.registerFlow(eventFlow);
          const _dispatchEventSpy = vi.spyOn(model, '_dispatchEvent');
          const _dispatchEventWithDebounceSpy = vi.spyOn(model, '_dispatchEventWithDebounce');
        ...
        test('should debounce multiple rapid calls when debounce is true', async () => {
          ...
          TestFlowModel.registerFlow(eventFlow);
          const handlerSpy = eventFlow.steps.eventStep.handler;
  ...
  describe('Relationships', () => {
    ...
    describe('parent-child relationships', () => {
      ...
      test('should not allow setting parent to null', () => {
        ...
        model.setParent(parent);
        expect(model.parent).toBe(parent);
        expect(() => model.setParent(null)).toThrow('Parent must be an instance of FlowModel');
      });
    });
    describe('subModels management', () => {
      ...
      describe('setSubModel (object type)', () => {
        test('should set single subModel with FlowModel instance', () => {
          ...
          expect(parentModel.subModels.testChild.uid).toBe(result.uid);
        ...
        test('should allow setSubModel via fork and bind to master', () => {
          ...
          const result = fork.setSubModel('testChildObject', childModel);
          expect(result.parent).toBe(parentModel);
          expect(parentModel.subModels['testChildObject']).toBe(result);
        });
        test('should allow multiple setSubModel via fork with same instance without error', () => {
          ...
          const fork = parentModel.createFork();
          const first = fork.setSubModel('testChildObject2', childModel);
          const second = fork.setSubModel('testChildObject2', childModel);
          expect(first).toBe(second);
          expect(second.parent).toBe(parentModel);
          expect(parentModel.subModels['testChildObject2']).toBe(second);
        });
      });
      describe('addSubModel (array type)', () => {
        test('should add subModel to array with FlowModel instance', () => {
          ...
          expect(parentModel.subModels.testChildren.some((model) => model.uid === result.uid)).toBe(true);
        ...
        test('should maintain sortIndex when adding to existing array', () => {
          ...
          parentModel.subModels.testChildren = [existingChild];
        ...
        test('should allow addSubModel via fork and bind to master', () => {
          ...
          const fork = parentModel.createFork();
          const result = fork.addSubModel('testChildren', childModel);
          expect(result.parent).toBe(parentModel);
          expect(Array.isArray(parentModel.subModels.testChildren)).toBe(true);
          expect(parentModel.subModels.testChildren.some((m) => m.uid === 'child-via-fork')).toBe(true);
        });
        test('should allow multiple addSubModel via fork with same instance without error', () => {
          ...
          const fork = parentModel.createFork();
          const r1 = fork.addSubModel('testChildren2', childModel);
          const r2 = fork.addSubModel('testChildren2', childModel);
          expect(r1).toBe(r2);
          expect(r1.parent).toBe(parentModel);
          const arr = parentModel.subModels['testChildren2'];
      ...
      describe('applySubModelsBeforeRenderFlows', () => {
        test('should apply auto flows to all array subModels', async () => {
          const child1 = new FlowModel({ uid: 'child1', flowEngine });
          const child2 = new FlowModel({ uid: 'child2', flowEngine });
          child1.dispatchEvent = vi.fn().mockResolvedValue(undefined);
          child2.dispatchEvent = vi.fn().mockResolvedValue(undefined);
        ...
        test('should apply auto flows to single subModel', async () => {
          ...
          child.dispatchEvent = vi.fn().mockResolvedValue(undefined);
      ...
      describe('subModels reactive behavior', () => {
        test('should trigger reactive updates when subModels change', () => {
          ...
          expect(parentModel.subModels.reactiveTest.uid).toBe('reactive-child');
      ...
      describe('subModels edge cases', () => {
        ...
        test('should handle setting subModel with same key multiple times', () => {
          ...
          expect(parentModel.subModels.sameKey.uid).toBe(child2.uid);
          expect(parentModel.subModels.sameKey.uid).toBe('child2');
        ...
        test('should handle adding subModels to different arrays', () => {
          ...
          expect(parentModel.subModels.arrayA.some((model) => model.uid === child1.uid)).toBe(true);
          expect(parentModel.subModels.arrayB.some((model) => model.uid === child2.uid)).toBe(true);
          expect(parentModel.subModels.arrayA.some((model) => model.uid === child2.uid)).toBe(false);
          expect(parentModel.subModels.arrayB.some((model) => model.uid === child1.uid)).toBe(false);
        });
      });
    });
  ...
  describe('Lifecycle', () => {
    ...
    describe('rendering operations', () => {
      ...
      test('should rerender triggers beforeRender without cache', async () => {
        const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        model.dispatchEvent = vi.fn().mockResolvedValue(undefined);
        try {
  ...
  describe('Title Management', () => {
    ...
    describe('title getter', () => {
      ...
      test('should handle null and undefined instance titles', () => {
        ...
        const modelWithMeta = new TestFlowModel(modelOptions);
        // Test with null
        modelWithMeta.setTitle(null);
        expect(modelWithMeta.title).toBe('Meta Title');
        // Test with undefined
        modelWithMeta.setTitle(undefined);
        expect(modelWithMeta.title).toBe('Meta Title');
      });
    });
  ...
  describe('Cache Invalidation', () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(FlowEngine, 'generateApplyFlowCacheKey').mockImplementation(
        (prefix, type, modelUid) => `${prefix}-${type}-${modelUid}`,
      );
      model = new FlowModel({
        uid: 'test-model-uid',
  ...
  describe('Expression Resolution', () => {
    ...
    describe('{{ctx.xxx.yyy.zzz}} expression resolution', () => {
      test('should resolve simple ctx expressions in step parameters', async () => {
        ...
        const handlerSpy = flow.steps.testStep.handler;
        await model.dispatchEvent('beforeRender');
      ...
      test('should resolve nested ctx expressions with multiple levels', async () => {
        ...
        const handlerSpy = flow.steps.nestedStep.handler;
        await model.dispatchEvent('beforeRender');
        expect(handlerSpy).toHaveBeenCalled();
      });
    });
...
//# sourceMappingURL=flowModel.test.js.map
