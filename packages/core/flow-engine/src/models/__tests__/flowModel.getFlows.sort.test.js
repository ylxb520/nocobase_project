...
describe('FlowModel.getFlows sorting and getEventFlows(beforeRender) order', () => {
  ...
  beforeEach(() => {
    ...
    const fakeEngine = { getModel: () => undefined };
    model = new TestFlowModel({ flowEngine: fakeEngine });
  });
  ...
  test("getEventFlows('beforeRender') keeps getFlows order and filters out manual/on flows", () => {
    ...
    // excluded by filters
    model.registerFlow('eventFlow', { title: 'Evt', on: 'click', sort: 1, steps: {} });
  ...
  test('getFlows tie-breaker: parent static before child static when sort equal', () => {
    ...
    const fakeEngine = { getModel: () => undefined };
    const instance = new Child({ flowEngine: fakeEngine });
  ...
  test('GlobalFlowRegistry.getFlows sorted: parent static before child static on tie', () => {
    ...
    class C extends P {}
    const fakeEngine = { getModel: () => undefined };
    const inst = new C({ flowEngine: fakeEngine });
    P.registerFlow('p', { steps: {}, sort: 1 });
    C.registerFlow('c', { steps: {}, sort: 1 });
    const staticFlows = C.globalFlowRegistry.getFlows();
  ...
  test('instance flows keep registration order when sort equal', () => {
    ...
    model.registerFlow('i1', { title: 'I1', sort: 1, steps: {} });
    model.registerFlow('i2', { title: 'I2', sort: 1, steps: {} });
    model.registerFlow('i3', { title: 'I3', sort: 1, steps: {} });
    // add some static flows to ensure grouping doesn't affect instance intra-order
    TestFlowModel.registerFlow('s1', { title: 'S1', sort: 1, steps: {} });
    TestFlowModel.registerFlow('s2', { title: 'S2', sort: 2, steps: {} });
    const keys = Array.from(model.getFlows().keys());
...
//# sourceMappingURL=flowModel.getFlows.sort.test.js.map
