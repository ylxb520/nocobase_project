...
describe('FlowModel scheduleModelOperation cross-model (target not created yet)', () => {
  it('should apply modification when target is created (when: created)', async () => {
    ...
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-created-1' });
    ...
    const to = engine.createModel({ use: 'FlowModel', uid: targetUid });
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(to.getProps().foo).toBe('bar');
  });
  it('should apply modification when target is mounted (when: mounted)', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-mounted-1' });
  ...
  it("should apply modification when beforeRender ends (when: 'event:beforeRender:end')", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-ready-1' });
  ...
  it("model.scheduleModelOperation proxies engine and runs on 'event:beforeRender:end'", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-model-proxy-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-model-proxy-1' });
    const fn = vi.fn();
  ...
  it("model.scheduleModelOperation triggers immediately on 'created' when target exists", async () => {
    const engine = newEngine();
    const target = engine.createModel({ use: 'FlowModel', uid: 'to-model-proxy-created' });
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-model-proxy-created' });
    const fn = vi.fn();
  ...
  it('model.scheduleModelOperation delegates to engine.scheduleModelOperation', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-model-proxy-delegate' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-model-proxy-delegate' });
    const cb = vi.fn();
    const spy = vi.spyOn(engine, 'scheduleModelOperation');
    from.scheduleModelOperation(to.uid, cb, { when: 'created' });
    expect(spy).toHaveBeenCalledTimes(1);
    const [argFrom, argUid, argFn, argOptions] = spy.mock.calls[0];
...
//# sourceMappingURL=flowModel.scheduleModelOperation.test.js.map
