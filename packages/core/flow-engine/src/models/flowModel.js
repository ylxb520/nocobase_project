...
const classActionRegistries = new WeakMap();
// 使用 WeakMap 为每个类缓存一个 ModelEventRegistry 实例
const classEventRegistries = new WeakMap();
// 使用WeakMap存储每个类的meta
const modelMetas = new WeakMap();
// 使用WeakMap存储每个类的 GlobalFlowRegistry
const modelGlobalRegistries = new WeakMap();
const classMenuExtensions = new WeakMap();
export var ModelRenderMode;
(function (ModelRenderMode) {
  ModelRenderMode['ReactElement'] = 'reactElement';
  ModelRenderMode['RenderFunction'] = 'renderFunction';
...
export class FlowModel {
  ...
  renderHiddenInConfig() {
    return React.createElement('span', { style: { opacity: 0.5 } }, this.translate?.('Hidden') || 'Hidden');
  }
  uid;
  sortIndex;
  hidden = false;
   ...
   */
  static renderMode = ModelRenderMode.ReactElement;
  constructor(options) {
  ...
  invalidateFlowCache(eventName, deep = false) {
    if (this.flowEngine) {
      ...
      // 同步失效所有 fork 的缓存
      this.forks.forEach((fork) => {
        const forkScope = `event:${fork.getFlowCacheScope?.(eventName || 'beforeRender') ?? String(fork['forkId'])}`;
  ...
  getActions() {
    ...
    const globalActions = this.flowEngine?.getActions();
  ...
  getEvents() {
    ...
    const globalEvents = this.flowEngine?.getEvents();
  ...
  getEvent(name) {
    ...
    if (own) return own;
    return this.flowEngine?.getEvent(name);
  }
  /**
  ...
  getAction(name) {
    ...
    if (own) return own;
    return this.flowEngine?.getAction(name);
  }
  getFlows() {
  ...
  async applyFlow(flowKey, inputArgs, runId) {
    ...
    console.log(
      `[FlowModel] applyFlow: uid=${this.uid}, flowKey=${flowKey}, isFork=${isFork}, cleanRun=${
        this.cleanRun
      }, targetIsFork=${target?.isFork === true}`,
    );
  ...
  _dispatchEventWithDebounce = _.debounce(
  ...
  async dispatchEvent(eventName, inputArgs, options) {
    ...
    const execOptions = {
      sequential: options?.sequential ?? defaults.sequential,
      useCache: options?.useCache ?? defaults.useCache,
    };
   ...
   */
  _rerunLastAutoRun = _.debounce(async () => {
  ...
  setupReactiveRender() {
    ...
    try {
      ...
      if (this.shouldSkipReactiveWrapping()) {
        const wrappedNonReactive = function () {
      ...
      const createReactiveWrapper = (modelInstance) => {
        const ReactiveWrapper = observer(() => {
          ...
          if (renderTarget !== modelInstance && renderTarget?.localProps !== undefined) {
  ...
  setParent(parent) {
    ...
    const isValidParent =
      parent && (parent.constructor === FlowModel || isInheritedFrom(parent.constructor, FlowModel));
    if (!isValidParent) {
  ...
  createFork(localProps, key, options) {
    ...
    const fork = new ForkFlowModel(this, localProps, forkId);
  ...
  clone() {
    ...
    const replaceUidReferences = (data, isRoot = false) => {
  ...
  static registerExtraMenuItems(opts) {
    const ModelClass = this;
    const registry = classMenuExtensions.get(ModelClass) || new Set();
    let entry;
...
export class ErrorFlowModel extends FlowModel {
  ...
  render() {
    return React.createElement(Typography.Text, { type: 'danger' }, this.errorMessage);
  }
}
