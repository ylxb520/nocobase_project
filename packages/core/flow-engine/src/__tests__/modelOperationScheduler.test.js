...
import { FlowEngine, createViewScopedEngine, createBlockScopedEngine } from '..';
function newEngine() {
...
describe('ModelOperationScheduler', () => {
  it("should execute on 'event:beforeRender:end' when beforeRender ends", async () => {
    ...
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-1' });
    const calls = [];
  ...
  it('should execute on mounted when model is mounted via ReactView', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-mount-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-mount-1' });
    const fn = vi.fn();
  ...
  it("should execute on generic event start ('event:foo:start')", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-evt-start-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-evt-start-1' });
    const fn = vi.fn();
  ...
  it("should execute on generic event end ('event:foo:end')", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-evt-end-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-evt-end-1' });
    const fn = vi.fn();
  ...
  it("should execute on generic event error ('event:foo:error')", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-evt-error-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-evt-error-1' });
  ...
  it('should execute on unmounted when model is unmounted (no immediate)', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-unmount-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-unmount-1' });
    const fn = vi.fn();
  ...
  it('should support predicate for when', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-pred-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-pred-1' });
    const fn = vi.fn();
    engine.scheduleModelOperation(from, to.uid, fn, {
      when: (e) => e.type === 'event:beforeRender:end' && e.uid === to.uid,
    });
    await engine.executor.dispatchEvent(to, 'beforeRender');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  it('should execute once when beforeRender ends (no immediate for existing target)', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-imd-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-imd-1' });
    const fn = vi.fn();
  ...
  it("should execute immediately when target already exists and when='created'", async () => {
    const engine = newEngine();
    // 先创建目标模型
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-created-immediate' });
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-created-immediate' });
    const fn = vi.fn();
  ...
  it("should execute when 'event:beforeRender:end' after beforeRender ends (no immediate)", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-2' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-2' });
    const calls = [];
    engine.scheduleModelOperation(
      from,
      to.uid,
  ...
  it("should run only once for 'event:beforeRender:end'", async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-3' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-3' });
    const calls = [];
    engine.scheduleModelOperation(
      from,
      to.uid,
  ...
  it("should run only once for 'event:beforeRender:end' regardless of events", async () => {
    const engine = new FlowEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-repeat-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-repeat-1' });
    const fn = vi.fn();
    engine.scheduleModelOperation(from, to.uid, fn, { when: 'event:beforeRender:end' });
    await engine.executor.dispatchEvent(to, 'beforeRender');
    await new Promise((resolve) => setTimeout(resolve, 0));
  ...
  it('should remove scheduled item after execution (cancel returns false)', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-auto-remove-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-auto-remove-1' });
    const fn = vi.fn();
  ...
  it('should still execute once even if source model destroyed later', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-4' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-4' });
    const fn = vi.fn();
    engine.scheduleModelOperation(from, to.uid, fn, { when: 'event:beforeRender:end' });
  ...
  it('should trigger on target destroyed', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-5' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-5' });
    const fn = vi.fn();
  ...
  it('should schedule via block engine but execute in parent view engine scope', async () => {
    ...
    const from = view.createModel({ use: 'FlowModel', uid: 'from-8' });
    const to = view.createModel({ use: 'FlowModel', uid: 'to-8' });
    const fn = vi.fn();
    // 调用来源是 block 引擎
    block.scheduleModelOperation(from, to.uid, fn, { when: 'event:beforeRender:end' });
    // 事件由 view 引擎派发（与 block 调度兼容，调度对象由 view 引擎维护）
    await view.executor.dispatchEvent(to, 'beforeRender');
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(fn).toHaveBeenCalledTimes(1);
  });
  it('executes multiple scheduled callbacks exactly once (after events)', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-multi-1' });
    const to = engine.createModel({ use: 'FlowModel', uid: 'to-multi-1' });
    const a = vi.fn();
    const b = vi.fn();
    // 先注册，再触发事件
  ...
  it('on target destroyed: only destroyed-matching executes; others are cancelled', async () => {
    const engine = newEngine();
    const from = engine.createModel({ use: 'FlowModel', uid: 'from-destroy-1' });
    ...
    const to = engine.createModel({ use: 'FlowModel', uid: toUid });
    await to.destroy();
    await new Promise((resolve) => setTimeout(resolve, 10));
...
//# sourceMappingURL=modelOperationScheduler.test.js.map
