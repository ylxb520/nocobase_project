...
describe('FlowModelContext.openView - navigation enforcement', () => {
  function setup() {
    ...
    const child = {
      ...
      dispatchEvent: vi.fn(async (_event, _params) => undefined),
    };
    // inject loadModel so ctx.openView finds existing model and skips createModel('PopupActionModel')
    engine.loadModel = vi.fn(async ({ uid }) => (uid ? child : null));
  ...
  it('forces options.navigation=false when options.defineProperties exists', async () => {
    ...
    await parent.context.openView('child-uid', {
      mode: 'drawer',
      navigation: true,
      defineProperties: {
        foo: { get: () => 'bar' },
    ...
    const viewCalls = child.context.defineProperty.mock.calls.filter((args) => args?.[0] === 'view');
...
//# sourceMappingURL=flowModel.openView.navigation.test.js.map
