...
describe('RunJS context contributions', () => {
  it('should apply contribution during setupRunJSContexts()', async () => {
    ...
    registerRunJSContextContribution(({ version, RunJSContextRegistry: Registry, FlowRunJSContext: BaseCtx }) => {
    ...
    expect(ctor.name).toBe('PluginTestRunJSContext');
    const ctx = { model: { constructor: { name: 'PluginTestModel' } } };
  ...
  it('should apply late contribution immediately after setup', async () => {
    ...
    await setupRunJSContexts();
    registerRunJSContextContribution(({ version, FlowRunJSContext }) => {
      if (version !== 'v1') return;
...
//# sourceMappingURL=runjsContributions.test.js.map
