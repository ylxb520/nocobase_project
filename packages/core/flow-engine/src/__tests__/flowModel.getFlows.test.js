...
describe('FlowModel.getFlow(s) with instance + global registries', () => {
  ...
  it('merges static(global) and instance flows; instance overrides static', async () => {
    ...
    childGR.addFlow('shared', {
      ...
      steps: { s: { title: 'S', handler: (ctx, params) => {} } },
    });
    const parentGR = BaseModel.globalFlowRegistry;
    parentGR.addFlow('baseOnly', {
      title: 'FromBaseStatic',
      steps: { p: { title: 'P', handler: (ctx, params) => {} } },
    });
    const model = engine.createModel({ use: 'ChildModel', uid: 'm1' });
    // 实例添加 flow
    model.flowRegistry.addFlow('instanceOnly', {
      title: 'FromInstance',
      steps: { i: { title: 'I', handler: (ctx, params) => {} } },
    });
    // 读取单个 flow
    ...
    model.flowRegistry.addFlow('shared', {
      ...
      steps: { ii: { title: 'II', handler: (ctx, params) => {} } },
    });
...
//# sourceMappingURL=flowModel.getFlows.test.js.map
