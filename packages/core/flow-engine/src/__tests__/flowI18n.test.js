...
describe('FlowI18n', () => {
  it('translates plain keys and templates', () => {
    const i18n = new FlowI18n({ i18n: { t: (k, _o) => (k === 'Hello' ? '你好' : k) } });
  ...
  it('template compile ignores malformed options', () => {
    ...
    const i18n = new FlowI18n({ i18n: { t: (k) => k } });
    // malformed options will be treated as no options, key is translated directly
    const s = i18n.translate("{{ t('X', oops) }}");
    expect(s).toBe('X');
    spy.mockRestore();
  });
});
//# sourceMappingURL=flowI18n.test.js.map
