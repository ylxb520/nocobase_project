...
describe('ViewScopedFlowEngine', () => {
  ...
  it('isolates model instances map by default', () => {
    ...
    class TestModel extends FlowModel {}
    const uid = 'same-uid';
    const pm = parent.createModel({ use: TestModel, uid });
    const cm = child.createModel({ use: TestModel, uid });
  ...
  it('isolates beforeRender event cache across engines with identical model uid', async () => {
    ...
    const pm = parent.createModel({ use: CounterModel, uid });
    const cm = child.createModel({ use: CounterModel, uid });
  ...
  it('creates resources using parent registry but binds to scoped context by default', () => {
    ...
    expect(res.context.foo).toBe(123);
  });
  it('uses local context and delegates to parent context', () => {
    ...
    expect(child.context.foo).toBe(42);
  });
  it('delegates saveModel to parent (concurrency gate sharing)', async () => {
    ...
    const m = child.createModel({ use: T });
  ...
  it('global model lookup traverses from top to root across stack', () => {
    ...
    const mRoot = root.createModel({ use: TM, uid });
    const m1 = c1.createModel({ use: TM, uid });
    const m2 = c2.createModel({ use: TM, uid });
  ...
  it('unlinkFromStack detaches only the top engine', () => {
    ...
    const resolved = root.getModel(uid, true);
  ...
  it('hydrateModelFromPreviousEngines returns null for ALL subModels when parent has flowSettingsEnabled (bug fix: ensures all children rebuild)', async () => {
    ...
    const parentModel = root.createModel({ use: ParentModel, uid: 'parent-with-settings' });
    const child1 = root.createModel({ use: ChildModel, uid: 'child-1' });
    const child2 = root.createModel({ use: ChildModel, uid: 'child-2' });
    ...
    const result1 = scoped.hydrateModelFromPreviousEngines({
      parentId: 'parent-with-settings',
      subKey: 'popup',
    });
    const result2 = scoped.hydrateModelFromPreviousEngines({
      parentId: 'parent-with-settings',
      subKey: 'items',
    });
  ...
  it('hydrateModelFromPreviousEngines returns hydrated model when parent does NOT have flowSettingsEnabled', async () => {
    ...
    class ChildModel extends FlowModel {}
    root.registerModels({ ParentModel, ChildModel });
    const parentModel = root.createModel({ use: ParentModel, uid: 'parent-normal' });
    const child = root.createModel({ use: ChildModel, uid: 'child-normal' });
    ...
    const result = scoped.hydrateModelFromPreviousEngines({
...
//# sourceMappingURL=viewScopedFlowEngine.test.js.map
