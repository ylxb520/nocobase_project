...
vi.mock('antd', () => {
  const Collapse = Object.assign((props) => 'Collapse', { Panel: (props) => 'Panel' });
  const FormItem = ({ children }) => children ?? 'FormItem';
  ...
  const InputNumber = (props) => 'InputNumber';
  ...
  return {
    ...
    ConfigProvider: ({ children }) => children ?? 'ConfigProvider',
    theme: { useToken: () => ({}) },
  };
});
...
const findPrimaryButton = async (node) => {
...
// Updated to work with new Cancel/Save component structure
const findCancelButton = async (node) => {
  const { Button } = await import('antd');
  const walk = (n) => {
    if (!n || typeof n !== 'object') return null;
...
describe('FlowSettings.open rendering behavior', () => {
  ...
  const createIsolatedFlowModel = (testId) => {
  ...
  it('renders single-step form directly when flowKey+stepKey provided (no Collapse)', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'testFlow', stepKey: 'general', uiMode: 'dialog' });
  ...
  it('renders Collapse when only one step matched but no stepKey provided', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'testFlow2', uiMode: 'dialog' });
    const container = screen.getByTestId('flow-settings-container');
    expect(container).toBeInTheDocument();
  ...
  it('uses drawer uiMode when specified', async () => {
    ...
    const openSpy = { lastTree: null };
    const viewerDrawer = vi.fn(({ content }) => {
      const dlg = { close: vi.fn(), Footer: (props) => null };
    ...
    await flowSettings.open({ model, flowKey: 'f', uiMode: 'drawer' });
  ...
  it('saves successfully and calls hooks, messages, and close', async () => {
    ...
    const save = vi.spyOn(model, 'saveStepParams').mockResolvedValue(undefined);
    ...
    model.context.defineProperty('viewer', {
      value: {
        dialog: ({ content }) => {
          lastDialog = { close: vi.fn(), Footer: (p) => null };
          lastTree = typeof content === 'function' ? content(lastDialog) : null;
          // return dialog object
          return lastDialog;
        },
        drawer: ({ content }) => model.context.viewer.dialog({ content }),
      },
    });
    await flowSettings.open({ model, flowKey: 'fx', stepKey: 'general', uiMode: 'dialog' });
  ...
  it('calls onSaved callback after successful save', async () => {
    ...
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    vi.spyOn(model, 'save').mockResolvedValue(undefined);
    let lastTree;
    let lastDialog;
    model.context.defineProperty('viewer', {
      value: {
    ...
    const onSaved = vi.fn();
    await flowSettings.open({ model, flowKey: 'flow', stepKey: 'step', onSaved });
    const primaryBtn = await findPrimaryButton(lastTree);
    expect(primaryBtn).toBeTruthy();
    await primaryBtn.props.onClick?.();
  ...
  it('calls onCancel callback when cancel button clicked', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'flow', stepKey: 'step', onCancel });
  ...
  it('handles save error by showing error message and keeping dialog open', async () => {
    ...
    const success = vi.fn();
    model.context.defineProperty('message', { value: { info, error, success } });
    vi.spyOn(model, 'saveStepParams').mockRejectedValue(new Error('boom'));
    let lastTree;
    let lastDialog;
    model.context.defineProperty('viewer', {
      value: {
    ...
    await flowSettings.open({ model, flowKey: 'fy', stepKey: 's' });
    const primaryBtn = await findPrimaryButton(lastTree);
    expect(primaryBtn).toBeTruthy();
    await primaryBtn.props.onClick?.();
  ...
  it('filters steps by preset when preset=true', async () => {
    ...
    const dialog = vi.fn(({ content }) => {
      const dlg = { close: vi.fn(), Footer: (p) => null };
    ...
    await flowSettings.open({ model, flowKey: 'pf', preset: true });
  ...
  it('shows info when preset=true but no step is preset', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'pf2', preset: true });
  ...
  it('shows dialog when preset=true and step has hideInSettings=true', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    await flowSettings.open({ model, flowKey: 'pf3', preset: true });
  ...
  it('respects hideInSettings when preset=false', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    await flowSettings.open({ model, flowKey: 'pf5', preset: false });
  ...
  it('sets title to step title when flowKey+stepKey are provided and only one step matches', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'tf-step', stepKey: 'general' });
  ...
  it('sets title to flow title when only one flow and no stepKey', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'tf-flow' });
    expect(dialog).toHaveBeenCalledTimes(1);
  });
  ...
  it('resolves function-based step uiMode when single step is rendered', async () => {
    ...
    M.registerFlow({
      ...
      steps: {
        step: {
          title: 'Step',
          uiMode: (ctx) => ({ type: 'drawer', props: { title: 'Function Title', width: 800 } }),
          uiSchema: { f: { type: 'string', 'x-component': 'Input' } },
        },
      },
    });
    ...
    model.context.defineProperty('viewer', { value: { drawer, dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithFunctionUiMode', stepKey: 'step' });
    expect(drawer).toHaveBeenCalledTimes(1);
    expect(dialog).not.toHaveBeenCalled();
  });
  it('resolves async function-based step uiMode when single step is rendered', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithAsyncFunctionUiMode', stepKey: 'step' });
    expect(dialog).toHaveBeenCalledTimes(1);
  });
  it('uses step static uiMode object when single step is rendered', async () => {
    ...
    model.context.defineProperty('viewer', { value: { drawer, dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithStaticUiMode', stepKey: 'step' });
    expect(drawer).toHaveBeenCalledTimes(1);
    expect(dialog).not.toHaveBeenCalled();
  });
  ...
  it('ignores step uiMode when multiple steps are rendered', async () => {
    ...
    M.registerFlow({
      ...
      steps: {
        ...
        step2: {
          title: 'Step2',
          uiMode: (ctx) => ({ type: 'drawer', props: { title: 'Should also be ignored', width: 888 } }),
          uiSchema: { g: { type: 'string', 'x-component': 'Input' } },
        },
      },
    });
  ...
  it('handles error in function-based step uiMode gracefully', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithErrorUiMode', stepKey: 'step' });
    expect(dialog).toHaveBeenCalledTimes(1);
  ...
  it('supports reactive objects in uiMode function and auto-updates dialog props', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithReactiveUiMode', stepKey: 'step' });
    expect(dialog).toHaveBeenCalledTimes(1);
  ...
  it('properly disposes reactive listener when dialog is closed', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithReactiveDispose', stepKey: 'step' });
    expect(dialog).toHaveBeenCalledTimes(1);
  ...
  it('handles async reactive uiMode function updates', async () => {
    ...
    model.context.defineProperty('viewer', { value: { dialog } });
    model.context.defineProperty('message', { value: { info: vi.fn(), error: vi.fn(), success: vi.fn() } });
    await flowSettings.open({ model, flowKey: 'flowWithAsyncReactiveUiMode', stepKey: 'step' });
    expect(dialog).toHaveBeenCalledTimes(1);
  ...
  it('assigns submit method to currentDialog and can be called externally', async () => {
    ...
    model.context.defineProperty('viewer', {
      value: {
        dialog: ({ content }) => {
          capturedDialog = { close: vi.fn(), Footer: (p) => null };
          if (typeof content === 'function') {
    ...
    await flowSettings.open({ model, flowKey: 'submitFlow', stepKey: 'step' });
  ...
  it('submit method handles multiple steps correctly', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'multiStepFlow' });
    // Call submit method
    await capturedDialog.submit();
  ...
  it('submit method handles FlowExitAllException by closing dialog without error message', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'exitFlow', stepKey: 'step' });
    // Call submit method
    await capturedDialog.submit();
  ...
  it('submit method keeps dialog open on FlowCancelSaveException', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'cancelSaveFlow', stepKey: 'step' });
    await capturedDialog.submit();
  ...
  it('submit method handles general errors by showing error message and keeping dialog open', async () => {
    ...
    vi.spyOn(model, 'saveStepParams').mockRejectedValue(new Error('Save failed'));
    let capturedDialog;
    model.context.defineProperty('viewer', {
      value: {
        dialog: ({ content }) => {
    ...
    const originalConsoleError = console.error;
    console.error = vi.fn();
    await flowSettings.open({ model, flowKey: 'errorFlow', stepKey: 'step' });
    // Call submit method
    await capturedDialog.submit();
  ...
  it('submit method calls onSaved callback after successful save', async () => {
    ...
    const onSaved = vi.fn();
    await flowSettings.open({ model, flowKey: 'callbackFlow', stepKey: 'step', onSaved });
    // Call submit method
    await capturedDialog.submit();
  ...
  it('submit method handles onSaved callback errors gracefully', async () => {
    ...
    await flowSettings.open({ model, flowKey: 'callbackErrorFlow', stepKey: 'step', onSaved });
    // Call submit method
    await capturedDialog.submit();
...
//# sourceMappingURL=flowSettings.open.test.js.map
