...
vi.mock('antd', () => {
  ...
  const FormItem = vi.fn(({ children }) => children ?? 'FormItem');
  ...
  return {
    ...
    Alert,
    Typography: {
      Paragraph: vi.fn(({ children }) => children ?? 'Paragraph'),
      Text: vi.fn(({ children }) => children ?? 'Text'),
    },
...
describe('FlowSettings', () => {
  ...
  describe('Step Settings Dialog', () => {
    ...
    test('should handle dialog errors', async () => {
      const { openStepSettingsDialog } = await import('../components/settings/wrappers/contextual/StepSettingsDialog');
      openStepSettingsDialog.mockRejectedValueOnce(new Error('Dialog error'));
      const props = {
        model: new FlowModel({ uid: 'test-model', flowEngine: new FlowEngine() }),
        flowKey: 'test-flow',
        stepKey: 'test-step',
  ...
  describe('Complex Integration Scenarios', () => {
    ...
    test('should handle complex toolbar sorting scenarios', () => {
      ...
      const items = [
        ...
        { key: 'item-b', component: () => 'B' },
        { key: 'item-c', component: () => 'C', sort: 30 },
        { key: 'item-d', component: () => 'D', sort: 10 },
...
//# sourceMappingURL=flowSettings.test.js.map
