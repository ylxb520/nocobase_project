...
describe('FlowContext properties and methods', () => {
  ...
  it('should support single-level mixed sync and async properties in getPropertyMetaTree', async () => {
    ...
    // 调用异步函数获取子节点
    const asyncChildren = await tree[1].children();
  ...
  it('should support multi-level mixed sync and async properties in getPropertyMetaTree', async () => {
    ...
    const children = complexNode.children;
  ...
  it('should define and call instance method with defineMethod', () => {
    const ctx = new FlowContext();
    ctx.defineMethod('hello', function (name) {
      return `Hello, ${name}!`;
    });
  ...
  it('should support method lookup and this binding in delegate chain', () => {
    const delegate = new FlowContext();
    delegate.defineMethod('add', function (a, b) {
      // this 指向 delegate
  ...
  it('should allow local defineMethod to override delegate method', () => {
    const delegate = new FlowContext();
    delegate.defineMethod('greet', function (name) {
      return `Hello from delegate, ${name}`;
    });
    const ctx = new FlowContext();
    ctx.addDelegate(delegate);
    // 覆盖 delegate 的 greet 方法
    ctx.defineMethod('greet', function (name) {
  ...
  it('should define and call method that uses property', () => {
    ...
    ctx.defineMethod('getFooPlus', function (n) {
      return this.foo + n;
    });
...
describe('FlowContext.getApiInfos', () => {
  ...
  it('should support defineMethod info object (completion/ref)', async () => {
    ...
    const infos = await ctx.getApiInfos();
    expect(infos.bar?.completion).toBeUndefined();
    expect(infos.bar?.ref?.url).toBe('https://example.com');
  });
  it('should return property infos with completion/ref/examples', async () => {
    ...
    const infos = await ctx.getApiInfos();
    expect(infos.token?.completion).toBeUndefined();
  ...
  it('should not return meta-only properties and should not trigger meta factory', async () => {
    const ctx = new FlowContext();
    const metaFactory = vi.fn(() => ({ type: 'string', title: 'MetaOnly' }));
  ...
  it('should not return keys starting with underscore', async () => {
    ...
    RunJSContextRegistry.register('test-private', '*', TestRunJSContext);
    const ctx = new FlowContext();
    ctx.defineProperty('_runtimeSecret', {
      info: { description: '_runtimeSecret' },
    });
    ctx.defineMethod('_runtimeMethod', () => 1, 'runtime hidden');
    const infos = await ctx.getApiInfos({ version: 'test-private' });
  ...
  it('should only return top-level keys and no completion', async () => {
    ...
    RunJSContextRegistry.register('test-layer', '*', TestRunJSContext);
    const ctx = new FlowContext();
    const infos = await ctx.getApiInfos({ version: 'test-layer' });
    expect(infos.api).toBeTruthy();
    expect(infos.api?.properties).toBeUndefined();
    expect(infos.api?.completion).toBeUndefined();
    expect(infos.request).toBeUndefined();
  });
  it('should exclude variable-like roots and expose libs docs under libs.*', async () => {
    ...
    RunJSContextRegistry.register('test-api-libs', '*', TestRunJSContext);
    const ctx = new FlowContext();
    const infos = await ctx.getApiInfos({ version: 'test-api-libs' });
    ...
    expect(infos.React).toBeUndefined();
    expect(infos.ReactDOM).toBeUndefined();
    expect(infos.antd).toBeUndefined();
...
describe('FlowContext.getEnvInfos', () => {
  ...
  it('should return currentViewBlocks for page view (BlockModel only)', async () => {
    ...
    BlockModelLike.define({ label: 'MyBlock' });
    NonBlockModel.define({ label: 'NotBlock' });
    ...
    engine.getModel('b1').resource = { dataSourceKey: 'main', collectionName: 'posts' };
    engine.getModel('b2').resource = {
      dataSourceKey: 'main',
    ...
    const blocks = envs.currentViewBlocks?.value || [];
...
describe('FlowEngine context', () => {
  ...
  it('engine.context.getVar should resolve variable by path', async () => {
    ...
    const v1 = await engine.context.getVar('ctx.foo.bar');
    expect(v1).toBe(1);
    await expect(engine.context.getVar('{{ ctx.foo.bar }}')).rejects.toThrow();
    await expect(engine.context.getVar('foo.bar')).rejects.toThrow();
  });
...
describe('ForkFlowModel context inheritance and isolation', () => {
  ...
  it('model.context.getAction/getActions should prefer model actions over global', async () => {
    ...
    TestModel.registerActions({ foo: modelFoo, baz: modelBaz });
    const model = engine.createModel({ use: 'TestModel' });
  ...
  it('should isolate fork.context property changes from subModel.context when subModel delegates to parent', () => {
    ...
    model.context.defineProperty('appName', { value: 'NocoBase2' });
    const sub = engine.getModel('sub1');
...
describe('FlowContext delayed meta loading', () => {
  ...
  it('should create lazy-loaded meta tree node with async meta function', async () => {
    ...
    const children = await userNode.children();
  ...
  it('should support nested async properties in delayed meta loading', async () => {
    ...
    // 第一层异步加载
    const level1Children = await rootNode.children();
...
describe('FlowContext resolveOnServer selective server resolution', () => {
  it('resolves ctx.date expressions on client context', async () => {
    const engine = new FlowEngine();
    const out = await engine.context.resolveJsonTemplate({
  ...
  it('does not call server by default (no resolveOnServer set)', async () => {
    ...
    const tpl = { id: '{{ ctx.view.record.id }}', type: '{{ ctx.view.type }}' };
    const out = await engine.context.resolveJsonTemplate(tpl);
  ...
  it('calls server for whole variable when resolveOnServer is true', async () => {
    ...
    const tpl = { uid: '{{ ctx.user.id }}' };
    await engine.context.resolveJsonTemplate(tpl);
    expect(api.request).toHaveBeenCalledTimes(1);
  });
  it('reads resolveOnServer from delegated context properties (e.g., engine.context -> model.context)', async () => {
    const engine = new FlowEngine();
    const api = {
      request: vi.fn(async (config) => ({ data: { ok: true } })),
    };
    engine.context.defineProperty('api', { value: api });
    ...
    const model = engine.createModel({ use: 'M' });
    const tpl = { uid: '{{ ctx.user.id }}' };
    await model.context.resolveJsonTemplate(tpl);
    expect(api.request).toHaveBeenCalledTimes(1);
  });
  ...
  it('still calls server when at least one var has contextParams (even if others require contextParams but none)', async () => {
    const engine = new FlowEngine();
    const api = { request: vi.fn(async () => ({ data: { ok: true } })) };
    engine.context.defineProperty('api', { value: api });
    ...
    const tpl = { a: '{{ ctx.foo.a }}', u: '{{ ctx.user.id }}' };
    await engine.context.resolveJsonTemplate(tpl);
    expect(api.request).toHaveBeenCalledTimes(1);
  });
});
...
describe('FlowContext meta sort and factory title', () => {
  it('should sort root nodes by meta.sort (including factory.sort)', async () => {
    ...
    const fFactory = async () => ({ type: 'object', title: 'F-resolved', properties: {} });
  ...
  it('should sort child nodes by meta.sort (sync properties)', () => {
    ...
    expect(Array.isArray(subTree)).toBe(true);
    const names = subTree.map((n) => n.name);
  ...
  it('should sort child nodes by meta.sort (async properties)', async () => {
    ...
    const nodes = typeof subTree === 'function' ? await subTree() : subTree;
...
describe('FlowContext getPropertyMetaTree with complex async/sync mixing scenarios', () => {
  ...
  it('should handle async meta factory with error in deep path resolution', async () => {
    ...
    const ensureArray = async (v) => (typeof v === 'function' ? await v() : v);
    const arr = await ensureArray(rootTree);
    const names = arr.map((n) => n.name).sort();
  ...
  it('aggregates multiple resolves into one batch request', async () => {
    ...
    const api = {
      request: vi.fn(async (config) => {
        ...
        const results = batch.map((item, idx) => ({ id: item.id, data: { ok: idx + 1 } }));
    ...
    const t1 = { a: '{{ ctx.foo.a }}' };
    const t2 = { b: '{{ ctx.foo.a }}' };
    // Fire two resolves within the same micro-batch window (concurrently)
    const [r1, r2] = await Promise.all([
      engine.context.resolveJsonTemplate(t1),
      engine.context.resolveJsonTemplate(t2),
    ]);
...
//# sourceMappingURL=flowContext.test.js.map
