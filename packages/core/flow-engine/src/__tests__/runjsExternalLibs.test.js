...
beforeEach(() => {
  globalThis.__nocobaseImportAsyncCache = undefined;
  globalThis.__nbRunjsRoots = undefined;
  runjsImportAsync.mockReset();
  if (typeof window !== 'undefined') {
    window.__esm_cdn_base_url__ = 'https://esm.sh';
    window.__esm_cdn_suffix__ = '';
  }
});
describe('RunJS external libs', () => {
  it('should override ctx.React/ReactDOM when importing external react', async () => {
    ...
    runjsImportAsync.mockImplementation(async (url) => {
  ...
  it('should not reuse roots across different ctx owners when rendererKey is the same', () => {
    ...
    renderer.createRoot.mockImplementationOnce(() => root1).mockImplementationOnce(() => root2);
    ctx1.defineProperty('ReactDOM', { value: renderer });
    ctx2.defineProperty('ReactDOM', { value: renderer });
    const container = document.createElement('div');
    ctx1.render({ step: 1 }, container);
    ctx2.render({ step: 2 }, container);
...
//# sourceMappingURL=runjsExternalLibs.test.js.map
