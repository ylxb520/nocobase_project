...
describe('FlowEngine dataSource dirty registry', () => {
  ...
  it('marks dirty on record write operations (single & multi)', async () => {
    ...
    single.runAction = vi.fn().mockResolvedValue({ data: {}, meta: {} });
    single.refresh = vi.fn().mockResolvedValue(undefined);
    await single.save({ title: 't' }, { refresh: false });
    ...
    multi.runAction = vi.fn().mockResolvedValue({ data: [], meta: {} });
    multi.refresh = vi.fn().mockResolvedValue(undefined);
    await multi.create({ name: 'n' }, { refresh: false });
    // exact association
...
//# sourceMappingURL=flowEngine.dataSourceDirty.test.js.map
