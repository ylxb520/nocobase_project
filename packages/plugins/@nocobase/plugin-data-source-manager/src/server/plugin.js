...
export class PluginDataSourceManagerServer extends Plugin {
  ...
  async beforeLoad() {
    ...
    this.app.db.on('dataSourcesCollections.afterSaveWithAssociations', async (model, { transaction }) => {
      await model.load({
        app: this.app,
        transaction,
      });
    ...
    this.app.db.on('dataSourcesRolesResourcesScopes.afterSaveWithAssociations', async (model, options) => {
      const { transaction } = options;
      const dataSourcesRolesResourcesActions = await this.app.db
  ...
  indexFieldForAI() {
    ...
    const getFieldDocument = (dataSource, collection, field) => ({
    ...
    this.app.dataSourceManager.beforeAddDataSource((dataSource) => {
      ...
      if (cm instanceof SequelizeCollectionManager) {
        const db = cm.db;
        db.on('afterDefineCollection', async (collection) => {
