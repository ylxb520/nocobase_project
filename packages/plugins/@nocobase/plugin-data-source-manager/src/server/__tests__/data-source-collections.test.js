 ...
 */
import { createMockServer, waitSecond } from '@nocobase/test';
...
describe('data source collection', () => {
  ...
  it('should update collection fields', async () => {
    ...
    collectionInJson.fields.forEach((field) => {
      if (field.name === 'name') {
        field.uiSchema = {
          title: 'Name',
          'x-component': 'Input',
    ...
    collectionInJson2.fields = collectionInJson2.fields.filter((field) => field.name !== 'age');
    await app.db.getRepository('dataSources.collections', 'mockInstance1').update({
      filterByTk: 'test',
...
//# sourceMappingURL=data-source-collections.test.js.map
