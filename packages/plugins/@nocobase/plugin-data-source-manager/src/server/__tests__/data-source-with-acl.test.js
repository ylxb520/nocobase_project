...
describe('data source with acl', () => {
  let app;
  const getDataSourceAgent = (agent, dataSourceKey) => {
    return agent.set('X-data-source', dataSourceKey);
  };
  beforeEach(async () => {
  ...
  it.skipIf(os.platform() === 'win32')('should call application middleware', async () => {
    ...
    const adminAgent = (await app.agent().login(adminUser)).set('x-data-source', 'mockInstance1');
  ...
  it('should set main data source strategy', async () => {
    ...
    const adminAgent = await app.agent().login(adminUser);
    const testUserAgent = await app.agent().login(testUser);
  ...
  it(`should update data sources`, async () => {
    ...
    const testUserAgent = await app.agent().login(testUser, 'testRole');
...
//# sourceMappingURL=data-source-with-acl.test.js.map
