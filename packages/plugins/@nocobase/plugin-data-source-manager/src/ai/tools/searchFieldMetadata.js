...
export default defineTools({
  ...
  invoke: async (ctx, args) => {
    ...
    try {
      const aiManager = ctx.app.aiManager;
      ...
      const results = (result || [])
        .map((path) => {
          ...
          const dataSource = ctx.app.dataSourceManager.get(ds);
          const collection = dataSource.collectionManager.getCollection(coll);
          const field = collection.getField(f);
...
//# sourceMappingURL=searchFieldMetadata.js.map
