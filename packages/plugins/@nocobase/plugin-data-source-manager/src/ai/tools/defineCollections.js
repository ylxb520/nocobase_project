  ...
  definition: {
    ...
    schema: z.object({
      intent: z.enum(['create', 'edit'])
        .describe(`Pass the intent of the current tool invocation as an enum value. The value must be either 'create' or 'edit':
    ...
    try {
      await ctx.db.sequelize.transaction(async (transaction) => {
        ...
        for (const rawOptions of sorted) {
          ...
          // Filter fields
          const relationFields = options.fields.filter((f) =>
            ['belongsTo', 'hasMany', 'belongsToMany', 'hasOne'].includes(f.type),
          );
          const normalFields = options.fields.filter(
            (f) => !['belongsTo', 'hasMany', 'belongsToMany', 'hasOne'].includes(f.type),
          );
          // System fields
          const systemFields = [];
...
//# sourceMappingURL=defineCollections.js.map
