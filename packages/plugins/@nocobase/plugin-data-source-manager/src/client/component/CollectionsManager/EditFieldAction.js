...
const getSchema = ({ schema, record, parentRecord, compile, getContainer }) => {
  if (!schema) {
    return;
  }
  const properties = cloneDeep(schema.properties);
  if (properties?.name) {
...
export const EditCollectionField = (props) => {
  ...
  return React.createElement(EditFieldAction, { item: record, parentItem: parentRecordData, ...props });
};
const EditFieldAction = (props) => {
  ...
  const isDialect = (dialect) => currentDatabase?.dialect === dialect;
      ...
      React.createElement(
        'a',
        {
          ...
          onClick: async () => {
            ...
            const defaultValues = cloneDeep(data?.data) || {};
...
//# sourceMappingURL=EditFieldAction.js.map
