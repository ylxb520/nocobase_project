...
const getSchema = (schema, record, compile, getContainer) => {
  ...
  const properties = cloneDeep(schema.configurableProperties) || {
    title: {
...
export const EditCollection = (props) => {
  const record = useRecord();
  return React.createElement(EditCollectionAction, { item: record, ...props });
};
const EditCollectionAction = (props) => {
  ...
  const filterTargetKeyOptions = record.fields?.map((item) => {
    return {
...
//# sourceMappingURL=EditCollectionAction.js.map
