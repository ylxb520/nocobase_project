...
export class BasicAuth extends BaseAuth {
  ...
  getSignupFormSettings() {
    ...
    if (
      !(
        signupForm.length &&
        signupForm.some((item) => ['username', 'email'].includes(item.field) && item.show && item.required)
      )
    ) {
  ...
  getEmailConfig() {
    ...
    return { ...options, enableResetPassword: this.authenticator.options?.public?.enableResetPassword };
  }
  async lostPassword() {
    const ctx = this.ctx;
    const {
