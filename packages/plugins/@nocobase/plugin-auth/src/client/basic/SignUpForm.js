...
const getSignupPageSchema = (fieldSchemas) => ({
  type: 'object',
  name: uid(),
  'x-component': 'FormV2',
  properties: {
...
export const SignUpForm = ({ authenticatorName: name }) => {
  ...
  if (!options?.allowSignUp) {
    return React.createElement(Navigate, { to: '/not-found', replace: true });
  }
  const schema = getSignupPageSchema(fieldSchemas);
  return React.createElement(SchemaComponent, { schema: schema, scope: { useBasicSignUp, t } });
};
