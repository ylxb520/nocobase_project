...
const SignupFormSettings = () => {
  ...
  const value = useMemo(() => {
    ...
    const newValue = fieldValue.filter((item) => fields.find((field) => field.name === item.field));
    for (const field of fields) {
      const exist = newValue.find((item) => item.field === field.name);
      if (!exist) {
        newValue.push({
          field: field.name,
...
export const Options = () => {
  ...
  const noChannelsFoundMessage = React.createElement(
    'span',
    null,
    t('No notification channels found. Please '),
    React.createElement(Link, { to: '/admin/settings/notification-manager/channels' }, t('add one first')),
    '.',
  );
  return React.createElement(SchemaComponent, {
    scope: { t },
                ...
                divider1: {
                  ...
                  'x-component': () => {
                    return React.createElement(
                      Divider,
                      { orientation: 'left', orientationMargin: '0' },
