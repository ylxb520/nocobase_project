...
import { NodeDefaultView, Branch, useFlowContext, useStyles, WorkflowVariableInput, WorkflowVariableTextArea, defaultFieldNames, nodesOptions, scopeOptions, triggerOptions, Instruction, RadioWithTooltip, renderEngineReference, CalculationConfig, useWorkflowVariableOptions, useNodeContext, } from '@nocobase/plugin-workflow/client';
...
function LoopCondition({ value, onChange }) {
    ...
    const onCheckpointChange = useCallback((ev) => {
        onChange({ ...value, checkpoint: ev.target.value });
    }, [value, onChange]);
    const onContinueOnFalseChange = useCallback((ev) => {
        onChange({ ...value, continueOnFalse: ev.target.value });
    }, [value, onChange]);
    const onCalculationChange = useCallback((calculation) => {
        onChange({ ...value, calculation });
    }, [value, onChange]);
    return (React.createElement(React.Fragment, null,
        React.createElement(Checkbox, { checked: Boolean(value), onChange: (ev) => {
                onChange(ev.target.checked
                    ? { checkpoint: 0, continueOnFalse: false, calculation: { group: { type: 'and', calculations: [] } } }
                    : false);
            } }, t('Enable loop condition', { ns: NAMESPACE })),
        value ? (React.createElement(Card, null,
            React.createElement(FormLayout, { layout: "vertical" },
                React.createElement(FormItem, { label: t('Condition', { ns: NAMESPACE }) },
                    React.createElement(CalculationConfig, { value: value.calculation, onChange: onCalculationChange, useVariableHook: useVariableHook })),
                React.createElement(FormItem, { label: t('When to check', { ns: NAMESPACE }) },
                    React.createElement(RadioWithTooltip, { value: value.checkpoint, onChange: onCheckpointChange, options: [
                            {
                ...
                React.createElement(FormItem, { label: t('When condition is not met on item', { ns: NAMESPACE }) },
                    React.createElement(RadioWithTooltip, { value: value.continueOnFalse, onChange: onContinueOnFalseChange, options: [
                            {
...
function useScopeVariables(node, options) {
    ...
    if (typeof target === 'string' && target.startsWith('{{') && target.endsWith('}}')) {
        const paths = target
            ...
            .split('.')
            .map((path) => path.trim());
        const targetOptions = scope ??
            [scopeOptions, nodesOptions, triggerOptions].map((item) => {
...
function useVariableHook(options = {}) {
...
function LoopVariableTextArea({ variableOptions, ...props }) {
    const scope = useVariableHook(variableOptions);
    return React.createElement(Variable.TextArea, { scope: scope, ...props });
}
export default class extends Instruction {
    ...
    icon = (React.createElement(RollbackOutlined, { style: {} }));
    fieldset = {
        target: {
            type: 'string',
    ...
    Component({ data }) {
        ...
        return (React.createElement(NodeDefaultView, { data: data },
            React.createElement("div", { className: styles.nodeSubtreeClass },
                ...
                padding-left: 4em;
              `) },
                    React.createElement(Branch, { from: data, entry: entry, branchIndex: entry?.branchIndex ?? 0 }),
                    React.createElement("div", { className: styles.branchClass, style: { minWidth: '4em' } },
                        React.createElement("div", { className: "workflow-branch-lines" }),
                        React.createElement("div", { className: styles.loopLineClass },
                            React.createElement(ArrowUpOutlined, null)))))));
    }
    useScopeVariables = useScopeVariables;
}
//# sourceMappingURL=LoopInstruction.js.map