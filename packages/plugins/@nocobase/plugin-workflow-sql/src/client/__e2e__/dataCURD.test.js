...
import { CollectionTriggerNode, apiCreateWorkflow, apiDeleteWorkflow, apiGetWorkflow, apiGetWorkflowNodeExecutions, apiUpdateWorkflowTrigger, appendJsonCollectionName, generalWithNoRelationalFields, SQLNode, apiGetRecord, } from '@nocobase/plugin-workflow-test/e2e';
...
test.describe('select data', () => {
    test('No variable SQL, select 1 record', async ({ page, mockCollections, mockRecords }) => {
        ...
        await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), SQLNodeAppendText)
            .collections);
...
test.describe('insert data', () => {
    test('No variable SQL, insert values,insert 1 record', async ({ page, mockCollections, mockRecords }) => {
        ...
        const sqlNodeId = await sqlNode.node.locator('.workflow-node-id').innerText();
        await sqlNode.nodeConfigure.click();
        await sqlNode.sqlEditBox.fill(`insert into ${SQLNodeCollectionName}(orgname,status_singleselect,staffnum) values('公司名称(单行文本)1','1',10) returning *`);
        await sqlNode.submitButton.click();
        // 2、测试步骤：添加数据触发工作流
        const triggerNodeCollectionRecordOne = triggerNodeFieldDisplayName + dayjs().format('YYYYMMDDHHmmss.SSS').toString();
        const triggerNodeCollectionRecords = await mockRecords(triggerNodeCollectionName, [
    ...
    test('No variable SQL, insert values, insert Multiple Records', async ({ page, mockCollections, mockRecords }) => {
        ...
        const sqlNodeId = await sqlNode.node.locator('.workflow-node-id').innerText();
        await sqlNode.nodeConfigure.click();
        await sqlNode.sqlEditBox.fill(`insert into ${SQLNodeCollectionName}(orgname,status_singleselect,staffnum) values('公司名称(单行文本)1','1',10),('公司名称(单行文本)2','1',20),('公司名称(单行文本)3','1',30) returning *`);
        await sqlNode.submitButton.click();
        // 2、测试步骤：添加数据触发工作流
        const triggerNodeCollectionRecordOne = triggerNodeFieldDisplayName + dayjs().format('YYYYMMDDHHmmss.SSS').toString();
        const triggerNodeCollectionRecords = await mockRecords(triggerNodeCollectionName, [
    ...
    test('The variable is the values value, insert 1 record', async ({ page, mockCollections, mockRecords }) => {
        ...
        const sqlNodeId = await sqlNode.node.locator('.workflow-node-id').innerText();
        await sqlNode.nodeConfigure.click();
        await sqlNode.sqlEditBox.fill(`insert into ${SQLNodeCollectionName}(orgname,status_singleselect,staffnum) values('`);
...
//# sourceMappingURL=dataCURD.test.js.map