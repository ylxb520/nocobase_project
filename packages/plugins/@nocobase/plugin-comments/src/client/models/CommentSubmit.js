...
export const CommentSubmit = observer((props) => {
  ...
  const setContent = useCallback((v) => {
    setValue({ content: v });
  }, []);
  ...
  return wrapSSR(
    React.createElement(
      'div',
      { style: { marginTop: 10 }, className: `${componentCls} ${hashId}` },
      markdown.edit({
      ...
      React.createElement(
        Button,
        { disabled: !canSubmit, onClick: () => submit(), type: 'primary', style: { marginTop: 10 } },
        t('Comment', { ns: NAMESPACE }),
      ),
    ),
  );
});
//# sourceMappingURL=CommentSubmit.js.map
