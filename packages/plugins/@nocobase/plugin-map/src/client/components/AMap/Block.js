...
import { RecordProvider, css, getLabelFormatValue, useCollection, useCollectionManager_deprecated, useCollectionParentRecordData, useCollection_deprecated, useCompile, useFilterAPI, usePopupUtils, useProps, } from '@nocobase/client';
...
export const AMapBlock = (props) => {
    ...
    const { collectionField, fieldNames, dataSource, fixedBlock, zoom, setSelectedRecordKeys, lineSort } = useProps(props);
    ...
    const mapRef = useRef();
    const geometryUtils = mapRef.current?.aMap?.GeometryUtil;
    ...
    const onSelectingComplete = useMemoizedFn(() => {
        ...
        const selectedOverlays = overlays?.filter((o) => {
            if (o === selectingOverlay || o.getExtData().id === undefined)
                return;
            if ('getPosition' in o) {
    ...
    useEffect(() => {
        if (!collectionField || !mapRef.current || !dataSource)
            return;
        const fieldPaths = Array.isArray(fieldNames?.field) && fieldNames?.field.length > 1
        ...
        if (collectionField.type === 'point' && lineSort?.length && overlays?.length > 1) {
            const positions = overlays.map((o) => o.getPosition());
            overlays[0].setzIndex(13);
            overlays[overlays.length - 1].setzIndex(13);
    ...
    return (React.createElement("div", { className: css `
      ...
      ` },
        React.createElement("div", { className: css `
          ...
          top: 10px;
          z-index: 999;
        ` }, isMapInitialization && !mapRef.current?.errMessage ? (React.createElement(Space, { direction: "vertical" },
            React.createElement(Button, { style: {
                    ...
                    e.stopPropagation();
                    setSelecting('');
                }, icon: React.createElement(EnvironmentOutlined, null) }),
            React.createElement(Button, { style: {
            ...
            selectingMode === 'selection' ? (React.createElement(Button, { type: "primary", icon: React.createElement(CheckOutlined, null), title: t('Confirm selection'), onClick: onSelectingComplete })) : null)) : null),
        React.createElement(RecordProvider, { record: record, parent: parentRecordData },
            React.createElement(MapBlockDrawer, null)),
        React.createElement(AMapComponent, { ...collectionField?.uiSchema?.['x-component-props'], ref: mapRefCallback, style: { height: fixedBlock ? '100%' : null }, zoom: zoom, disabled: true, block: true, overlayCommonOptions: {
                strokeColor: '#F18b62',
                fillColor: '#F18b62',
            } })));
};
