 ...
 */
import { DndProvider, MultiRecordResource, FlowModelRenderer, tExpr, DragHandler, AddSubModelButton, FlowSettingsButton, Droppable, } from '@nocobase/flow-engine';
...
const MapFieldCascader = observer((props) => {
    const field = useField();
    const options = props?.options ?? props?.dataSource ?? field?.dataSource ?? [];
    return React.createElement(Cascader, { ...props, options: options });
});
export class MapBlockModel extends CollectionBlockModel {
    ...
    renderConfigureAction() {
        return (React.createElement(AddSubModelButton, { key: 'map-add-actions', model: this, subModelBaseClass: this.getModelClassName('MapActionGroupModel'), subModelKey: "actions" },
    ...
    renderComponent() {
        ...
        return React.createElement(MapBlockContent, { model: this, heightMode: heightMode, height: height });
    }
}
...
const useMapHeight = ({ heightMode, containerRef, actionsRef, deps = [], }) => {
...
const MapBlockContent = observer(({ model, heightMode, height }) => {
    ...
    return (React.createElement("div", { ref: containerRef, style: containerStyle },
        React.createElement("div", { ref: actionsRef },
                    ...
                    React.createElement(Space, null,
                        model.mapSubModels('actions', (action) => {
                            ...
                            if (action.props.position === 'left') {
                                return (React.createElement(FlowModelRenderer, { key: action.uid, model: action, showFlowSettings: { showBackground: false, showBorder: false, toolbarPosition: 'above' } }));
                            }
                            return null;
                        }),
        ...
        React.createElement("div", { className: "nb-map-content", style: mapStyle },
            React.createElement(MapBlockComponent, { ...model.props, fields: model.collection.getFields(), name: model.collection.name, primaryKey: model.collection.filterTargetKey, setSelectedRecordKeys: model.setSelectedRecordKeys.bind(model), dataSource: model.resource.getData(), height: mapHeight ? mapHeight - 5 : null }))));
});
const getCollectionFieldsOptions = (collectionName, collectionManager, type, interfaces, opts) => {
    const { association = false, cached = {}, collectionNames = [collectionName], maxDepth = 1, allowAllTypes = false, exceptInterfaces = [], prefixFieldValue = '', usePrefix = false, dataSource: customDataSourceNameValue, } = opts || {};
    ...
    steps: {
        init: {
            ...
            uiSchema: (ctx) => {
                ...
                const getMapFieldOptions = () => getCollectionFieldsOptions(ctx.collection.name, collectionManager, ['point', 'lineString', 'polygon'], null, {
                    association: ['o2o', 'obo', 'oho', 'o2m', 'm2o', 'm2m'],
                }) || [];
                const getMarkerFieldOptions = () => getCollectionFieldsOptions(ctx.collection.name, collectionManager, 'string', null, {
...
//# sourceMappingURL=MapBlockModel.js.map