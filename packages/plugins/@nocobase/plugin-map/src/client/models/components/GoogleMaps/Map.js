...
export const GoogleMapsCom = React.forwardRef((props, ref) => {
    const { value, onChange, block = false, readonly, disabled = block, zoom = 13, overlayCommonOptions, height, type, } = props;
    ...
    if (!accessKey || errMessage) {
        return (React.createElement(Alert, { action: React.createElement(Button, { type: "primary", onClick: () => {
                ...
                } }, t('Go to the configuration page')), message: errMessage || t('Please configure the Api key first'), type: "error" }));
    }
