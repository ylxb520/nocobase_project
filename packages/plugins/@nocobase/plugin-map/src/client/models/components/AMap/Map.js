...
export const AMapCom = React.forwardRef((props, ref) => {
    const { accessKey, securityJsCode } = useMapConfig(props.mapType) || {};
    const { value, onChange, block = false, readonly, disabled = block, zoom, overlayCommonOptions, height, type, } = props;
    ...
    if (!accessKey || errMessage) {
        return (React.createElement(Alert, { action: React.createElement(Button, { type: "primary", onClick: () => {
                    ctx.view.close();
                    navigate(app.pluginSettingsManager?.getRoutePath('map') || '/admin/settings/map');
                } }, t('Go to the configuration page')), message: errMessage || t('Please configure the AccessKey and SecurityJsCode first'), type: "error" }));
    }
