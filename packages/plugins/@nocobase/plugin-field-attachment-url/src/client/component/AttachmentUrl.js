...
const InnerAttachmentUrl = (props) => {
    ...
    const useTableSelectorProps = () => {
        const { multiple, options, setSelectedRows, selectedRows: rcSelectRows = [], onChange, } = useContext(RecordPickerContext);
    ...
    return (React.createElement("div", { style: { width: '100%', overflow: 'auto' } },
        React.createElement(AssociationField.FileSelector, { toValueItem: toValueItem, value: options, quickUpload: fieldSchema['x-component-props']?.quickUpload !== false, selectFile: collectionField?.target && collectionField?.target !== 'attachments'
                ? fieldSchema['x-component-props']?.selectFile !== false
                : false, action: `${collectionField?.target || 'attachments'}:create`, onSelect: handleSelect, onChange: onChange, disabled: disabled }),
        React.createElement(ActionContextProvider, { value: {
            ...
            } }, collectionField?.target && collectionField?.target !== 'attachments' && (React.createElement(RecordPickerProvider, { ...pickerProps },
            React.createElement(CollectionProvider_deprecated, { name: collectionField?.target, dataSource: 'main' },
                React.createElement(FormProvider, null,
                    React.createElement(TableSelectorParamsProvider, { params: {} },
                        React.createElement(SchemaComponentOptions, { scope: { usePickActionProps, useTableSelectorProps } },
                            React.createElement(RecursionField, { onlyRenderProperties: true, basePath: field.address, schema: fieldSchema, filterProperties: (s) => {
...
const FileManageReadPretty = connect((props) => {
    ...
    if (componentMode === 'url') {
        return React.createElement(EllipsisWithTooltip, { ellipsis: true }, value);
    }
    return (React.createElement(EllipsisWithTooltip, { ellipsis: true }, collectionField ? React.createElement(Upload.ReadPretty, { ...props }) : null));
});
export const AttachmentUrl = connect(InnerAttachmentUrl, mapReadPretty(FileManageReadPretty));
//# sourceMappingURL=AttachmentUrl.js.map