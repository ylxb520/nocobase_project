...
const datetimeProperties = {
    ...
    'uiSchema.x-component-props.showTime': {
        ...
        'x-reactions': [
            ...dateTimeProps['uiSchema.x-component-props.showTime']['x-reactions'],
            ...datetimeReactions,
        ],
    },
...
const resolveFormulaDataType = (meta) => {
...
const formulaFilterOperators = formulaOperatorGroups.flatMap(({ types, operators: operatorList }) => operatorList.map((operator) => ({
    ...operator,
    visible: (meta) => {
...
export class FormulaFieldInterface extends CollectionFieldInterface {
    ...
    properties = {
        ...
        engine: {
            ...
            enum: Array.from(evaluators.getEntities()).reduce((result, [value, options]) => result.concat({ value, ...options }), []),
            required: true,
            default: 'formula.js',
        },
        expression: {
