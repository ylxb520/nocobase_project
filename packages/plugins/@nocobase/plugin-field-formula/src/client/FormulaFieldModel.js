...
const resolveFormulaUsageFlags = (form, ctx) => {
...
export function FormulaResult(props) {
    ...
    if (isFilterContext || isDefaultValueDialog) {
        ...
        return React.createElement(EditableComp, { ...others, value: value, onChange: (...args) => others?.onChange?.(...args) });
    }
    const Component = TypedComponents[dataType] ?? InputString;
    if (!collectionField) {
        return;
    }
    return (React.createElement(Component, { ...others, value: dataType === 'double' ? toFixedByStep(editingValue, props.step) : editingValue }));
}
export class FormulaFieldModel extends FieldModel {
    render() {
        return (React.createElement(FormulaResult, { ...this.props, collectionField: this.context.collectionField, form: this.context.form, context: this.context }));
    }
}
...
//# sourceMappingURL=FormulaFieldModel.js.map