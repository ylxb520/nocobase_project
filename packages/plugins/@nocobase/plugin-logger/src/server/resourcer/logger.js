...
const getLastestLogs = async (path) => {
    ...
    const getLatestLog = (prefix) => {
        const logs = logsWithTime.filter((file) => file[0].startsWith(prefix));
        if (!logs.length) {
            return null;
        }
        return logs.reduce((a, b) => (a[1] > b[1] ? a : b))[0];
    };
...
export default {
    ...
    actions: {
        ...
        collect: async (ctx, next) => {
            ...
            if (!files.length) {
                ctx.throw(404, ctx.t('No log files found. Please check the LOGGER_TRANSPORTS environment variable configuration.'));
            }
            try {
                ctx.attachment('logs.tar.gz');
