...
export const createForeignKey = (db) => {
    return async (model, { transaction }) => {
        ...
        if (name === foreignKey) {
            throw new Error(`Naming collision between attribute '${foreignKey}' and association '${name}' on model ${collectionName}. To remedy this, change either foreignKey or as in your association definition`);
        }
