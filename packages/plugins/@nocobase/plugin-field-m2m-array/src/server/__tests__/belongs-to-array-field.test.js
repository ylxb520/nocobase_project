    ...
    describe('association keys check', async () => {
        ...
        it('element type of foreign field must be match the type of target field', async () => {
            ...
            await expect(db.sequelize.transaction(async (transaction) => {
                ...
                await field.load({ transaction });
            })).rejects.toThrow(/The element type "STRING" of foreign key "tag_ids" does not match the type "BIGINT" of target key "id" in collection "tags"/);
            await expect(db.sequelize.transaction(async (transaction) => {
                const field = await fieldRepo.create({
                    values: {
                        interface: 'mbm',
...
//# sourceMappingURL=belongs-to-array-field.test.js.map