 ...
 */
import { BelongsToArrayAssociation, RelationField } from '@nocobase/database';
...
export class BelongsToArrayField extends RelationField {
    ...
    checkAssociationKeys() {
        ...
        if (!['ARRAY', 'JSONTYPE', 'JSON', 'JSONB'].includes(foreignType)) {
            throw new Error(`The type of foreign key "${foreignKey}" in collection "${this.collection.name}" must be ARRAY, JSON or JSONB`);
        }
        ...
        if (foreignType === 'ARRAY' && elementType !== targetType) {
            throw new Error(`The element type "${elementType}" of foreign key "${foreignKey}" does not match the type "${targetType}" of target key "${targetKey}" in collection "${target}"`);
        }
    }
    bind() {
...
//# sourceMappingURL=belongs-to-array-field.js.map