...
import { FormEventTriggerNode, apiCreateWorkflow, apiDeleteWorkflow, apiGetDataSourceCount, apiGetWorkflow, appendJsonCollectionName, generalWithNoRelationalFields, } from '@nocobase/plugin-workflow-test/e2e';
...
test.describe('Configuration page to configure the Trigger node', () => {
    ...
    // TODO: 需要转移到新事件中
    test.skip('Form Submit to Workflow Button Add Data Trigger', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        const triggerNodeAppendText = faker.string.alphanumeric(5);
        //创建触发器节点数据表
        const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
...
//# sourceMappingURL=configuration1.test.js.map