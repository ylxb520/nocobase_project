...
const flatTreeData = (data = [], fieldNames) => {
  const nodeChildren = fieldNames?.children || 'children';
...
const DefaultFilterComponent = ({ value, onChange }) => {
...
export const Tree = withDynamicSchemaProps(
  (props) => {
    ...
    const dataList = useMemo(() => (searchable || defaultExpandAll ? flatTreeData(clonedTreeData, fieldNames) : null), [
      clonedTreeData,
    ]);
    const allKeys = useMemo(() => dataList?.map((item) => item[fieldKeyName]), [dataList, fieldKeyName]);
    const [expandedKeys, setExpandedKeys] = useState(propsExpandedKeys);
    ...
    return React.createElement(
      ...
      searchable &&
        React.createElement(
          ...
          React.createElement(FilterComponent, { value: searchValue, onChange: onSearch.run }),
        ),
      !loading
        ? searchedTreeData.length
          ? React.createElement(AntdTree, {
              ...treeProps,
              onExpand: onExpand,
        ...
        : React.createElement(
            'div',
            { style: { height: 200, marginTop: token.margin, textAlign: 'center' } },
