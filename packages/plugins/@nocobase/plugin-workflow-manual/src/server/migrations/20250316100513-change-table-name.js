    ...
    async up() {
        ...
        if (!exists) {
            ...
            if (newPrimaryKeys.length > 1) {
                // @ts-ignore
                const constraints = await queryInterface.showConstraint(newTableName);
        ...
        await db.sequelize.transaction({
            ...
            if (this.db.isPostgresCompatibleDialect()) {
                await db.sequelize.query(`ALTER TABLE ${oldTableNameWithQuotes} RENAME TO "${db.options.tablePrefix || ''}${db.options.underscored ? 'workflow_manual_tasks' : 'workflowManualTasks'}";`, {
                    transaction,
                });
            }
            else {
            ...
            for (const item of indexes) {
                if (item.name.startsWith(oldIndexPrefix)) {
                    if (this.db.isPostgresCompatibleDialect()) {
                        await db.sequelize.query(`ALTER INDEX ${db.options.schema ? `"${db.options.schema}".` : ''}"${item.name}" RENAME TO "${item.name.replace(oldIndexPrefix, newIndexPrefix)}";`, { transaction });
                    }
                    else if (this.db.isMySQLCompatibleDialect()) {
                        await db.sequelize.query(`ALTER TABLE ${newTableNameWithQuotes} RENAME INDEX ${item.name} TO ${item.name.replace(oldIndexPrefix, newIndexPrefix)};`, { transaction });
                    }
                }
            }
        });
...
//# sourceMappingURL=20250316100513-change-table-name.js.map