...
test.describe('field data entry', () => {
    test('Collection event to add a data trigger, entering mailbox data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        ...
        const hasEmail = Object.values(manualNodeJobResult).some((value) => value.email === manualNodeRecord);
        expect(hasEmail).toBe(true);
        // 4、后置处理：删除工作流
        await apiDeleteWorkflow(workflowId);
    });
    test('Collection event to add a data trigger, entering integer data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        const triggerNodeAppendText = 'a' + faker.string.alphanumeric(4);
        const manualNodeAppendText = 'b' + dayjs().format('HHmmss').toString();
        // 创建触发器节点数据表
        ...
        expect(manualNodeJobStatus).toBe(1);
        const manualNodeJobResult = manualNodeJob.result;
        const hasStaffnum = Object.values(manualNodeJobResult).some((value) => value.staffnum == manualNodeRecord);
        expect(hasStaffnum).toBe(true);
        // 4、后置处理：删除工作流
        await apiDeleteWorkflow(workflowId);
    });
    test('Collection event to add a data trigger, entering number data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        const triggerNodeAppendText = 'a' + faker.string.alphanumeric(4);
        const manualNodeAppendText = 'b' + dayjs().format('HHmmss').toString();
        // 创建触发器节点数据表
        ...
        expect(manualNodeJobStatus).toBe(1);
        const manualNodeJobResult = manualNodeJob.result;
        const hasRegcapital = Object.values(manualNodeJobResult).some((value) => value.regcapital == manualNodeRecord);
        expect(hasRegcapital).toBe(true);
        // 4、后置处理：删除工作流
        await apiDeleteWorkflow(workflowId);
    });
});
//# sourceMappingURL=customFormBlocks3.test.js.map