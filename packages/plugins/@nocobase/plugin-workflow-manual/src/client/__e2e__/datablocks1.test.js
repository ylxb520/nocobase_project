...
import { ManualNode, apiCreateWorkflow, apiCreateWorkflowNode, apiDeleteWorkflow, apiFilterList, apiGetList, apiGetWorkflow, apiGetWorkflowNode, apiUpdateWorkflowTrigger, appendJsonCollectionName, generalWithNoRelationalFields, } from '@nocobase/plugin-workflow-test/e2e';
import { expect, test } from '@nocobase/test/e2e';
import { dayjs } from '@nocobase/utils';
test.describe('field data', () => {
    test('Collection event to add a data trigger, get single line text data for manual node custom form', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        ...
        await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), preManualNodeAppendText)
            .collections);
        // 创建Manual节点数据表
    ...
    test('Collection event to add a data trigger, get single line text data for manual node create record form', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        const triggerNodeAppendText = 'a' + faker.string.alphanumeric(4);
        const manualNodeAppendText = 'b' + dayjs().format('HHmmss').toString();
        const preManualNodeAppendText = 'c' + dayjs().format('HHmmss').toString();
    ...
    test('Collection event to add a data trigger, get single line text data for manual node update record form', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        const triggerNodeAppendText = 'a' + faker.string.alphanumeric(4);
        const manualNodeAppendText = 'b' + dayjs().format('HHmmss').toString();
        const preManualNodeAppendText = 'c' + dayjs().format('HHmmss').toString();
...
//# sourceMappingURL=datablocks1.test.js.map