...
import { AggregateNode, CreateRecordNode, ManualNode, apiCreateWorkflow, apiCreateWorkflowNode, apiDeleteWorkflow, apiGetWorkflow, apiGetWorkflowNode, apiUpdateWorkflowTrigger, appendJsonCollectionName, generalWithNoRelationalFields, } from '@nocobase/plugin-workflow-test/e2e';
import { expect, test } from '@nocobase/test/e2e';
import { dayjs } from '@nocobase/utils';
test.describe('relation field data', () => { });
test.describe('field data', () => {
    test('Collection event to add a data trigger, get create record node data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
    ...
    test('Collection event to add a data trigger, get aggregate node data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        //数据表后缀标识
        const triggerNodeAppendText = 'a' + faker.string.alphanumeric(4);
        const manualNodeAppendText = 'b' + dayjs().format('HHmmss').toString();
        ...
        await expect(page
            ...
            .getByText('8')).toBeAttached();
        // 4、后置处理：删除工作流
        await apiDeleteWorkflow(workflowId);
    });
});
//# sourceMappingURL=datablocks.test.js.map