...
test.describe('field data entry', () => {
    test('Collection event to add a data trigger, entering mailbox data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        ...
        const hasPhone = Object.values(manualNodeJobResult).some((value) => value.email === manualNodeRecord);
    ...
    test('Collection event to add a data trigger, entering integer data', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        ...
        expect(manualNodeJobStatus).toBe(1);
        const manualNodeJobResult = manualNodeJob.result;
        const hasPhone = Object.values(manualNodeJobResult).some((value) => value.staffnum == manualNodeRecord);
        expect(hasPhone).toBe(true);
        const getRecords = await apiGetRecord(manualNodeCollectionName, 1);
        const getRecordsObj = JSON.parse(JSON.stringify(getRecords));
...
//# sourceMappingURL=createRecordForm5.test.js.map