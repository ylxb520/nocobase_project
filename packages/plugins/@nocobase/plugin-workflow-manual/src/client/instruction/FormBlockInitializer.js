...
import { CollectionProvider_deprecated, SchemaInitializerItem, useRecordCollectionDataSourceItems, useSchemaInitializer, useSchemaInitializerItem, useSchemaTemplateManager, } from '@nocobase/client';
...
function InternalFormBlockInitializer({ schema, ...others }) {
    ...
    async function onConfirm({ item }) {
        ...
        const actionKey = Object.entries(result.properties[formKey].properties).find(([key, f]) => f['x-component'] === 'ActionBar')?.[0] ||
            'actions';
    ...
    return React.createElement(SchemaInitializerItem, { ...others, onClick: onConfirm, items: items });
}
export function FormBlockInitializer() {
    const itemConfig = useSchemaInitializerItem();
    return (React.createElement(CollectionProvider_deprecated, { dataSource: itemConfig.schema?.dataSource, collection: itemConfig.schema?.collection },
        React.createElement(InternalFormBlockInitializer, { ...itemConfig })));
}
//# sourceMappingURL=FormBlockInitializer.js.map