...
import { FormProvider, Schema, useFieldSchema, useForm } from '@formily/react';
import { Alert, Button, Modal, Space, message } from 'antd';
import React, { useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { Action, ActionContextProvider, CompatibleSchemaInitializer, DefaultValueProvider, FormActiveFieldsProvider, GeneralSchemaDesigner, InitializerWithSwitch, SchemaComponent, SchemaComponentContext, SchemaInitializerItem, SchemaSettingsDivider, SchemaSettingsItem, SchemaSettingsRemove, VariableScopeProvider, gridRowColWrap, useDataSourceManager, useFormActiveFields, useFormBlockContext, usePlugin, useSchemaInitializer, useSchemaInitializerItem, useSchemaOptionsContext, } from '@nocobase/client';
import WorkflowPlugin, { DetailsBlockProvider, JOB_STATUS, SimpleDesigner, useAvailableUpstreams, useFlowContext, useNodeContext, useTrigger, useWorkflowExecuted, useWorkflowVariableOptions, } from '@nocobase/plugin-workflow/client';
    ...
    items: [
        ...
        },
        {
            ...
            useChildren() {
                ...
                return Array.from(manualFormTypes.getValues()).map((item) => {
...
function AssignedFieldValues() {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(SchemaSettingsItem, { title: title, onClick: () => setOpen(true) }, title),
        React.createElement(Modal, { width: '50%', title: title, open: open, onCancel: onCancel, footer: React.createElement(Space, null,
                React.createElement(Button, { onClick: onCancel }, t('Cancel')),
                React.createElement(Button, { type: "primary", onClick: onSubmit }, t('Submit'))) },
            React.createElement(DefaultValueProvider, { isAllowToSetDefaultValue: () => false },
                React.createElement(VariableScopeProvider, { scope: scope },
                    React.createElement(FormActiveFieldsProvider, { name: "form", getActiveFieldsName: upLevelActiveFields?.getActiveFieldsName },
                        React.createElement(FormProvider, { form: form },
                            React.createElement(FormLayout, { layout: 'vertical' },
                                React.createElement(Alert, { message: t('Values preset in this form will override user submitted ones when continue or reject.') }),
                                React.createElement("br", null),
                                open && schema && (React.createElement(SchemaComponentContext.Provider, { value: {
                                        ...ctx,
                                        refresh() {
                                            setInitialSchema(lodash.get(schema.toJSON(), 'properties.grid'));
                                        },
                                    } },
                                    React.createElement(SchemaComponent, { schema: schema, components: components })))))))))));
}
function ManualActionDesigner(props) {
    return (React.createElement(GeneralSchemaDesigner, { ...props, disableInitializer: true },
        React.createElement(Action.Designer.ButtonEditor, null),
        React.createElement(AssignedFieldValues, null),
...
function ContinueInitializer() {
    ...
    return (React.createElement(SchemaInitializerItem, { ...others, onClick: () => {
            insert({
                type: 'void',
                title: others.title,
...
function ActionInitializer() {
    const itemConfig = useSchemaInitializerItem();
    const { action, actionProps, ...others } = itemConfig;
    return (React.createElement(InitializerWithSwitch, { ...others, item: itemConfig, schema: {
            type: 'void',
            title: others.title,
            'x-decorator': 'ManualActionStatusProvider',
            'x-decorator-props': {
...
export function SchemaConfig({ value, onChange }) {
    ...
    const refresh = useCallback(function refresh() {
        // ctx.refresh?.();
        const { tabs } = lodash.get(schema.toJSON(), 'properties.drawer.properties');
    ...
    return (React.createElement(SchemaComponentContext.Provider, { value: {
            ...ctx,
            designable: !executed,
            refresh,
        } },
        React.createElement(SchemaComponent, { schema: schema, components: {
                ...nodeComponents,
                // @ts-ignore
...
export function SchemaConfigButton(props) {
    ...
    const { t } = usePluginTranslation();
    const onSetVisible = useCallback((v) => {
        if (!v) {
    ...
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement(ActionContextProvider, { value: { visible, setVisible: onSetVisible, formValueChanged: false } }, props.children)));
}
