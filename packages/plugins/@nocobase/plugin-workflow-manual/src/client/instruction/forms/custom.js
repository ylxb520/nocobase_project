...
import { ActionContextProvider, CollectionProvider_deprecated, CompatibleSchemaInitializer, FormBlockContext, RecordProvider, SchemaComponent, SchemaInitializerItem, SchemaInitializerItems, gridRowColWrap, useCollectionManager_deprecated, useCollectionRecordData, useCollection_deprecated, useSchemaInitializer, useSchemaInitializerItem, } from '@nocobase/client';
...
function CustomFormBlockProvider(props) {
    ...
    return !userJob?.status || values ? (React.createElement(CollectionProvider_deprecated, { collection: {
            ...props.collection,
            fields,
        } },
        React.createElement(RecordProvider, { record: values, parent: null },
            React.createElement(FormBlockContext.Provider, { value: {
                    form,
                    field,
                    setCollectionFields,
                } }, props.children)))) : null;
...
function CustomFormBlockInitializer() {
    ...
    return (React.createElement(SchemaInitializerItem, { ...itemConfig, onClick: () => {
            insert({
                type: 'void',
...
const AddCustomFormFieldButtonContext = React.createContext({});
function useAction() {
    const { values, query, reset } = useForm();
    const messages = [useLang('Field name existed in form')];
    const { collection, interfaceOptions, setCollectionFields, insert, setCallback, setInterface } = useContext(AddCustomFormFieldButtonContext);
    return {
        async run() {
            ...
            const newField = merge(options, values);
...
const CustomItemsComponent = (props) => {
    const [interfaceOptions, setInterface] = useState(null);
    const [insert, setCallback] = useState();
    const items = useCommonInterfaceInitializers();
    const collection = useCollection_deprecated();
    const { setCollectionFields } = useContext(FormBlockContext);
    const form = useMemo(() => createForm(), []);
    return (React.createElement(AddCustomFormFieldButtonContext.Provider, { value: {
            collection,
            insert,
            interfaceOptions,
            onAddField(item) {
                const fieldInterface = pick(item, [
                    'name',
                    'group',
                    'title',
                    'default',
                ...
                const { properties: { unique, type, layout, autoIncrement, ...properties }, } = item;
        ...
        React.createElement(SchemaInitializerItems, { ...props, items: items }),
        React.createElement(ActionContextProvider, { value: {
            ...
            } }, interfaceOptions ? (React.createElement(SchemaComponent, { schema: {
                type: 'void',
                name: 'drawer',
...
function CustomFormFieldInitializer() {
    ...
    return (React.createElement(SchemaInitializerItem, { key: itemConfig.fieldInterface, onClick: () => {
            setCallback(() => insert);
    ...
    config: {
        ...
        parseFormOptions(root) {
            ...
            formBlocks.forEach((formBlock) => {
                ...
                formBlock['x-decorator-props'].collection.fields = fields.map((field) => field['x-component-props']?.field ?? field['x-interface-options']);
