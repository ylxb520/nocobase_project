...
import { UserDataResource, } from '@nocobase/plugin-user-data-sync';
    ...
    async updateUserDepartments(user, currentDepartmentIds, sourceDepartments, sourceName) {
        ...
        if (!sourceDepartments || !sourceDepartments.length) {
            ...
            for (const sourceDepartment of sourceDepartments) {
                ...
                this.logger.debug('update dept owner: ' +
            ...
            if (currentDepartmentIds !== undefined && currentDepartmentIds.length > 0) {
                const toRemoveDepartmentIds = currentDepartmentIds.filter((currentDepartmentId) => !newDepartmentIds.includes(currentDepartmentId));
                recordResourceChangeds.push(...toRemoveDepartmentIds.map((departmentId) => ({ resourcesPk: departmentId, isDeleted: true })));
                const toAddDepartmentIds = newDepartmentIds.filter((newDepartmentId) => !currentDepartmentIds.includes(newDepartmentId));
                recordResourceChangeds.push(...toAddDepartmentIds.map((departmentId) => ({ resourcesPk: departmentId, isDeleted: false })));
            }
            else {
                recordResourceChangeds.push(...toRealAddDepartments.map((department) => ({ resourcesPk: department.id, isDeleted: false })));
            }
...
//# sourceMappingURL=department-data-sync-resource.js.map