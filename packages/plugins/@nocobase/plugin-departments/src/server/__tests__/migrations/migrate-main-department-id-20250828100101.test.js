...
async function verifyMigrationSuccess(db, userId, deptId) {
  const fieldsRepo = db.getRepository('fields');
  ...
  const opts = convertedField?.get('options');
  expect(opts?.foreignKey).toBe('mainDepartmentId');
}
//# sourceMappingURL=migrate-main-department-id-20250828100101.test.js.map
