 ...
 */
export const getAppendsOwners = async (ctx, next) => {
...
export const aggregateSearch = async (ctx, next) => {
...
export const setOwner = async (ctx, next) => {
...
export const removeOwner = async (ctx, next) => {
    const { userId, departmentId } = ctx.action.params.values || {};
    const throughRepo = ctx.db.getRepository('departmentsUsers');
    await throughRepo.update({
        filter: {
