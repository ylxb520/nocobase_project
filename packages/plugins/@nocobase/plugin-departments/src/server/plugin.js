...
import { destroyDepartmentCheck, resetUserDepartmentsCache, setDepartmentOwners, setMainDepartment as setMainDepartmentMiddleware, updateDepartmentIsLeaf, setDepartmentsInfo, } from './middlewares';
...
export class PluginDepartmentsServer extends Plugin {
    ...
    registerErrorHandler() {
        ...
        errorHandlerPlugin.errorHandler.register((err) => {
            return err.message === "Invalid main department, it must be one of the user's departments";
        }, (err, ctx) => {
            // 翻译错误消息
            return ctx.throw(400, ctx.i18n.t("Invalid main department, it must be one of the user's departments", { ns: 'departments' }));
        });
    }
}
