...
const AddMembersListProvider = (props) => {
...
const RowRemoveAction = () => {
  const { department } = useContext(ResourcesContext);
  return department
    ? React.createElement(SchemaComponent, { scope: { useRemoveMemberAction }, schema: rowRemoveActionSchema })
    : null;
};
const MemberActions = () => {
  const { department } = useContext(ResourcesContext);
  return department ? React.createElement(SchemaComponent, { schema: membersActionSchema }) : null;
};
...
export const Member = () => {
  ...
  return React.createElement(
    ...
    null,
    !user
      ? React.createElement('h2', null, t(department?.title || 'All users'))
      : React.createElement('h2', null, t('Search results')),
