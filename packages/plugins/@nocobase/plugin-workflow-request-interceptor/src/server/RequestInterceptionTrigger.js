...
export default class RequestInterceptionTrigger extends Trigger {
    ...
    middleware = async (context, next) => {
        const { resourceName, actionName, params: { filterByTk, filter, values, triggerWorkflows = '' }, } = context.action;
        ...
        const workflows = Array.from(this.workflow.enabledCache.values()).filter((item) => item.type === this.constructor.TYPE &&
    ...
    constructor(workflow) {
        ...
        workflow.app.pm.get(PluginErrorHandler).errorHandler.register((err) => err instanceof RequestInterceptionError || err.name === 'RequestInterceptionError', async (err, ctx, next) => {
            ctx.body = {
                errors: err.messages,
            };
