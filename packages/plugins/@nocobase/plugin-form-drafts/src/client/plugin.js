        ...
        createDraft: {
            ...
            async handler(ctx, params) {
                ...
                ctx.model.context.defineMethod('showDraftAlert', (message) => {
                    ...
                    ctx.model['decoratorProps'].beforeContent = (React.createElement(Alert, { style: { marginBottom: '1em' }, type: "warning", action: React.createElement(Button, { ghost: true, size: "small", danger: true, onClick: () => {
                                ctx.draftRepository.delete();
                                ctx.form.resetFields();
                                ctx.model['decoratorProps'].beforeContent = null;
                            } }, ctx.t('Delete draft', { ns: NAMESPACE })), description: ctx.t(message, { ns: NAMESPACE }) }));
                });
                if (!params.enabled) {
                ...
                const model = ctx.blockModel;
                model.form.setFieldsValue(draft.values || {});
                ctx.showDraftAlert('This is an unsubmitted draft. You can continue editing and submit it, or click “Delete draft” to start over.');
            },
        },
    },
});
    ...
    steps: {
        saveDraft: {
            async handler(ctx) {
                ...
                ctx.showDraftAlert('Your draft has been saved. You can continue editing and submit it, or click “Delete Draft” to start over.');
            },
        },
    },
});
