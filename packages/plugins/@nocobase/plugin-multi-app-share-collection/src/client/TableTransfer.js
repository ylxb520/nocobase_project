...
const useCollectionsGraph = ({ removed = [] }) => {
    ...
    const findAddable = useCallback((name) => {
    ...
    const findRemovable = useCallback((name) => {
        return CollectionsGraph.connectedNodes({
            collections,
            nodes: [name],
            excludes: excludeCollections,
...
const useRemovedDataSource = ({ collections, removed }) => {
    ...
    const setNameFilter = useMemo(() => lodash.debounce((name) => {
        setFilter({
            ...filter,
            name,
        });
...
export const TableTransfer = connect((props) => {
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            dataIndex: 'category',
            render: (categories) => categories.map((category) => (React.createElement(Tag, { key: category.name, color: category.color }, category.name))),
        },
    ], []);
    if (loading) {
    ...
    return (React.createElement("div", null,
        React.createElement(Row, { gutter: 24, className: css `
        ...
        ` },
            React.createElement(Col, { span: 12 },
              ...
              margin-bottom: 8px;
            ` },
                    React.createElement("strong", { style: { fontSize: token.fontSizeLG, color: token.colorText } }, t('Unshared collections')),
                    React.createElement(Input.Group, { compact: true, style: { width: 360 } },
                        React.createElement(Select, { popupMatchSelectWidth: false, onChange: (value) => {
                                removedDataSource.setCategoryFilter(value);
                            }, mode: 'multiple', style: { width: '35%' }, size: 'middle', placeholder: t('All categories'), options: categories, allowClear: true }),
                        React.createElement(Input, { onChange: (e) => removedDataSource.setNameFilter(e.target.value), style: { width: '65%' }, placeholder: t('Enter name or title...'), allowClear: true }))),
                React.createElement(Table, { bordered: true, rowKey: 'name', rowSelection: {
                        type: 'checkbox',
                    ...
                    }, pagination: false, size: 'small', columns: columns, 
                    // dataSource={collections.filter((collection) => removed.includes(collection.name))}
                    dataSource: removedDataSource.dataSource, scroll: { y: 'calc(100vh - 260px)' }, onRow: ({ name, disabled }) => ({
                        onClick: () => {
                            if (disabled)
                                return;
                            ...
                            modal.confirm({
                                ...
                                content: (React.createElement("div", null,
                                    React.createElement(Table, { size: 'small', columns: columns, dataSource: collections.filter((collection) => adding.includes(collection.name)), pagination: false, scroll: { y: '60vh' } }))),
                                onOk() {
                                    change();
                                },
                            });
            ...
            React.createElement(Col, { span: 12 },
              ...
              margin-bottom: 8px;
            ` },
                    React.createElement("strong", { style: { fontSize: token.fontSizeLG, color: token.colorText } }, t('Shared collections')),
                    React.createElement(Input.Group, { compact: true, style: { width: 360 } },
                        React.createElement(Select, { popupMatchSelectWidth: false, onChange: (value) => {
                                addedDataSource.setCategoryFilter(value);
                            }, mode: 'multiple', style: { width: '35%' }, size: 'middle', placeholder: t('All categories'), options: categories, allowClear: true }),
                        React.createElement(Input, { onChange: (e) => addedDataSource.setNameFilter(e.target.value), style: { width: '65%' }, placeholder: t('Enter name or title...'), allowClear: true }))),
                React.createElement(Table, { bordered: true, rowKey: 'name', rowSelection: {
                        type: 'checkbox',
                    ...
                    }, pagination: false, size: 'small', columns: columns, dataSource: addedDataSource.dataSource, 
                    // dataSource={collections.filter((collection) => !selected.includes(collection.name))}
                    scroll: { y: 'calc(100vh - 260px)' }, onRow: ({ name }) => ({
                        onClick: () => {
                            ...
                            modal.confirm({
                                ...
                                width: '60%',
                                content: (React.createElement("div", null,
                                    React.createElement(Table, { size: 'small', columns: columns, dataSource: collections.filter((collection) => removing.includes(collection.name)), pagination: false, scroll: { y: '60vh' } }))),
                                onOk() {
                                    change();
                                },
                            });
...
//# sourceMappingURL=TableTransfer.js.map