...
import { createMockServer, isPg } from '@nocobase/test';
...
describe.runIf(isPg())('collection sync after main', () => {
    ...
    it('should sync collections in sub app', async () => {
        ...
        expect(postCollection.options.schema).toBe(process.env.COLLECTION_MANAGER_SCHEMA || mainApp.db.options.schema || 'public');
...
describe.runIf(isPg())('collection sync', () => {
    ...
    it('should not sync roles in sub app', async () => {
        ...
        expect(await subApp1.db.getRepository('roles').findOne({
            filter: {
                name: 'role1',
            },
        })).toBeFalsy();
    ...
    it('should sync custom collections after sub app created', async () => {
        ...
        await subApp1.runCommand('restart');
        const postCollection = subApp1.db.getCollection('posts');
        expect(postCollection.options.schema).toBe(process.env.COLLECTION_MANAGER_SCHEMA || mainDb.options.schema || 'public');
    });
    ...
    it('should sync collections in sub apps', async () => {
        ...
        expect(await subApp1.db.getRepository('fields').findOne({
            filter: {
                collectionName: 'mainCollection',
                name: 'title',
            },
        ...
        expect(await subApp1.db.getRepository('collections').findOne({
            filter: {
                name: 'mainCollection',
            },
        })).toBeFalsy();
    ...
    it('should set collection black list', async () => {
        ...
        const BlackListRepository = await mainApp.db
            ...
            .repository.relation('collectionBlacklist')
            .of('sub1');
...
//# sourceMappingURL=collection-sync.test.js.map