...
pgOnly()('Inherited Collection', () => {
  ...
  it('should return child model at get action in belongsTo', async () => {
    ...
    const child1ViaObject1 = await db.getRepository('users.assoc', user1.get('id')).findOne({
    ...
    await db.getRepository('users.assoc', user1.get('id')).update({
      values: {
        childName: 'child2',
        __collection: 'child',
      },
    });
    const child2 = await db.getRepository('users.assoc', user1.get('id')).findOne({
      targetCollection: 'child',
    });
  ...
  it('should return child model at get action in belongsToMany', async () => {
    ...
    const child1ViaObject1 = await db.getRepository('object.assocs', object1.id).findOne({
  ...
  it('should return child model at get action in hasMany', async () => {
    ...
    expect(child1ViaObject1.get('childName')).toBe('child1');
    await db.getRepository('object.assocs', object1.id).update({
      filterByTk: child1.get('id'),
      targetCollection: 'child',
      values: {
        childName: 'child2',
    ...
    const child2 = await db.getRepository('object.assocs', object1.id).findOne({
      filterByTk: child1.get('id'),
      targetCollection: 'child',
    });
    expect(child2.get('childName')).toBe('child2');
    ...
    const child3 = await db.getRepository('object.assocs', object1.id).findOne({
      filterByTk: child1.get('id'),
      targetCollection: 'child',
    });
...
//# sourceMappingURL=inherited-collection.test.js.map
