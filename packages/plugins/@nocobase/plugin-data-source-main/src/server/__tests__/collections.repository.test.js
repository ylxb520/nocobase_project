 ...
 */
import { CollectionGroupManager } from '@nocobase/database';
...
describe('collections repository', () => {
  ...
  it('should extend collections collection', async () => {
    expect(db.getRepository('collections')).toBeTruthy();
    db.extendCollection({
      name: 'collections',
      fields: [{ type: 'string', name: 'tests' }],
    });
    expect(Collection.getField('tests')).toBeTruthy();
    const afterRepository = db.getRepository('collections');
  ...
  it('should update association field', async () => {
    ...
    await db.sync();
    await db.getRepository('collections.fields', 'c').update({
      filterByTk: 'a',
      values: {
        key: C.key,
        collectionName: 'c',
...
//# sourceMappingURL=collections.repository.test.js.map
