 ...
 */
import { expect, expectInitializerMenu, oneTableBlockWithAddNewAndViewAndEditAndBasicFields, test, } from '@nocobase/test/e2e';
...
test.describe('form item & edit form', () => {
    test('configure fields', async ({ page, mockPage, mockRecord }) => {
        ...
        await expectInitializerMenu({
            ...
            expectValue: async () => {
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.email-email').getByRole('textbox')).toHaveValue(record.email);
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.singleLineText-singleLineText')
                    .getByRole('textbox')).toHaveValue(record.singleLineText);
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.integer-integer').getByRole('spinbutton')).toHaveValue(String(record.integer));
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.number-number').getByRole('spinbutton')).toHaveValue(String(record.number));
                // await expect(
                ...
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.longText-longText').getByRole('textbox')).toHaveValue(record.longText);
            },
        });
    });
});
test.describe('form item & view form', () => {
    test('configure fields', async ({ page, mockPage, mockRecord }) => {
        ...
        await expectInitializerMenu({
            ...
            expectValue: async () => {
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.email-email').getByText(record.email)).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.singleLineText-singleLineText')
                    .getByText(record.singleLineText)).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.singleLineText-singleLineText')
                    .getByText(record.singleLineText)).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.integer-integer')
                    .getByText(record.integer.toLocaleString())).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.number-number')
                    .getByText(record.number.toFixed(0))).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.percent-percent')
                    .getByText(`${Math.round(record.percent * 100).toLocaleString()}%`)).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.longText-longText')
                    .getByText(record.longText.slice(0, 10))).toBeVisible();
            },
        });
    });
});
    ...
    test('configure columns', async ({ page, mockPage, mockRecord }) => {
        ...
        await expectInitializerMenu({
            ...
            expectValue: async () => {
                ...
                await expect(page.getByRole('button', { name: `${Math.round(record.percent * 100).toLocaleString()}%` })).toBeVisible();
                await expect(page.getByRole('button', { name: record.longText.slice(0, 10) })).toBeVisible();
            },
        });
    });
});
//# sourceMappingURL=schemaInitializer.test.js.map