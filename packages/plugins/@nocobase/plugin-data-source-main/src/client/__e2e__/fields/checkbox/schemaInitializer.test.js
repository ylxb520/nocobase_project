 ...
 */
import { expect, expectInitializerMenu, oneTableBlockWithAddNewAndViewAndEditAndChoicesFields, test, } from '@nocobase/test/e2e';
...
test.describe('form item & edit form', () => {
    test('configure fields', async ({ page, mockPage, mockRecord }) => {
        ...
        await expectInitializerMenu({
            ...
            supportedOptions: ['checkbox', 'chinaRegion', 'multipleSelect', 'radioGroup', 'singleSelect', 'checkboxGroup'],
            expectValue: async () => {
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox').getByRole('checkbox')).toBeChecked({ checked: record.checkbox });
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.multipleSelect-multipleSelect')
                    .getByText(record.multipleSelect.map((item) => item).join(''))).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.singleSelect-singleSelect')
                    .getByText(record.singleSelect)).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByRole('checkbox', { name: 'Option1' })).toBeChecked({ checked: record.checkboxGroup.includes('option1') });
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByRole('checkbox', { name: 'Option2' })).toBeChecked({ checked: record.checkboxGroup.includes('option2') });
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByRole('checkbox', { name: 'Option3' })).toBeChecked({ checked: record.checkboxGroup.includes('option3') });
            },
        });
    });
});
            ...
            expectValue: async () => {
                if (record.checkbox) {
                    await expect(page
                        ...
                        .getByLabel('check', { exact: true })).toBeVisible();
                }
                else {
                    await expect(page
                        .getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox')
                        .getByLabel('check', { exact: true })).toBeHidden();
                }
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.multipleSelect-multipleSelect')
                    .getByText(record.multipleSelect.join(''))).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.singleSelect-singleSelect')
                    .getByText(record.singleSelect)).toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByText(record.checkboxGroup.join(''))).toBeVisible();
            },
        });
    });
});
