 ...
 */
import { expect, expectInitializerMenu, mockUserRecordsWithoutDepartments, oneTableBlockWithAddNewAndViewAndEditAndAssociationFields, test, } from '@nocobase/test/e2e';
...
test.describe('form item & edit form', () => {
    test('configure fields', async ({ page, mockPage, mockRecords, mockRecord }) => {
        ...
        await expectInitializerMenu({
            ...
            supportedOptions: ['oneToMany', 'manyToOne', 'manyToMany', 'oneToOneBelongsTo', 'oneToOneHasOne'],
            expectValue: async () => {
                await expect(page.getByRole('button', { name: record.oneToMany.map((item) => item.id).join(' ') })).toBeVisible();
                await expect(page.getByRole('button', { name: record.manyToOne.id })).toBeVisible();
                await expect(page.getByRole('button', { name: record.manyToMany.map((item) => item.id).join(' ') })).toBeVisible();
...
test.describe('table column & table', () => {
    test('configure columns', async ({ page, mockPage, mockRecord }) => {
        ...
        await expectInitializerMenu({
            ...
            supportedOptions: ['oneToMany', 'manyToOne', 'manyToMany', 'oneToOneBelongsTo', 'oneToOneHasOne'],
            expectValue: async () => {
                await expect(page.getByText(record.oneToMany.map((item) => item.id).join(','))).toBeVisible();
                await expect(page.getByText(record.manyToOne.id)).toBeVisible();
                await expect(page.getByText(record.manyToMany.map((item) => item.id).join(','))).toBeVisible();
