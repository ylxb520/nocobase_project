 ...
 */
import { expect, expectSettingsMenu, oneTableBlockWithAddNewAndViewAndEditAndChoicesFields, test, } from '@nocobase/test/e2e';
    ...
    test('set default value', async ({ page, mockPage }) => {
        await testDefaultValue({
            ...
            expectConstantValue: async () => {
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox').getByRole('checkbox')).toBeChecked();
            },
        });
    });
    test('pattern', async ({ page, mockPage, mockRecord }) => {
        await testPattern({
            ...
            expectReadonly: async () => {
                // checkbox 应该被禁用
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox').getByRole('checkbox')).toBeDisabled();
            },
            expectEasyReading: async () => {
                // checkbox 应该被隐藏，然后只显示一个图标
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox').getByRole('checkbox')).not.toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox')
                    .getByRole('img', { name: 'check' })).toBeVisible();
            },
        });
    });
});
        ...
        await testPattern({
            ...
            expectEasyReading: async () => {
                if (record.checkbox) {
                    await expect(page
                        .getByLabel('block-item-CollectionField-general-form-general.checkbox-checkbox')
                        .getByRole('img', { name: 'check' })).toBeVisible({ visible: record.checkbox });
                }
                else {
...
//# sourceMappingURL=schemaSettings.test.js.map