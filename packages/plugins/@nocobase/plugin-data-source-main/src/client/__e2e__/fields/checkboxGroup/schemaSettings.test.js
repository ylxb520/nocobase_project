        ...
        await testDefaultValue({
            ...
            expectConstantValue: async () => {
                await expect(page
                    ...
                    .getByRole('checkbox', { name: 'Option1' })).toBeChecked();
            },
        });
    });
    test('pattern', async ({ page, mockPage }) => {
        await testPattern({
            ...
            expectReadonly: async () => {
                ...
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByRole('checkbox', { name: 'Option1' })).toBeDisabled();
            },
            expectEasyReading: async () => {
                // checkbox 应该被隐藏，然后只显示一个标签
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByRole('checkbox', { name: 'Option1' })).not.toBeVisible();
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByText('Option1')).toBeVisible();
            },
        });
    });
});
        ...
        await testPattern({
            ...
            expectEditable: async () => {
                for (const option of record.checkboxGroup) {
                    await expect(page
                        .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                        .getByRole('checkbox', { name: option })).toBeChecked();
                }
            },
            expectReadonly: async () => {
                // checkbox 应该被禁用
            ...
            expectEasyReading: async () => {
                ...
                await expect(page
                    .getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')
                    .getByRole('checkbox', { name: 'Option1' })).not.toBeVisible();
                await expect(page.getByLabel('block-item-CollectionField-general-form-general.checkboxGroup-checkboxGroup')).toHaveText(`checkboxGroup:${record.checkboxGroup.join('')}`, { ignoreCase: true });
            },
        });
    });
});
