...
const InternalExpression = observer((props) => {
    ...
    return React.createElement(Variable.TextArea, { ...props, scope: options });
...
function Result(props) {
    ...
    const compile = useCompile();
    const collectionManager = useCollectionManager();
    const options = useMemo(() => getCollectionFieldOptions({ collection: values.sourceCollection, compile, collectionManager }), [collectionManager, compile, values.sourceCollection]);
    return props.value ? (React.createElement(Variable.TextArea, { ...props, scope: options })) : (React.createElement(Tag, null, t('Unconfigured', { ns: NAMESPACE })));
}
export const DynamicExpression = connect(InternalExpression, mapReadPretty(Result));
//# sourceMappingURL=DynamicExpression.js.map