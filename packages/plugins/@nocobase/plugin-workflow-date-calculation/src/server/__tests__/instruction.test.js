...
describe('workflow-date-calculation', () => {
    ...
    describe('date calculate functions', () => {
        ...
        it('transDuration', async () => {
            ...
            function runTest({ unitBefore, unitAfter, inputValue, expectValue, round = false, throwMessage }) {
                console.log(`test: unitBefore: ${unitBefore}, unitAfter: ${unitAfter}, input: ${inputValue}, expectValue: ${expectValue}`);
            ...
            for (let i = 0; i < tests.length; i++) {
                ...
                for (let j = 0; j < test.transTo.length; j++) {
                    const transItem = test.transTo[j];
        ...
        it('changeTimezone', async () => {
            ...
            expect(changeTimezone(dayjs('2024-07-15T22:07:30.23Z'), { timezone: 'Asia/Shanghai' }).format('YYYY-MM-DD HH:mm:ss')).toBe('2024-07-16 06:07:30');
        });
    });
