...
function TimezoneSelect(props) {
    ...
    return React.createElement(Select, { ...props, options: options });
}
        ...
        const items = Object.keys(group).map((key) => {
            ...
            const groupItem = {
                ...
                children: group[key].map((item) => ({
                    ...
                    label: (React.createElement(Flex, { justify: "space-between", align: "center", gap: "large" },
                        compile(item.title),
                        React.createElement(Tag, { color: DATA_TYPES_OPTION_MAP[item.outputType].color, className: css `
                  margin-inline-end: 0;
