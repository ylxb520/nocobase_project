 ...
 */
import { OFFICIAL_PLUGIN_PREFIX, Plugin } from '@nocobase/server';
...
export class PluginLocalizationServer extends Plugin {
    ...
    addNewTexts = async (texts, options) => {
        ...
        await this.db
            ...
            .bulkCreate(texts.map(({ text, module }) => ({
            module,
            text,
        })), {
            ...
            if (options?.locale && newTexts?.length) {
                await this.db.getModel('localizationTranslations').bulkCreate(newTexts.map((text) => ({
