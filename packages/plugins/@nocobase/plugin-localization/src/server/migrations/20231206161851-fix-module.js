...
export const getSchemaUid = async (db, migrate = false) => {
    if (migrate) {
...
export const getTextsFromMenu = async (db, migrate = false) => {
    const result = {};
    ...
    if (mobileSchemaUid) {
        ...
        if (schema?.['properties']?.tabBar?.properties) {
            Object.values(schema['properties']?.tabBar?.properties).forEach((item) => {
...
//# sourceMappingURL=20231206161851-fix-module.js.map