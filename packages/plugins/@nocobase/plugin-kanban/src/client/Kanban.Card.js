...
import { DndContext, FormProvider, getCardItemSchema, PopupContextProvider, useCollection, useCollectionRecordData, usePopupSettings, usePopupUtils, VariablePopupRecordProvider, } from '@nocobase/client';
...
export const KanbanCard = () => {
    ...
    const { layout = 'vertical', labelAlign = 'left', labelWidth = 120, labelWrap = true, } = cardItemSchema?.['x-component-props'] || {};
    return (React.createElement(React.Fragment, null,
        React.createElement(PopupContextProvider, null,
            React.createElement(Card, { onClick: handleCardClick, bordered: false, hoverable: true, style: cardStyle, className: cardCss },
                React.createElement(DndContext, { onDragStart: onDragStart, onDragEnd: onDragEnd },
                    React.createElement(FormLayout, { layout: layout, labelAlign: labelAlign, labelWidth: layout === 'horizontal' ? labelWidth : null, labelWrap: labelWrap },
                        React.createElement(FormProvider, { form: form },
                            React.createElement(MemorizedRecursionField, { schema: fieldSchema, onlyRenderProperties: true })))))),
        React.createElement(PopupContextProvider, { visible: visible, setVisible: setVisible },
            React.createElement(VariablePopupRecordProvider, { recordData: recordData, collection: collection },
                React.createElement(MemorizedRecursionField, { schema: wrappedPopupSchema })))));
};
function getPopupSchemaFromParent(fieldSchema) {
...
function findSchemaByUid(uid, rootSchema, resultRef = { value: null }) {
    resultRef = resultRef || {
        value: null,
    };
