...
const Columns = forwardRef((props, ref) => {
    return React.createElement("div", { ref: ref, style: { whiteSpace: 'nowrap', overflowY: 'clip' }, ...props });
});
...
function UncontrolledBoard({ initialBoard, onCardDragEnd, onColumnDragEnd, allowAddColumn, cardAdderPosition, renderCardAdder, renderColumnAdder, onNewColumnConfirm, onColumnRemove, renderColumnHeader, allowRemoveColumn, allowRenameColumn, onColumnRename, onCardNew, renderCard, allowRemoveCard, onCardRemove, onColumnNew, disableCardDrag, disableColumnDrag, allowAddCard, onNewCardConfirm, }) {
    ...
    return (React.createElement(BoardContainer, { cardAdderPosition: cardAdderPosition, onCardDragEnd: handleOnCardDragEnd, onColumnDragEnd: handleOnColumnDragEnd, renderCardAdder: renderCardAdder, renderColumnAdder: () => {
            if (!allowAddColumn)
                return null;
            ...
            if (!onNewColumnConfirm)
                return null;
            return React.createElement(ColumnAdder, { onConfirm: (title) => handleColumnAdd({ title, cards: [] }) });
        }, ...(renderColumnHeader && {
            ...
            return (React.createElement(DefaultCard, { dragging: dragging, allowRemoveCard: allowRemoveCard, onCardRemove: (card) => handleCardRemove(column, card) }, card));
...
function ControlledBoard({ children: board, onCardDragEnd, onColumnDragEnd, allowAddColumn, renderCardAdder, renderColumnAdder, onNewColumnConfirm, onColumnRemove, renderColumnHeader, allowRemoveColumn, allowRenameColumn, onColumnRename, renderCard, allowAddCard, allowRemoveCard, onCardRemove, disableCardDrag, disableColumnDrag, cardAdderPosition, }) {
    ...
    return (React.createElement(BoardContainer, { cardAdderPosition: cardAdderPosition, onCardDragEnd: handleOnCardDragEnd, onColumnDragEnd: handleOnColumnDragEnd, renderCardAdder: renderCardAdder, renderColumnAdder: () => {
            ...
            if (!onNewColumnConfirm)
                return null;
            return React.createElement(ColumnAdder, { onConfirm: (title) => onNewColumnConfirm({ title, cards: [] }) });
        }, ...(renderColumnHeader && { renderColumnHeader: renderColumnHeader }), renderCard: (column, card, dragging) => {
            if (renderCard)
                return renderCard(card, { column, dragging });
            return (React.createElement(DefaultCard, { dragging: dragging, allowRemoveCard: allowRemoveCard, onCardRemove: (card) => onCardRemove(card, column) }, card));
        }, allowAddCard: allowAddCard, allowRemoveColumn: allowRemoveColumn, onColumnRemove: onColumnRemove, allowRenameColumn: allowRenameColumn, onColumnRename: onColumnRename, disableColumnDrag: disableColumnDrag, disableCardDrag: disableCardDrag }, board));
}
function BoardContainer(props) {
    const { children: board, renderCard, disableColumnDrag, disableCardDrag, cardAdderPosition, renderCardAdder, renderColumnHeader, renderColumnAdder, allowRemoveColumn, onColumnRemove, allowRenameColumn, onColumnRename, onColumnDragEnd, onCardDragEnd, onCardNew, allowAddCard, } = props;
    ...
    return (React.createElement(DragDropContext, { onDragEnd: handleOnDragEnd },
        React.createElement("div", { style: { overflowY: 'hidden', display: 'flex', alignItems: 'flex-start', height: '100%' } },
            React.createElement(DroppableBoard, { droppableId: "board-droppable", direction: "horizontal", type: "BOARD", style: { height: '200px' } }, board.columns?.map((column, index) => (React.createElement(Column, { key: column.id, index: column?.index ?? index, renderCard: renderCard, renderCardAdder: renderCardAdder, renderColumnHeader: (column) => renderColumnHeader ? (renderColumnHeader(column)) : (React.createElement(DefaultColumnHeader, { allowRemoveColumn: allowRemoveColumn, onColumnRemove: onColumnRemove, allowRenameColumn: allowRenameColumn, onColumnRename: onColumnRename }, column)), cardAdderPosition: cardAdderPosition, disableColumnDrag: disableColumnDrag, disableCardDrag: disableCardDrag, onCardNew: onCardNew, allowAddCard: allowAddCard }, column)))),
            renderColumnAdder())));
}
