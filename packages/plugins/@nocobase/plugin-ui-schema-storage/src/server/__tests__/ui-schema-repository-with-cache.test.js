...
describe('ui_schema repository with cache', () => {
    ...
    beforeEach(async () => {
        ...
        db = app.db;
        repository = db.getCollection('uiSchemas').repository;
        cache = app.cache;
    ...
    it('should cache when getJsonSchema with readFromCache', async () => {
        ...
        const result = await repository.getJsonSchema(xUid, { readFromCache: true });
        ...
        const secondResult = await repository.getJsonSchema(xUid, { readFromCache: true });
    ...
    it('should not cache when getJsonSchema with readFromCache false', async () => {
        ...
        expect(await cache.get(xUid)).toBeUndefined();
        await repository.insert(schema);
        const result = await repository.getJsonSchema(xUid, { readFromCache: false });
    ...
    it('should cache when getProperties with readFromCache', async () => {
        ...
        expect(await cache.get(xUid)).toBeUndefined();
        await repository.insert(schema);
        const result = await repository.getProperties(xUid, { readFromCache: true });
        ...
        const secondResult = await repository.getProperties(xUid, { readFromCache: true });
        expect(secondResult).toMatchObject(result);
    });
    it('should not cache when getProperties with readFromCache false', async () => {
        ...
        expect(await cache.get(xUid)).toBeUndefined();
        await repository.insert(schema);
        const result = await repository.getProperties(xUid, { readFromCache: false });
        expect(result).toBeDefined();
    ...
    it('should clear cache when remove', async () => {
        ...
        expect(await cache.get(xUid)).toBeUndefined();
        await repository.insert(schema);
        const sResult = await repository.getJsonSchema(xUid, { readFromCache: true });
        expect(await cache.get(`s_${xUid}`)).toMatchObject(sResult);
        const pResult = await repository.getProperties(xUid, { readFromCache: true });
    ...
    it('should clear cache when remove children schema', async () => {
        ...
        expect(await cache.get(xUid)).toBeUndefined();
        await repository.insert(schema);
        let sResult = await repository.getJsonSchema(xUid, { readFromCache: true });
        expect(await cache.get(`s_${xUid}`)).toMatchObject(sResult);
        expect(sResult.properties.a1.properties.c1).toBeDefined();
        let pResult = (await repository.getProperties(xUid, { readFromCache: true }));
        expect(await cache.get(`p_${xUid}`)).toMatchObject(pResult);
        expect(pResult.properties.a1.properties.c1).toBeDefined();
        await repository.remove(schema.properties.a1.properties.c1['x-uid']);
        expect(await cache.get(`s_${xUid}`)).toBeUndefined();
        expect(await cache.get(`p_${xUid}`)).toBeUndefined();
        sResult = await repository.getJsonSchema(xUid, { readFromCache: true });
        expect(await cache.get(`s_${xUid}`)).toMatchObject(sResult);
        expect(sResult.properties.a1.properties.c1).toBeUndefined();
        pResult = (await repository.getProperties(xUid, { readFromCache: true }));
        expect(await cache.get(`p_${xUid}`)).toMatchObject(pResult);
    ...
    it('should clear cache when move children schema 2', async () => {
        ...
        let sResult1 = await repository.getJsonSchema(xUid_A, { readFromCache: true });
        expect(await cache.get(`s_${xUid_A}`)).toMatchObject(sResult1);
        let pResult1 = (await repository.getProperties(xUid_A, { readFromCache: true }));
        expect(await cache.get(`p_${xUid_A}`)).toMatchObject(pResult1);
        let sResult2 = await repository.getJsonSchema(xUid_D, { readFromCache: true });
        expect(await cache.get(`s_${xUid_D}`)).toMatchObject(sResult2);
        let pResult2 = (await repository.getProperties(xUid_D, { readFromCache: true }));
        ...
        expect(await cache.get(`s_${xUid_D}`)).toBeUndefined();
        expect(await cache.get(`p_${xUid_D}`)).toBeUndefined();
        sResult1 = await repository.getJsonSchema(xUid_A, { readFromCache: true });
        expect(await cache.get(`s_${xUid_A}`)).toMatchObject(sResult1);
        pResult1 = (await repository.getProperties(xUid_A, { readFromCache: true }));
        expect(await cache.get(`p_${xUid_A}`)).toMatchObject(pResult1);
        sResult2 = await repository.getJsonSchema(xUid_D, { readFromCache: true });
        expect(await cache.get(`s_${xUid_D}`)).toMatchObject(sResult2);
        pResult2 = (await repository.getProperties(xUid_D, { readFromCache: true }));
        expect(await cache.get(`p_${xUid_D}`)).toMatchObject(pResult2);
...
//# sourceMappingURL=ui-schema-repository-with-cache.test.js.map