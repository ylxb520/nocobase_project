...
describe('resolveExpectedRootUse', () => {
    it('allows ApplyFormModel and ProcessFormModel to share templates', () => {
        expect(resolveExpectedRootUse({ use: 'ApplyFormModel' })).toEqual([
            'ApplyFormModel',
            'ProcessFormModel',
        ]);
        expect(resolveExpectedRootUse({ use: 'ProcessFormModel' })).toEqual([
            'ApplyFormModel',
            'ProcessFormModel',
        ]);
    });
    it('keeps CreateFormModel and EditFormModel sharing behavior', () => {
        expect(resolveExpectedRootUse({ use: 'CreateFormModel' })).toEqual([
            'CreateFormModel',
            'EditFormModel',
        ]);
        expect(resolveExpectedRootUse({ use: 'EditFormModel' })).toEqual(['CreateFormModel', 'EditFormModel']);
    });
});
describe('SubModelTemplateImporterModel', () => {
    it('stores derived targetUid into stepParams in beforeParamsSave', async () => {
        ...
        const form = engine.createModel({ uid: 'host-form', use: 'FormBlockModel' });
        ...
        const importer = engine.createModel({
            uid: 'importer-1',
        ...
        const flow = importer.getFlow('subModelTemplateImportSettings');
        const step = flow?.getStep?.('selectTemplate')?.serialize?.();
        ...
        const params = { templateUid: 'tpl-1', mode: 'reference' };
    ...
    it('filters by expectedRootUse and disables mismatched dataSource/collection', async () => {
        ...
        const { options } = await importer.fetchTemplateOptions(ctx, '');
        ...
        const mismatch = options.find((o) => o.value === 'tpl-mismatch');
    ...
    it('resolves expected collection via associationName when collectionName is missing/wrong', async () => {
        ...
        if (!main)
            throw new Error('missing main dataSource');
    ...
    it('skips confirm when current grid has no FormItem fields', async () => {
        ...
        const dummy = engine.createModel({
            uid: 'dummy-item',
            use: 'DummyItemModel',
            parentId: grid.uid,
            subKey: 'items',
    ...
    it('prompts confirm when details grid has existing items', async () => {
        ...
        const block = engine.createModel({ uid: 'host-details', use: 'DetailsBlockModel' });
        const grid = engine.createModel({
            uid: 'host-grid',
    ...
    it('waits parent grid saveStepParams before replaceModel save', async () => {
        ...
        const saveCalls = [];
        let resolveStepParamsSave;
        const stepParamsSavePromise = new Promise((resolve) => {
            resolveStepParamsSave = () => resolve({ uid: 'host-grid' });
        });
        engine.setModelRepository({
            findOne: vi.fn(async () => null),
            save: vi.fn(async (model, options) => {
                saveCalls.push({
        ...
        const page = engine.createModel({ uid: 'page', use: 'PageModel' });
        const form = engine.createModel({
            uid: 'host-form',
            use: 'FormBlockModel',
            parentId: page.uid,
        ...
        expect(form.subModels?.grid?.use).toBe('ReferenceFormGridModel');
    });
});
//# sourceMappingURL=SubModelTemplateImporterModel.test.js.map