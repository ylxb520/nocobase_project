...
describe('ReferenceBlockModel', () => {
    let engine;
    let gridModel;
    let targetBlockModel;
    let referenceBlockModel;
    let scopedEngine;
    let store;
    let lastSavedSnapshot;
    beforeEach(() => {
        vi.spyOn(ReferenceBlockModel.prototype, 'rerender').mockResolvedValue(undefined);
        ...
        const mockRepository = {
            ...
            save: vi.fn(async (model) => {
                ...
                if (sub && typeof sub === 'object') {
                    Object.values(sub).forEach((child) => {
                        if (!child)
                            return;
        ...
        vi.spyOn(ReferenceBlockModel.prototype, '_ensureScopedEngine').mockReturnValue(scopedEngine);
        vi.spyOn(ReferenceBlockModel.prototype, '_resolveFinalTarget').mockImplementation(async (uid) => {
            if (!uid)
                return null;
    ...
    describe('Original block should remain', () => {
        it('should not change target parentId when creating a reference block', async () => {
            // 记录目标区块的初始 parentId
            const originalParentId = targetBlockModel._options.parentId;
            ...
            const currentParentId = targetBlockModel._options.parentId;
            ...
            expect(targetBlockModel._options.parentId).not.toBe('reference-block-uid');
        }, TEST_TIMEOUT);
        it('should not persist target block when saving the reference block', async () => {
            referenceBlockModel = engine.createModel({
                uid: 'reference-block-uid',
                use: 'ReferenceBlockModel',
                parentId: 'grid-uid',
        ...
        it('should expose target collection on reference block context', async () => {
            referenceBlockModel = engine.createModel({
                uid: 'reference-block-uid',
                use: 'ReferenceBlockModel',
                parentId: 'grid-uid',
        ...
        it('should find target block by grid parentId after a reload', async () => {
            ...
            expect(reloadedTargetBlock.parentId).toBe('grid-uid');
            // 验证 Grid 可以通过 findModelByParentId 找到目标区块
            const mockRepo = engine.modelRepository;
    ...
    describe('Template option disabled rules', () => {
        it('disables template when associationName mismatches in association context', async () => {
            ...
            const flow = referenceBlockModel.getFlow('referenceSettings');
            const step = flow?.getStep?.('useTemplate');
            ...
            const field = { componentProps: {}, data: {} };
            reactions[0](field);
    ...
    describe('Reference block basics', () => {
        it('should keep target runtime context engine scoped after parent delegation', async () => {
            referenceBlockModel = engine.createModel({
                uid: 'reference-block-uid',
                use: 'ReferenceBlockModel',
                parentId: 'grid-uid',
            ...
            gridModel.addSubModel('items', referenceBlockModel);
            await referenceBlockModel.dispatchEvent('beforeRender');
            const targetModel = referenceBlockModel._targetModel;
            expect(targetModel).toBeDefined();
            expect(targetModel.context.engine).toBe(scopedEngine);
        }, TEST_TIMEOUT);
        it('should resolve the target block correctly', async () => {
            gridModel.addSubModel('items', targetBlockModel);
            referenceBlockModel = engine.createModel({
                uid: 'reference-block-uid',
                use: 'ReferenceBlockModel',
        ...
        it('should clear target model when target UID is empty', async () => {
            referenceBlockModel = engine.createModel({
                uid: 'reference-block-uid',
                use: 'ReferenceBlockModel',
                parentId: 'grid-uid',
        ...
        it('should render reference block title from target', async () => {
            // 创建目标区块，包含标题
            ...
            expect(referenceBlockModel.extraTitle).toMatch(/Reference template|引用模板/);
        }, TEST_TIMEOUT);
    });
});
//# sourceMappingURL=ReferenceBlockModel.test.js.map