...
import { BlockModel, CommonItemModel, FilterFormBlockModel } from '@nocobase/client';
import { FlowModel, createBlockScopedEngine, FlowExitException, isInheritedFrom, tExpr, } from '@nocobase/flow-engine';
...
import { TEMPLATE_LIST_PAGE_SIZE, calcHasMore, getTemplateAvailabilityDisabledReason, parseResourceListResponse, resolveExpectedResourceInfoByModelChain, } from '../utils/templateCompatibility';
...
function resolveMappedFormItemUse(mountTarget) {
    const sources = [
        mountTarget,
        mountTarget.context,
    ];
...
function normalizeGridModelOptionsForMappedFormItemUse(input, mappedFormItemUse, options, depth = 0) {
    ...
    if (next.subModels) {
        ...
        next.subModels = Object.fromEntries(Object.entries(subModels).map(([k, v]) => [
            k,
...
function isModelInsideReferenceBlock(model) {
    if (!model)
        return false;
    return model.parent?.use === 'ReferenceBlockModel';
}
export function resolveExpectedRootUse(blockModel) {
    // Create/Edit：允许互通
...
export class SubModelTemplateImporterModel extends CommonItemModel {
    resolveExpectedResourceInfo(ctx, start) {
    ...
    async afterAddAsSubModel() {
        ...
        const step = (this.getStepParams(FLOW_KEY, 'selectTemplate') || {});
        ...
        if (mode === 'copy') {
            ...
            const gridModel = root.subModels?.grid;
            ...
            const mappedUse = rawMappedUse && rawMappedUse !== 'FormItemModel' && mountTarget.flowEngine.getModelClass(rawMappedUse)
                ? rawMappedUse
                : undefined;
            const mappedClass = mappedUse
                ? mountTarget.flowEngine.getModelClass(mappedUse)
                : undefined;
            const normalizeFn = mappedClass && typeof mappedClass.normalizeSubModelTemplateImportNode === 'function'
            ...
            const newGrid = mountTarget.flowEngine.createModel({
                ...merged.options,
        ...
        const oldStepParams = existingGrid.stepParams && typeof existingGrid.stepParams === 'object'
            ? existingGrid.stepParams
            : {};
        const prevRefSettings = oldStepParams[GRID_REF_FLOW_KEY] && typeof oldStepParams[GRID_REF_FLOW_KEY] === 'object'
            ? oldStepParams[GRID_REF_FLOW_KEY]
            : {};
    ...
    getTemplateDisabledReason(ctx, tpl, expected) {
        ...
        if (!expectedDataSourceKey || !expectedCollectionName)
            return undefined;
        return getTemplateAvailabilityDisabledReason(ctx, tpl, { dataSourceKey: expectedDataSourceKey, collectionName: expectedCollectionName }, { dataSourceManager: this.context.dataSourceManager });
    }
    async fetchTemplateOptions(ctx, keyword, pagination) {
        const api = ctx.api;
        if (!api?.resource)
        ...
        try {
            ...
            const expects = expectedRootUse
                ...
                : [];
            // useModel 不匹配直接不显示
            const useModelFilter = expects.length > 0
                ? {
            ...
            const { rows: rawRows, count } = parseResourceListResponse(res);
            ...
            const withIndex = rawRows.flatMap((r, idx) => {
                ...
                if (expects.length > 0) {
                    if (!useModel)
                        return [];
                    if (!expects.includes(String(useModel)))
                        return [];
                }
...
SubModelTemplateImporterModel.define({
    ...
    createModelOptions: (ctx) => {
        ...
        const expectedDataSourceKey = typeof resourceInit?.dataSourceKey === 'string' ? resourceInit.dataSourceKey : undefined;
        const expectedCollectionName = typeof resourceInit?.collectionName === 'string' ? resourceInit.collectionName : undefined;
        return {
    ...
    steps: {
        selectTemplate: {
            ...
            uiSchema: (ctx) => {
                ...
                const hasMappedUse = rawMappedUse && rawMappedUse !== 'FormItemModel' && !!mountTarget?.flowEngine?.getModelClass?.(rawMappedUse);
                const fetchOptions = (keyword, pagination) => m.fetchTemplateOptions(ctx, keyword, pagination);
                return {
                    templateUid: {
                        title: tStr('Template'),
                        ...
                        'x-reactions': [
                            (field) => {
                                bindInfiniteScrollToFormilySelect(field, async (keyword, page, pageSize) => {
            ...
            async beforeParamsSave(ctx, params) {
                const importer = ctx.model;
                const parent = importer.parent;
                if (!parent) {
                ...
                const api = ctx.api;
                if (!mountTarget) {
                    throw new Error(`[block-reference] Cannot resolve mount target from importer parent (uid='${parent.uid}', use='${parent.use}').`);
                }
                ...
                const expectedResource = importer.resolveExpectedResourceInfo(ctx, mountTarget);
                const disabledReason = importer.getTemplateDisabledReason(ctx, tpl || {}, expectedResource);
                if (disabledReason) {
                ...
                if (mode !== 'copy') {
                    ...
                    if (isReferenceGrid) {
                        ...
                        const isSame = norm(existing.templateUid) === norm(templateUid) && norm(existing.targetUid) === norm(targetUid);
                        if (isSame) {
                    ...
                    const fieldItemBaseClasses = ['FormItemModel', 'FormCustomItemModel', 'FormJSFieldItemModel']
                    ...
                    if (!isReferenceGrid) {
                        ...
                        mountTarget.flowEngine.forEachModel((m) => {
                            if (hasExistingFields)
                                return;
                            if (!(m instanceof FlowModel))
                                return;
                            if (m.uid === mountTarget.uid || m.uid === importer.uid)
                                return;
                            if (m.parent?.uid !== existingGrid.uid || m.subKey !== 'items')
                                return;
                            if (isFieldItem(m)) {
                                hasExistingFields = true;
                            }
                    ...
                    if (hasExistingFields) {
                        const viewer = ctx.viewer || mountTarget.context.viewer || importer.context.viewer;
                        ...
                        const confirmed = await new Promise((resolve) => {
                            ...
                            viewer.dialog({
                                title: ctx.t?.('Field template', { ns: [NAMESPACE, 'client'], nsMode: 'fallback' }) ||
                                    'Field template',
                                width: 520,
                                destroyOnClose: true,
                                content: (currentDialog) => (React.createElement(React.Fragment, null,
                                    React.createElement("div", { style: { marginBottom: 16 } }, message),
...
//# sourceMappingURL=SubModelTemplateImporterModel.js.map