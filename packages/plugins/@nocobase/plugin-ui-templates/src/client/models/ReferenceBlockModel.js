...
import { escapeT, FlowContext } from '@nocobase/flow-engine';
...
import { TEMPLATE_LIST_PAGE_SIZE, calcHasMore, getTemplateAvailabilityDisabledReason, normalizeStr, parseResourceListResponse, } from '../utils/templateCompatibility';
...
export class ReferenceBlockModel extends BlockModel {
    ...
    _resolvedTargetUid;
    _invalidTargetUid;
    _bridgeTargetContext(target, engine) {
        ...
        bridge.addDelegate(this.context);
    ...
    onInit(option) {
        ...
        const contextKeys = ['collection', 'dataSource', 'resource', 'association', 'resourceName'];
    ...
    _ensureScopedEngine() {
        ...
        ensureScopedEngineView(this._scopedEngine, this.context);
    ...
    async _resolveFinalTarget(uid) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            const model = await engine.loadModel({ uid: currentUid });
            if (!model)
                return null;
    ...
    async onDispatchEventStart(eventName) {
        if (eventName !== 'beforeRender')
            return;
        const stepParams = this.getStepParams?.('referenceSettings', 'target') || {};
        const targetUid = (stepParams?.targetUid || '').trim() || undefined;
        if (!targetUid) {
            this._syncExtraTitle(false);
            const oldTarget = this.subModels['target'];
            if (oldTarget) {
                this.emitter?.emit?.('onSubModelRemoved', oldTarget);
            ...
            this.subModels['target'] = undefined;
            return;
        }
        ...
        if (!target) {
            ...
            const latestStepParams = this.getStepParams?.('referenceSettings', 'target') || {};
        ...
        if (oldTarget?.uid !== target.uid) {
            ...
            this.subModels['target'] = target;
            this.emitter?.emit?.('onSubModelReplaced', { oldModel: oldTarget, newModel: target });
        }
    ...
    renderComponent() {
        const target = this.subModels?.['target'];
    ...
    render() {
        return React.createElement("div", { ref: this.context.ref }, this.renderComponent());
    }
}
    ...
    steps: {
        useTemplate: {
            ...
            uiSchema: (ctx) => {
                ...
                const api = ctx?.api;
                const resolveExpectedAssociationName = () => {
                    try {
                        const init = m.getStepParams?.('resourceSettings', 'init') || {};
                        const fromInit = normalizeStr(init?.associationName);
                        if (fromInit)
                            return fromInit;
                        const assocName = normalizeStr(m?.context?.association?.resourceName);
                        if (assocName)
                            return assocName;
                        const resourceCtx = m?.context?.resource;
                        if (resourceCtx) {
                            const fromResourceAssoc = typeof resourceCtx.getAssociationName === 'function'
                                ? normalizeStr(resourceCtx.getAssociationName())
                                : '';
                            if (fromResourceAssoc)
                                return fromResourceAssoc;
                            // resourceName 在关联资源场景通常形如 `users.profile`
                            const fromResourceName = typeof resourceCtx.getResourceName === 'function' ? normalizeStr(resourceCtx.getResourceName()) : '';
                            if (fromResourceName)
                                return fromResourceName;
                        }
                        const viewArgs = m?.context?.view?.inputArgs || {};
                ...
                const getTemplateDisabledReason = (tpl) => {
                    return getTemplateAvailabilityDisabledReason(ctx, tpl, { associationName: expectedAssociationName }, { checkResource: false, associationMatch: 'associationResourceOnly' });
                };
                const fetchOptions = async (keyword, pagination) => {
                    ...
                    try {
                        ...
                        const { rows, count } = parseResourceListResponse(res);
            ...
            async beforeParamsSave(ctx, params) {
                ...
                if (mode === 'copy' && targetUid) {
                    const flow = ctx.model.constructor.globalFlowRegistry.getFlow('referenceSettings');
                    const targetStepDef = flow?.steps?.target;
            ...
            async handler(ctx, params) {
                ...
                const useTemplateParams = ctx.model.getStepParams('referenceSettings', 'useTemplate') || {};
                ctx.model.setStepParams('referenceSettings', 'useTemplate', {
                    ...useTemplateParams,
                    templateUid,
                    templateName,
                    templateDescription,
                ...
                if (targetUid) {
                    const targetParams = ctx.model.getStepParams('referenceSettings', 'target') || {};
                    ctx.model.setStepParams('referenceSettings', 'target', {
                        ...targetParams,
                        targetUid,
                        mode,
                    });
                }
                const resourceInit = ctx.model.getStepParams('resourceSettings', 'init') || {};
                ...
                if (dataSourceKey || collectionName || associationName || filterByTk || sourceId) {
                    ctx.model.setStepParams('resourceSettings', 'init', {
                        ...resourceInit,
                        dataSourceKey,
                        collectionName,
                        associationName,
        ...
        target: {
            ...
            async beforeParamsSave(ctx, params) {
                ...
                const subKey = oldModel.subKey;
                const subType = oldModel.subType;
                // 若没有父模型，直接退出（无处安放新实例）
                if (!parent || !subKey) {
                    ctx.exit();
                    return;
                ...
                let insertIndex = -1;
                if (subType === 'array') {
                    const arr = (parent.subModels[subKey] || []);
                ...
                const newModel = engine.createModel(newOptions);
                newModel.setParent(parent);
                newModel.sortIndex = insertIndex >= 0 ? insertIndex : 0;
                const isPresetOrNew = !!oldModel.isNew;
                ...
                if (isPresetOrNew) {
                    ...
                    newModel.isNew = true;
                    parent.emitter?.emit?.('onSubModelAdded', newModel);
...
//# sourceMappingURL=ReferenceBlockModel.js.map