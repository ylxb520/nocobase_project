...
describe('openViewActionExtensions (popup template)', () => {
    it('resolves popupTemplateUid to uid and delegates to base', async () => {
        ...
        const baseHandler = vi.fn(async (_ctx, params) => params);
        const baseOpenView = {
            name: 'openView',
            title: 'openView',
            uiSchema: {
        ...
        ctx.api = api;
        ctx.t = (k) => k;
        const params = { popupTemplateUid: 'tpl-1', uid: 'old' };
    ...
    it('clears filterByTk/sourceId when template does not define them (avoid leaking record defaults)', async () => {
        ...
        const model = {
            getStepParams: vi.fn((flowKey, stepKey) => {
        ...
        const ctx = { engine, api, model, t: (k) => k };
        const params = {
        ...
        expect(baseBefore).toHaveBeenCalledTimes(1);
        const forwarded = baseBefore.mock.calls[0][1];
    ...
    it('treats collection-scene popup template as no filterByTk even if template stores ctx.record expr', async () => {
        ...
        class CollectionOnlyModel {
            static _isScene(scene) {
                return scene === 'collection';
            }
        }
        engine.registerModels({ AddNewActionModel: CollectionOnlyModel });
        const baseBefore = vi.fn(async () => { });
        const baseOpenView = {
            name: 'openView',
            title: 'openView',
    ...
    it('runtime overrides resource keys via shadow ctx (non-relation template in relation trigger)', async () => {
        ...
        let capturedCtx;
        let capturedParams;
        const baseHandler = vi.fn(async (ctxArg, paramsArg) => {
            capturedCtx = ctxArg;
    ...
    it('infers target filterByTk from belongsTo record when reusing target collection template in relation field', async () => {
        const engine = new FlowEngine();
        let capturedCtx;
        const baseHandler = vi.fn(async (ctxArg) => {
            capturedCtx = ctxArg;
            return undefined;
        });
        const baseOpenView = {
    ...
    it('infers record-scene needs filterByTk when template row misses filterByTk', async () => {
        ...
        engine.registerModels({ EditActionModel: RecordOnlyModel });
        let capturedCtx;
        const baseHandler = vi.fn(async (ctxArg) => {
            capturedCtx = ctxArg;
            return undefined;
    ...
    it('allows non-relation popup template when target collection matches (relation field)', async () => {
        ...
        const dataSourceManager = {
            getCollection: vi.fn((_ds, name) => {
                if (name !== 'users')
                    return undefined;
                return {
                    getFieldByPath: vi.fn((path) => {
                        if (path !== 'roles')
                            return undefined;
        ...
        const ctx = { engine, api, model, dataSourceManager, t: (k) => k };
        const params = { popupTemplateUid: 'tpl-1', uid: 'old', associationName: 'users.roles' };
    ...
    it('rejects record-scene popup template when association context cannot provide filterByTk (collection scene)', async () => {
        ...
        engine.registerModels({ CollectionOnlyModel });
        const baseBefore = vi.fn(async () => { });
        const baseOpenView = {
            name: 'openView',
            title: 'openView',
    ...
    it('allows record-scene popup template in association context when current action is record scene', async () => {
        ...
        engine.registerModels({ RecordOnlyModel });
        const baseBefore = vi.fn(async () => { });
        const baseOpenView = {
            name: 'openView',
            title: 'openView',
...
//# sourceMappingURL=openViewActionExtensions.test.js.map