...
export function parseResourceListResponse(res) {
    let cur = res;
    ...
    for (let i = 0; i < 4; i++) {
        if (Array.isArray(cur))
            break;
        if (cur && typeof cur === 'object' && Array.isArray(cur.rows))
            break;
        if (cur && typeof cur === 'object' && 'data' in cur) {
            cur = cur.data;
            continue;
    ...
    const count = toFiniteNumber(cur?.count ?? cur?.total ?? cur?.totalCount);
    return { rows, count };
}
export function calcHasMore(args) {
...
export function resolveTargetResourceByAssociation(ctx, init, options) {
    ...
    if (!dataSourceKey || !associationName)
        return undefined;
    ...
    if (!baseCollectionName || !fieldPath)
        return undefined;
    ...
    if (!targetDataSourceKey || !targetCollectionName)
        return undefined;
...
export function resolveExpectedResourceInfoByModelChain(ctx, startModel, options) {
    ...
    while (cur && depth < maxDepth) {
        ...
        try {
            const c = cur?.collection || (fallbackCollectionFromCtx ? ctx?.collection : undefined);
 ...
 */
export function resolveActionScene(getModelClass, useModel) {
    const useKey = normalizeStr(useModel);
    if (!useKey || !getModelClass)
        return undefined;
...
const includesRecordVar = (expr) => expr.includes('ctx.record');
const includesResourceVar = (expr) => expr.includes('ctx.resource');
/**
 * 推断弹窗模板的 filterByTk/sourceId 上下文需求
 */
export function inferPopupTemplateContextFlags(scene, filterByTkExpr, sourceIdExpr) {
...
export function getTemplateAvailabilityDisabledReason(ctx, tpl, expected, options) {
...
//# sourceMappingURL=templateCompatibility.js.map