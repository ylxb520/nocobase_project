...
export const defaultSelectOptionComparator = (a, b) => {
...
export function mergeSelectOptions(prev, next, options) {
    const getKey = options?.getKey || ((item) => String(item?.value || ''));
...
export function bindInfiniteScrollToFormilySelect(field, fetchPage, options) {
    ...
    const resetAndLoad = async (keyword) => {
    ...
    field.componentProps.onDropdownVisibleChange = async (open) => {
        if (!open)
            return;
        await resetAndLoad('');
    };
    const debouncedSearch = debounce(async (kw) => resetAndLoad(kw), debounceMs);
    field.componentProps.onSearch = (value) => {
        if (isComposing())
            return;
    ...
    field.componentProps.onCompositionEnd = (e) => {
        setComposing(false);
        const keyword = typeof e?.target?.value === 'string' ? e.target.value.trim() : '';
        debouncedSearch(keyword);
    };
    field.componentProps.onPopupScroll = (e) => {
...
//# sourceMappingURL=infiniteSelect.js.map