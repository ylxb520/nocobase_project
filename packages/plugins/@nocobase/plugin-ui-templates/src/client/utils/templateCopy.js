 ...
 */
export function patchGridOptionsFromTemplateRoot(templateRoot, gridOptions) {
    if (!templateRoot || !isPlainObject(gridOptions)) {
        return { options: gridOptions, patched: false };
    }
    const baseStepParams = isPlainObject(gridOptions.stepParams) ? gridOptions.stepParams : {};
    const nextStepParams = { ...baseStepParams };
    let patched = false;
    for (const { flowKey, stepKey } of DELEGATED_STEP_PARAMS) {
        const val = templateRoot.getStepParams?.(flowKey, stepKey);
        if (typeof val === 'undefined')
            continue;
        const flow = isPlainObject(nextStepParams[flowKey]) ? { ...nextStepParams[flowKey] } : {};
...
//# sourceMappingURL=templateCopy.js.map