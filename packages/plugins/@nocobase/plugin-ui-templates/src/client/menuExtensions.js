...
import { extractPopupTemplateContextFlagsFromParams, inferPopupTemplateContextFlags, normalizeStr, resolveActionScene, } from './utils/templateCompatibility';
import { patchGridOptionsFromTemplateRoot } from './utils/templateCopy';
const unwrapData = (val) => val?.data?.data ?? val?.data ?? val;
const normalizeTitle = (val) => {
    if (!val)
        return '';
    return String(val).replace(/\s+/g, ' ').trim();
};
const openConvertDialogs = new WeakSet();
const openCopyDialogs = new WeakSet();
const openFieldsCopyDialogs = new WeakSet();
const openSavePopupTemplateDialogs = new Set();
const openConvertPopupTemplateDialogs = new Set();
const GRID_REF_FLOW_KEY = 'referenceSettings';
const GRID_REF_STEP_KEY = 'useTemplate';
function getReferenceFormGridSettings(block) {
    const grid = block.subModels?.grid;
    if (!grid)
        return null;
    if (grid.use !== 'ReferenceFormGridModel')
        return null;
...
async function handleConvertToTemplate(model, _t) {
    ...
    const TemplateDialogContent = ({ currentDialog }) => {
        ...
        const handleSubmit = async () => {
            ...
            if (!isConvertMode) {
                ...
                const duplicatedUid = (dupBody && typeof dupBody === 'object' && (dupBody.uid || dupBody.data?.uid)) ||
            ...
            if (isConvertMode) {
                ...
                if (!parent || !subKey || !engine) {
                    ...
                    const newModel = engine.createModel({
                    ...
                    if (subType === 'array') {
                        let arr = (parent.subModels[subKey] || []);
                        if (!Array.isArray(arr)) {
                            parent.subModels[subKey] = [];
                            arr = parent.subModels[subKey];
                        }
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(Form, { form: form, layout: "vertical", initialValues: {
                    name: normalizeTitle(model.title),
                    description: '',
                    saveMode: 'convert',
                } },
                React.createElement(Form.Item, { name: "name", label: React.createElement(Typography.Text, { strong: true }, t('Template name')), rules: [{ required: true, message: t('Template name is required') }] },
                    React.createElement(Input, null)),
                React.createElement(Form.Item, { name: "description", label: React.createElement(Typography.Text, { strong: true }, t('Template description')) },
                    React.createElement(Input.TextArea, { rows: 3 })),
                React.createElement(Form.Item, { name: "saveMode", label: React.createElement(Typography.Text, { strong: true }, t('Save mode')) },
                    React.createElement(Radio.Group, null,
                        React.createElement(Space, { direction: "vertical" },
                            React.createElement(Radio, { value: "convert" },
                                t('Convert current block to template'),
                                React.createElement(Tooltip, { title: t('Convert current block to template description') },
                                    React.createElement(QuestionCircleOutlined, { style: { marginLeft: 4, color: '#999' } }))),
                            React.createElement(Radio, { value: "duplicate" },
                                t('Duplicate current block as template'),
                                React.createElement(Tooltip, { title: t('Duplicate current block as template description') },
                                    React.createElement(QuestionCircleOutlined, { style: { marginLeft: 4, color: '#999' } }))))))),
            React.createElement(currentDialog.Footer, null,
                React.createElement(Space, { align: "end" },
                    React.createElement(Button, { onClick: handleCancel }, t('Cancel')),
                    React.createElement(Button, { type: "primary", loading: submitting, disabled: submitting, onClick: handleConfirm }, t('Confirm'))))));
    };
    const dialog = viewer.dialog({
        title: t('Save as template'),
        width: 600,
        destroyOnClose: true,
        content: (currentDialog) => React.createElement(TemplateDialogContent, { currentDialog: currentDialog }),
        onClose: release,
    });
    return dialog;
}
async function handleConvertToCopy(model, _t) {
    const t = getPluginT(model);
    const flow = model.constructor.globalFlowRegistry.getFlow('referenceSettings');
    const stepDef = flow?.steps?.target;
    ...
    Modal.confirm({
        ...
        onOk: async () => {
            try {
                ...
                await stepDef.beforeParamsSave({ engine: model.flowEngine, model, exit: () => { } }, params);
            }
            catch (e) {
                console.error(e);
...
async function handleConvertFieldsToCopy(model, _t) {
    const t = getPluginT(model);
    ...
    const viewer = model.context.viewer;
    if (openFieldsCopyDialogs.has(model))
        return;
    ...
    const doConvert = async () => {
        try {
            ...
            const existingGridUids = [];
            model.flowEngine.forEachModel((m) => {
                if (!m || m.uid === model.uid)
                    return;
            ...
            const root = await scoped.loadModel({ uid: targetUid });
            const gridModel = _.get(root, targetPath);
            if (!gridModel) {
            ...
            const newGrid = model.flowEngine.createModel({
                ...merged.options,
                parentId: model.uid,
                subKey: 'grid',
                subType: 'object',
            ...
            const clearTemplateTitle = (m) => {
                ...
                if (!reg.test(curTitle))
                    return;
                const nextTitle = curTitle.replace(reg, '').trim();
                m.setTitle(nextTitle || '');
            };
            const masterModel = model?.isFork ? model.master || model : model;
            clearTemplateTitle(model);
            clearTemplateTitle(masterModel);
            masterModel?.forks?.forEach?.((f) => f instanceof FlowModel && clearTemplateTitle(f));
            // 删除旧的 grid（若存在）
            ...
            console.error(e);
            model.context.message?.error?.(e instanceof Error ? e.message : t('Convert reference to duplicate is unavailable'));
        }
        finally {
            release();
        }
...
async function handleSavePopupAsTemplate(model, _t) {
    const api = model.context.api;
    const viewer = model.context.viewer;
    const pluginT = getPluginT(model);
    const tNs = (key, opt) => {
        const tt = model.context?.t;
        if (typeof tt !== 'function')
            return pluginT(key, opt);
        return tt(key, { ns: [NAMESPACE, 'client'], nsMode: 'fallback', ...(opt || {}) });
    };
    const tClient = (key, opt) => {
        const tt = model.context?.t;
        if (typeof tt !== 'function')
            return pluginT(key, opt);
    ...
    const defaultName = fieldTitle ||
        normalizeTitle(model?.getTitle?.()) ||
        normalizeTitle(model?.title) ||
        normalizeTitle(model.uid);
    const resolveOpenViewStepKey = (flow) => {
        const steps = flow?.steps || {};
        if (steps?.openView)
            return 'openView';
        const found = Object.entries(steps).find(([, def]) => def?.use === 'openView');
        return found?.[0];
    };
    const openViewStepKey = (model.getStepParams('popupSettings', 'openView') !== undefined && 'openView') ||
        resolveOpenViewStepKey(popupFlow) ||
        'openView';
    const openViewParams = model.getStepParams('popupSettings', openViewStepKey) || {};
    const toNonEmptyString = (val) => {
        if (val === undefined || val === null)
            return undefined;
    ...
    const resolveDefaultOpenViewResource = () => {
        ...
        const isAssociationField = (f) => !!f?.isAssociationField?.();
    ...
    const templateDataSourceKey = defaultOpenViewResource.dataSourceKey || toNonEmptyString(openViewParams?.dataSourceKey);
    const templateCollectionName = defaultOpenViewResource.collectionName || toNonEmptyString(openViewParams?.collectionName);
    const templateAssociationName = defaultOpenViewResource.associationName;
    const getDefaultFilterByTkExpr = () => {
    ...
    const duplicatePopupTarget = async () => {
            ...
            const TemplateDialogContent = () => {
                ...
                const handleSubmit = async () => {
                    ...
                    try {
                        ...
                        if (values?.saveMode === 'convert' && tplUid) {
                            ...
                            const nextOpenView = { ...(openViewParams || {}) };
                            ...
                            const scene = resolveActionScene((use) => model.flowEngine?.getModelClass?.(use), ctor?.name);
                            const inferred = inferPopupTemplateContextFlags(scene, openViewParams?.filterByTk, openViewParams?.sourceId);
                ...
                return (React.createElement(React.Fragment, null,
                    React.createElement(Form, { form: form, layout: "vertical", initialValues: {
                        ...
                        React.createElement(Form.Item, { name: "name", label: tNs('Template name'), rules: [{ required: true, message: tNs('Template name is required') }] },
                            React.createElement(Input, { autoFocus: true })),
                        React.createElement(Form.Item, { name: "description", label: tNs('Template description') },
                            React.createElement(Input.TextArea, { rows: 3 })),
                        React.createElement(Form.Item, { name: "saveMode", label: React.createElement(Typography.Text, { strong: true }, tNs('Save mode')) },
                            React.createElement(Radio.Group, null,
                                React.createElement(Space, { direction: "vertical" },
                                    React.createElement(Radio, { value: "convert" },
                                        tNs('Convert current popup to template'),
                                        React.createElement(Tooltip, { title: tNs('Convert current popup to template description') },
                                            React.createElement(QuestionCircleOutlined, { style: { marginLeft: 4, color: '#999' } }))),
                                    React.createElement(Radio, { value: "duplicate" },
                                        tNs('Duplicate current popup as template'),
                                        React.createElement(Tooltip, { title: tNs('Duplicate current popup as template description') },
                                            React.createElement(QuestionCircleOutlined, { style: { marginLeft: 4, color: '#999' } }))))))),
                    React.createElement(currentDialog.Footer, null,
                        React.createElement(Space, { align: "end" },
                            ...
                            React.createElement(Button, { type: "primary", loading: submitting, onClick: handleSubmit }, tClient('Confirm'))))));
            };
            return React.createElement(TemplateDialogContent, null);
        },
        onClose: release,
    });
}
async function handleConvertPopupTemplateToCopy(model, _t) {
    const api = model.context.api;
    const viewer = model.context.viewer;
    const pluginT = getPluginT(model);
    const tNs = (key, opt) => {
    ...
    const openViewStepKey = (model.getStepParams('popupSettings', 'openView') !== undefined && 'openView') ||
        ...
        'openView';
    const openViewParams = model.getStepParams('popupSettings', openViewStepKey) || {};
    const templateUid = typeof openViewParams?.popupTemplateUid === 'string' ? openViewParams.popupTemplateUid.trim() : '';
    if (!templateUid) {
        model.context.message?.error?.(tNs('This pop-up is not using a popup template'));
        release();
        return;
    }
    const unwrap = (val) => val?.data?.data ?? val?.data ?? val;
    const fetchTemplateRow = async () => {
        if (!api?.resource) {
            return null;
        }
    ...
    const inferFromTemplateRow = (tplRow) => {
        const scene = resolveActionScene((use) => model.flowEngine?.getModelClass?.(use), tplRow?.useModel);
    ...
    const doConvert = async () => {
        ...
        delete nextOpenView.popupTemplateUid;
 ...
 */
function isReferenceTarget(model) {
...
export function registerMenuExtensions() {
    BlockModel.registerExtraMenuItems({
        ...
        sort: -10,
        matcher: (model) => {
            return (!isReferenceTarget(model) &&
    ...
    FlowModel.registerExtraMenuItems({
        ...
        items: async (model, t) => {
            ...
            const openViewStepKey = (model.getStepParams('popupSettings', 'openView') !== undefined && 'openView') ||
                ...
                'openView';
            const openViewParams = model.getStepParams('popupSettings', openViewStepKey) || {};
            const templateUid = typeof openViewParams?.popupTemplateUid === 'string'
                ? openViewParams.popupTemplateUid.trim()
                : '';
            const hasTemplate = !!templateUid;
            const disablePopupTemplateMenu = !!openViewParams?.disablePopupTemplateMenu;
            const pluginT = getPluginT(model);
            if (hasTemplate) {
                return [
                    {
