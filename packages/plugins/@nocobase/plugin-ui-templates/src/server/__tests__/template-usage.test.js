...
describe('ui templates and usages', () => {
    ...
    let flowRepo;
    let usageRepo;
    const buildOptions = (templateUid, useTemplateExtra = {}) => {
        return {
    ...
    const createReferenceBlock = async (uid, templateUid) => {
    ...
    beforeEach(async () => {
        ...
        flowRepo = app.db.getRepository('flowModels');
    ...
    it('should maintain usages via flowModels:save/destroy actions', async () => {
        ...
        const row = rows.find((r) => r?.uid === 'tpl-save');
...
//# sourceMappingURL=template-usage.test.js.map