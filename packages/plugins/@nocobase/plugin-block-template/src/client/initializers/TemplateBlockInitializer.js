 ...
 */
import { SchemaInitializerItem, usePlugin, useSchemaInitializer } from '@nocobase/client';
...
export function convertTplBlock(tpl, virtual = false, isRoot = true, newRootId, templateKey, options) {
  if (!newRootId) {
    newRootId = uid();
  }
...
export function formSchemaPatch(currentSchema, options) {
...
function detailsSchemaPatch(currentSchema, options) {
...
function schemaPatch(currentSchema, options) {
...
export function convertTemplateToBlock(data, templateKey, options) {
...
export const TemplateBlockInitializer = () => {
      ...
      : [
          {
            ...
            label: React.createElement(
              'div',
              { onClick: (e) => e.stopPropagation() },
              React.createElement(Empty, { image: Empty.PRESENTED_IMAGE_SIMPLE, description: t('No data') }),
            ),
          },
        ]),
  ];
  ...
  if (loading) {
    return React.createElement('div', null, React.createElement(LoadingOutlined, null), ' ', t('Templates'));
  }
  return React.createElement(SchemaInitializerItem, {
    closeInitializerMenuWhenClick: true,
    title: '{{t("Block template")}}',
    icon: React.createElement(CopyOutlined, { style: { marginRight: 0 } }),
    items: menuItems,
    name: 'templates',
    onClick: onClick,
  });
};
//# sourceMappingURL=TemplateBlockInitializer.js.map
