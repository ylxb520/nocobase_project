...
export class PluginBlockTemplateClient extends Plugin {
  ...
  async load() {
    ...
    // add mobile router
    this.app.pluginManager.get('mobile')?.mobileRouter?.add('mobile.schema.blockTemplate', {
      path: `/ui-templates/inherited-v1/:key/:pageSchemaUid`,
      Component: BlockTemplateMobilePage,
    });
  }
  isInBlockTemplateConfigPage() {
    const mobilePath = this.app.pluginManager.get('mobile')?.mobileBasename + '/ui-templates/inherited-v1';
  ...
  setTemplateCache = (schema) => {
    if (!schema) {
      return;
    }
    this.templateschemacache[schema['x-uid']] = schema;
  };
  clearTemplateCache = (templateRootUid) => {
