...
export function findSchemaByUid(schema, uid) {
  if (!schema) {
    return null;
  }
  if (schema['x-uid'] === uid) {
...
export function collectSchemaFirstVirtualUids(schema) {
  const uids = new Set();
  const collectUids = (schema) => {
    if (!schema) {
      return;
    }
    if (schema['x-virtual']) {
 ...
 */
export function findSchemaCache(cache, uid) {
 ...
 */
export function findFirstVirtualSchema(schema, uid, wrap) {
  // Helper function to find next non-virtual node
  const findNextNonVirtualNode = (properties, startIndex) => {
 ...
 */
export function convertToCreateSchema(schema, skipUids = []) {
...
export function syncExtraTemplateInfo(schema, templateInfos, savedSchemaUids) {
 ...
 */
export function getFullSchema(schema, templateschemacache, templateInfos, savedSchemaUids = new Set()) {
 ...
 */
function mergeSchema(target, source, rootId, templateschemacache, templateInfos, templateKey) {
 ...
 */
export function setToTrueSchema(schema) {
  if (schema['x-virtual']) {
    delete schema['x-virtual'];
  }
  if (schema.properties) {
