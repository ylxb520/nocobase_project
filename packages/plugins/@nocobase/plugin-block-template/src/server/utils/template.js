...
export async function getNewFullBlock(db, transaction, blockUid) {
...
export function mergeSchema(template, schema, rootTemplate) {
  ...
  return _.mergeWith(template, schema, (objectValue, sourceValue, keyName, object, source) => {
    if (sourceValue == null) {
      return objectValue;
    }
...
export function cleanSchema(schema, templateId) {
  const properties = schema?.properties || {};
