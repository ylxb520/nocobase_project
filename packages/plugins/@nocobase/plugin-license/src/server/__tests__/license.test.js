...
describe('env match', () => {
    ...
    it('new env match old key', async () => {
        const result = isEnvMatch(envWithDbId, licenseKey);
        expect(result).toEqual(true);
    });
    it('new env match old key with different Version', async () => {
        const result = isEnvMatch(envWithNewVersion, licenseKey);
        expect(result).toEqual(false);
    });
    it('new env match new key', async () => {
        const result = isEnvMatch(envWithNewVersion, licenseKeyWithId);
        expect(result).toEqual(true);
    });
});
const makeEnv = (env) => env;
const makeKeyData = (instanceData) => ({
    instanceData,
});
describe('isEnvMatch', () => {
    // ========== 1. db.id 存在时，只比较 id ==========
    ...
    test('fails when env is null', () => {
        expect(isEnvMatch(null, makeKeyData({ db: {} }))).toBe(false);
    });
    test('fails when keyData.instanceData is null', () => {
        expect(isEnvMatch(makeEnv({ db: {} }), null)).toBe(false);
    });
    test('fails when both missing db', () => {
        const env = makeEnv({ sys: 'linux', osVer: 'ubuntu' });
        const keyData = makeKeyData({ sys: 'linux', osVer: 'ubuntu' });
