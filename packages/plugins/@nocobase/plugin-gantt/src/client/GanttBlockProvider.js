...
import { useACLRoleContext, useCollection_deprecated, useBlockRequestContext, TableBlockProvider, useTableBlockContext, getLabelFormatValue, useCollection, useCollectionManager_deprecated, } from '@nocobase/client';
export const GanttBlockContext = createContext({});
GanttBlockContext.displayName = 'GanttBlockContext';
const formatData = (data = [], fieldNames, tasks = [], projectId = undefined, hideChildren = false, checkPermassion, primaryKey, labelUiSchema) => {
    data.forEach((item) => {
        ...
        if (item.children && item.children.length) {
            ...
            formatData(item.children, fieldNames, tasks, item.id + '', hideChildren, checkPermassion, primaryKey, labelUiSchema);
        }
        else {
            tasks.push({
...
const InternalGanttBlockProvider = (props) => {
    ...
    //   return <Spin />;
    // }
    return (React.createElement(GanttBlockContext.Provider, { value: {
            field,
            service,
            resource,
            fieldNames,
...
export const GanttBlockProvider = (props) => {
    ...
    return (React.createElement("div", { "aria-label": "block-item-gantt", role: "button" },
        React.createElement(TableBlockProvider, { ...props, params: params },
            React.createElement(InternalGanttBlockProvider, { ...props }))));
};
...
export const useGanttBlockProps = () => {
    ...
    const onExpanderClick = (task) => {
        ...
        const tasksData = data.map((t) => (t.id === task.id ? task : t));
        setTasks(tasksData);
        ctx.field.data = tasksData;
    };
    const expandAndCollapseAll = (flag) => {
        const data = formatData(ctx.service.data?.data, ctx.fieldNames, [], undefined, flag, checkPermission, primaryKey, labelUiSchema);
        setTasks(data);
        ctx.field.data = data;
    };
    useEffect(() => {
        ...
        if (!ctx?.service?.loading) {
            const data = formatData(ctx.service.data?.data, ctx.fieldNames, [], undefined, false, checkPermission, primaryKey, labelUiSchema);
            setTasks(data);
            setLoading(false);
            ctx.field.data = data;
            if (tasks.length > 0) {
...
//# sourceMappingURL=GanttBlockProvider.js.map