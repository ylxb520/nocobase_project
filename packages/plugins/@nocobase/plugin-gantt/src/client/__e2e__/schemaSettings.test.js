...
test.describe('configure params in gantt block', () => {
    ...
    test('set start date field ', async ({ page, mockPage, mockRecord }) => {
        ...
        await expect(page.getByLabel('nb-gantt-tooltip')).toHaveText(new RegExp(String(getYmd(new Date(mockData['startDatetime2'])))));
    });
    test('set end date field ', async ({ page, mockPage, mockRecord }) => {
        ...
        await page.getByRole('button', { name: 'Actions', exact: true }).click();
        await page.locator('.bar').hover({ position: { x: 20, y: 10 } });
        await expect(page.getByLabel('nb-gantt-tooltip')).toHaveText(new RegExp(String(getYmd(new Date(mockData['endDatetime2'])))));
    });
