...
import { NocoBaseRecursionField, PopupContextProvider, RecordProvider, useAPIClient, useBlockRequestContext, useCollectionParentRecordData, useCurrentAppInfo, useDesignable, usePopupUtils, useProps, useTableBlockContext, useToken, withDynamicSchemaProps, } from '@nocobase/client';
...
const debounceHandleTaskChange = debounce(async (task, resource, fieldNames, service, t) => {
...
const debounceHandleProcessChange = debounce(async (task, resource, fieldNames, service, t) => {
    await resource.update({
        filterByTk: task.id,
        values: {
...
export const Gantt = withDynamicSchemaProps((props) => {
    const { styles } = useStyles();
    const { token } = useToken();
    const api = useAPIClient();
    const currentTheme = JSON.parse(api.auth.getOption('theme'))?.uid;
    const { listCellWidth = '155px', ganttHeight = 0, preStepsCount = 1, barFill = 60, barCornerRadius = token.borderRadiusXS, barProgressColor = token.colorPrimary, barProgressSelectedColor = token.colorPrimary, barBackgroundColor = token.colorPrimary, barBackgroundSelectedColor = token.colorPrimary, projectProgressColor = token.colorPrimary, projectProgressSelectedColor = token.colorPrimary, projectBackgroundColor = token.colorPrimary, projectBackgroundSelectedColor = token.colorPrimary, milestoneBackgroundColor = '#f1c453', milestoneBackgroundSelectedColor = '#f29e4c', rtl = false, handleWidth = 8, timeStep = 300000, arrowColor = 'grey', fontFamily = `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol', 'Noto Color Emoji'`, fontSize = token.fontSize, arrowIndent = 20, todayColor = 'rgba(252, 248, 227, 0.5)', viewDate, TooltipContent = StandardTooltipContent, onDoubleClick, onDelete, onSelect, onExpanderClick, tasks, expandAndCollapseAll, fieldNames, loading, } = useProps(props); // 新版 UISchema（1.0 之后）中已经废弃了 useProps，这里之所以继续保留是为了兼容旧版的 UISchema
    ...
    const columnWidth = getColumnWidth(dateSetup.dates.length, verticalGanttContainerRef.current?.clientWidth);
    ...
    const handleTableExpanderClick = (expanded, record) => {
        const task = ctx?.field?.data.find((v) => v.id === record.id + '');
      ...
      `), ref: ganttRef },
        React.createElement(PopupContextProvider, { visible: visible, setVisible: setVisible },
            React.createElement(RecordProvider, { record: record, parent: parentRecordData },
                React.createElement(GanttRecordViewer, null)),
            React.createElement(NocoBaseRecursionField, { name: 'anctionBar', schema: fieldSchema.properties.toolBar }),
            React.createElement(NocoBaseRecursionField, { name: 'table', schema: fieldSchema.properties.table }),
            React.createElement("div", { className: styles.wrapper, onKeyDown: handleKeyDown, tabIndex: 0, ref: wrapperRef },
                React.createElement(TaskGantt, { gridProps: gridProps, calendarProps: calendarProps, barProps: barProps, ganttHeight: ganttHeight, scrollY: scrollY, scrollX: scrollX, ref: verticalGanttContainerRef }),
                ganttEvent.changedTask && (React.createElement(Tooltip, { arrowIndent: arrowIndent, rowHeight: rowHeight, svgContainerHeight: svgContainerHeight, svgContainerWidth: svgContainerWidth, fontFamily: fontFamily, fontSize: fontSize, scrollX: scrollX, scrollY: scrollY, task: ganttEvent.changedTask, headerHeight: headerHeight, taskListWidth: taskListWidth, TooltipContent: TooltipContent, rtl: rtl, svgWidth: svgWidth })),
                React.createElement(VerticalScroll, { ganttFullHeight: ganttFullHeight, ganttHeight: ganttHeight, headerHeight: headerHeight, scroll: scrollY, onScroll: handleScrollY, rtl: rtl }),
                React.createElement(Spin, { spinning: loading, style: { visibility: 'hidden' } },
                    React.createElement(HorizontalScroll, { svgWidth: svgWidth, taskListWidth: taskListWidth, scroll: scrollX, rtl: rtl, onScroll: handleScrollX }))))));
});
