...
import { EXECUTION_STATUS, Trigger, } from '@nocobase/plugin-workflow';
...
export default class CustomActionTrigger extends Trigger {
    ...
    async globalTriggerAction(context, next) {
        ...
        for (const key of triggerWorkflowsArray) {
            ...
            if (workflow &&
                workflow.type === this.constructor.TYPE &&
    ...
    triggerAction = async (context, next) => {
        const { resourceName, params: { filterByTk, values, triggerWorkflows = '', associatedIndex }, } = context.action;
        if (!triggerWorkflows) {
        ...
        for (const workflow of workflows) {
            ...
            if (filterByTk != null) {
                if (Array.isArray(filterByTk)) {
                    data = (await repository.find({ filterByTk, appends, context })).map((item) => Object.assign(item.toJSON(), formData));
                }
                else {
    ...
    constructor(workflow) {
        ...
        workflow.app.pm.get(PluginErrorHandler).errorHandler.register((err) => err.name === 'CustomActionInterceptionError', async (err, ctx) => {
            ctx.body = {
                errors: err.messages,
            };
    ...
    async processEvents({ events: [syncGroup = [], asyncGroup = []], context, next, }) {
        ...
        if (type === CONTEXT_TYPE.MULTIPLE_RECORDS) {
            ...
            if (data && typeof data === 'object') {
                filterByTk = collection.isMultiFilterTargetKey()
                    ? pick(data, collection.filterTargetKey.sort((a, b) => a.localeCompare(b)))
