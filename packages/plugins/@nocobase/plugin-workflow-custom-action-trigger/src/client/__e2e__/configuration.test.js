 ...
 */
import { CustomActionEventTriggerNode, WorkflowListRecords, apiCreateWorkflow, apiDeleteWorkflow, apiGetDataSourceCount, apiGetWorkflow, apiTriggerCustomActionEvent, apiUpdateWorkflowTrigger, appendJsonCollectionName, generalWithNoRelationalFields, } from '@nocobase/plugin-workflow-test/e2e';
...
test.describe('Upper Left Path Jump', () => {
    ...
    test('Form event Workflow History Version Configuration Page Path Jump Workflow Management Page', async ({ page, mockPage, mockCollections, mockRecords, }) => {
        ...
        const triggerNodeCollectionRecords = await apiTriggerCustomActionEvent(triggerNodeCollectionName, triggerWorkflows, { orgname: triggerNodeCollectionFieldData });
...
test.describe('copy to new version', () => {
    test('Copy the Form event of the Configuration Trigger node', async ({ page, mockCollections, mockRecords }) => {
        ...
        const customActionEventTriggerNode = new CustomActionEventTriggerNode(page, workFlowName, triggerNodeCollectionName);
        ...
        await expect(page
            ...
            .getByText(`Main / ${triggerNodeCollectionDisplayName}`)).toBeVisible();
        // 4、后置处理：删除工作流
        await apiDeleteWorkflow(workflowId);
    });
});
