...
describe('actions', () => {
  ...
  it('should set scope with user associations', async () => {
    ...
    // list with user
    const userAgent = (await app.agent().login(testUser)).set('x-role', 'testRole');
  ...
  it('can destroy users role', async () => {
    ...
    const rolesCheckResponse = await loggedAgent.set('Accept', 'application/json').get('/roles:check');
    ...
    expect(response.statusCode).toEqual(200);
    const rolesCheckResponse2 = await loggedAgent.set('Accept', 'application/json').get('/roles:check');
