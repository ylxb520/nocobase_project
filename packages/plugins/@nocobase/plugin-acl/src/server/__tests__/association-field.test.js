...
describe.skip('association field acl', () => {
  ...
  it('should revoke association action on action revoke', async () => {
    ...
    const actionId = viewAction.get('id');
  ...
  it('should revoke association action on field deleted', async () => {
    ...
    const action = await db.getRepository('dataSourcesRolesResources.actions', roleResource.get('id')).findOne({
      filter: {
        name: 'create',
      },
    });
    ...
    // remove field
    await db.getRepository('collections.fields', 'users').destroy({
      filter: {
        name: 'name',
      },
      context: {},
...
//# sourceMappingURL=association-field.test.js.map
