...
export const PluginPermissions = ({ active }) => {
  const app = useApp();
  ...
  const [snippets, setSnippets] = useState(role?.snippets || []);
  ...
  const allChecked = useMemo(() => snippets.includes('pm.*') && snippets.every((item) => !item.startsWith('!pm.')), [
    snippets,
  ]);
    ...
    columns: [
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          return React.createElement(Checkbox, { checked: checked, onChange: () => handleChange(checked, record) });
        },
      },
    ],
    dataSource: settings
