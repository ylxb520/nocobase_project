...
`;
const translateTitle = (menus, t, compile) => {
...
const DesktopRoutesContext = createContext({ routeList: [] });
const useDesktopRoutes = () => {
  return useContext(DesktopRoutesContext);
};
const DesktopRoutesProvider = ({ children, refreshRef }) => {
  const api = useAPIClient();
  const resource = useMemo(() => api.resource('desktopRoutes'), [api]);
  const { data, runAsync: refresh } = useRequest(
    () =>
      resource
        .list({
          tree: true,
  ...
  return React.createElement(DesktopRoutesContext.Provider, { value: value }, children);
};
export const DesktopAllRoutesProvider = ({ children, active }) => {
  ...
  return React.createElement(DesktopRoutesProvider, { refreshRef: refreshRef }, children);
};
export const MenuPermissions = ({ active }) => {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (_, schema) => {
            ...
            return React.createElement(Checkbox, { checked: checked, onChange: () => handleChange(checked, schema) });
          },
        },
      ],
...
//# sourceMappingURL=MenuPermissions.js.map
