...
const TokenInput = ({ value, theme, onChange, light, readonly, onReset, canReset: customCanReset, hideTheme, }) => {
    ...
    const addonAfter = !readonly && (React.createElement("span", { style: {
            display: 'flex',
            alignItems: 'center',
            minWidth: hideTheme ? '' : 80,
        } }, canReset || hideTheme ? (React.createElement(Button, { style: {
            fontSize: 12,
        }, onClick: handleReset, type: "link", size: "small", disabled: !canReset }, locale.reset)) : (React.createElement("span", { style: { padding: '0 8px' } }, theme?.name))));
    let inputNode;
    if (typeof valueRef.current === 'string' && isColor(valueRef.current)) {
        inputNode = (React.createElement(Input, { bordered: false, addonAfter: addonAfter, value: String(tokenValue), disabled: readonly, addonBefore: React.createElement(Dropdown, { trigger: ['click'], overlay: React.createElement(ColorPanel, { alpha: true, color: String(tokenValue), onChange: (v) => {
                        handleTokenChange(v);
                    } }) },
                React.createElement(ColorPreview, { color: String(tokenValue), dark: theme?.key === 'dark', style: {
                        cursor: 'pointer',
                        marginInlineEnd: 8,
                        verticalAlign: 'top',
                    } })), onChange: (e) => {
        ...
        inputNode = (React.createElement(InputNumber, { addonAfter: addonAfter, bordered: false, value: tokenValue, disabled: readonly, onChange: (newValue) => {
                handleTokenChange(Number(newValue));
            } }));
    }
    else {
        inputNode = (React.createElement(Input, { addonAfter: addonAfter, bordered: false, value: String(tokenValue), disabled: readonly, onChange: (e) => {
                handleTokenChange(typeof value === 'number' ? Number(e.target.value) : e.target.value);
            } }));
    }
    return wrapSSR(React.createElement("div", { className: classNames('previewer-token-input', hashId, {
