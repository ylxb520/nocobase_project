...
const ThemeSelect = (props) => {
    ...
    const shownThemeEntities = useMemo(() => themes.filter((theme) => shownThemes.includes(theme.key)), [themes, shownThemes]);
    return wrapSSR(React.createElement("div", { className: classNames('previewer-theme-select', hashId) },
        shownThemeEntities.map((theme) => (React.createElement("span", { onClick: () => {
                if (theme.fixed) {
                    return;
                }
                onEnabledThemeChange(enabledThemes.includes(theme.key)
                    ? enabledThemes.filter((item) => item !== theme.key)
                    : [...enabledThemes, theme.key]);
            }, key: theme.key, className: classNames('previewer-theme-select-tag', {
                'previewer-theme-select-tag-active': enabledThemes.includes(theme.key),
            }) },
            theme.name,
            theme.closable && (React.createElement("span", { className: "previewer-theme-select-tag-close-btn", onClick: (e) => {
                    e.stopPropagation();
                    onEnabledThemeChange(enabledThemes.filter((item) => item !== theme.key));
                    onShownThemeChange(shownThemes.filter((item) => item !== theme.key), theme.key, { type: 'deselect' });
                } },
                React.createElement(CloseOutlined, null)))))),
        showAddTheme && (React.createElement(Dropdown, { placement: "bottomRight", trigger: ['click'], overlay: React.createElement(Menu, { items: dropdownItems }), overlayClassName: classNames('previewer-theme-select-dropdown', hashId) },
            React.createElement(Button, { type: "primary", shape: "circle", className: "previewer-theme-select-add-btn", icon: React.createElement(PlusOutlined, null) })))));
};
