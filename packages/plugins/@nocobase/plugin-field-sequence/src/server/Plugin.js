...
export default class PluginFieldSequenceServer extends Plugin {
    ...
    async load() {
        ...
        db.on('fields.beforeSave', async (field, { transaction }) => {
            ...
            await patterns.reduce((promise, p) => promise.then(async () => {
            ...
            for (const [collectionName, sequencesList] of Object.entries(groupedSequences)) {
                tasks.push(async () => {
                    ...
                    if (!autoIncrementField && !createAtField) {
                        app.log.warn(`Collection [${collection.name}] does not have autoIncrement or createdAt fields. Skipping sequences refresh.`);
                        return;
                    }
                    ...
                    const sequencesFieldSet = _.uniq(sequencesList.map(({ field }) => field));
                    for (const sequencesField of sequencesFieldSet) {
                        const field = fieldMap[sequencesField];
                        app.log.info(`Repair sequences: collection=${collection.name}, field=${sequencesField}, type=${field?.constructor?.name}`);
                        if (!field) {
                            app.log.warn(`Collection [${collection.name}] field [${sequencesField}] definition not found. Skipping sequences repair.`);
                            continue;
                        }
                        if (!(field instanceof SequenceField)) {
                            app.log.warn(`Collection [${collection.name}] field [${sequencesField}] is not a SequenceField. Skipping sequences repair.`);
                            continue;
                        }
