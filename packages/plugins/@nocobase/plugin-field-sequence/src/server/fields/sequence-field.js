 ...
 */
import { DataTypes, Field, ValidationError, ValidationErrorItem, } from '@nocobase/database';
import { Registry } from '@nocobase/utils';
import parser from 'cron-parser';
import dayjs from 'dayjs';
import lodash from 'lodash';
export const sequencePatterns = new Registry();
function parseBigInt(str, radix = 10) {
    ...
    for (const ch of chars) {
        ...
        if (value < 0n || value >= BigInt(radix))
            throw new SyntaxError(`Invalid digit "${ch}" for base ${radix}`);
...
sequencePatterns.register('integer', {
    ...
    async generate(instance, options, { transaction }) {
        const recordTime = instance.get('createdAt') ?? new Date();
        ...
        if (lastSeq.get('current') != null) {
            ...
            if (cycle) {
                const interval = parser.parseExpression(cycle, { currentDate: lastSeq.get('lastGeneratedAt') });
...
sequencePatterns.register('randomChar', {
    ...
    getMatcher(options) {
        const pattern = [
            ...new Set((options.charsets || ['number']).reduce((acc, charset) => {
                switch (charset) {
                    case 'number':
                        return acc + '0-9';
                    case 'lowercase':
...
export class SequenceField extends Field {
    ...
    validate = (instance) => {
        ...
        if (value != null && inputable && match && !this.match(value)) {
            throw new ValidationError('sequence pattern not match', [
                new ValidationErrorItem(`input value of ${name} field not match the sequence pattern (${this.matcher.toString()}) which is required`, 'validation error', // NOTE: type should only be this which in sequelize enum set
                name, value, instance, 'sequence_pattern_not_match', name, []),
            ]);
        }
    };
    ...
    setGroupValue = async (instances, options) => {
        ...
        await patterns.reduce((promise, p, i) => promise.then(() => sequencePatterns.get(p.type).batchGenerate.call(this, instances, array[i], p.options ?? {}, options)), Promise.resolve());
        instances.forEach((instance, i) => {
            const value = instance.get(name);
...
//# sourceMappingURL=sequence-field.js.map