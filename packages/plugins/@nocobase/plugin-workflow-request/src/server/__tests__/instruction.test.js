...
describe('workflow > instructions > request', () => {
    ...
    beforeEach(async () => {
        ...
        db = app.db;
        instruction = app.pm.get(PluginWorkflow).instructions.get('request');
    ...
    describe('request db resource', () => {
        it('request db resource', async () => {
            ...
            const n1 = await workflow.createNode({
                type: 'request',
                config: {
                    url: `http://localhost:${server.address().port}/api/categories`,
                    method: 'POST',
    ...
    describe('sync request', () => {
        ...
        it('sync trigger', async () => {
            ...
            const workflowPlugin = app.pm.get(PluginWorkflow);
            const processor = (await workflowPlugin.trigger(syncFlow, { data: { title: 't1' } }));
