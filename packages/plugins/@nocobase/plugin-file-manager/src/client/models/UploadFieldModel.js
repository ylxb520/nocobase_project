...
export const CardUpload = (props) => {
    const { allowSelectExistingRecord, multiple, value, disabled, onSelectExitRecordClick, quickUpload = true, showFileName, } = props;
    ...
    const switchPreviewIndex = (nextIndex) => {
    ...
    return (React.createElement(FieldContext.Provider, { value: {
        ...
        React.createElement("div", { style: { display: 'flex' }, className: css `
        ...
        ` },
            React.createElement(Upload, { onPreview: handlePreview, ...props, listType: "picture-card", fileList: fileList, onChange: (newFileList) => {
                    setFileList(newFileList);
                    const doneFiles = newFileList.filter((f) => f.status === 'done' || f.id);
                    if (newFileList.every((f) => f.status === 'done' || f.id)) {
                        if (props.maxCount === 1) {
                    ...
                    return (React.createElement(React.Fragment, null,
                        originNode,
                        showFileName && (React.createElement("div", { style: {
                                fontSize: 12,
                                whiteSpace: 'nowrap',
                                textOverflow: 'ellipsis',
                                overflow: 'hidden',
                            }, title: file.filename }, file.filename))));
                } }, quickUpload && React.createElement(UploadOutlined, { style: { fontSize: 20 } })),
            previewImage && (React.createElement(FilePreviewRenderer, { open: previewOpen, file: previewImage, list: fileList, index: currentImageIndex, onOpenChange: setPreviewOpen, onClose: () => setPreviewImage(null), onSwitchIndex: switchPreviewIndex, onDownload: onDownload })),
            allowSelectExistingRecord ? (React.createElement("div", { style: { marginLeft: 5 } },
                React.createElement(Upload, { disabled: disabled, multiple: multiple, listType: 'picture-card', showUploadList: false },
...
export let UploadFieldModel = class UploadFieldModel extends FieldModel {
    ...
    render() {
        return React.createElement(CardUpload, { ...this.props });
    }
};
UploadFieldModel = __decorate([
    largeField()
], UploadFieldModel);
UploadFieldModel.registerFlow({
    ...
    steps: {
        ...
        allowMultiple: {
            ...
            hideInSettings(ctx) {
                return (!ctx.collectionField || !['belongsToMany', 'hasMany', 'belongsToArray'].includes(ctx.collectionField.type));
            },
            defaultParams(ctx) {
                return {
                    multiple: ctx.collectionField &&
    ...
    steps: {
        step1: {
            async handler(ctx, params) {
                ...
                const fileManagerPlugin = ctx.app.pm.get('@nocobase/plugin-file-manager');
    ...
    steps: {
        openView: {
            ...
            handler(ctx, params) {
                ...
                const quickEditPopover = ctx.model.parent?.use === 'QuickEditFormModel' ? ctx.model.parent?.viewContainer : null;
                ...
                ctx.viewer.open({
                        ...
                        rowSelectionProps: {
                            ...
                            onChange: (_, selectedRows) => {
                                if (toOne) {
                                    ...
                                    // 去重，防止同一个值重复
                                    const unique = merged.filter((row, index, self) => index ===
                    ...
                    content: () => React.createElement(RecordPickerContent, { model: ctx.model }),
                    styles: {
                        content: {
                            padding: 0,
...
EditableItemModel.bindModelToInterface('UploadFieldModel', ['attachment', 'm2m', 'm2o', 'o2o', 'o2m', 'oho', 'obo', 'updatedBy', 'createdBy', 'mbm'], {
    isDefault: true,
    order: 30,
    when: (ctx, field) => {
...
//# sourceMappingURL=UploadFieldModel.js.map