...
export function FileSizeField(props) {
    const { value, defaultUnit = 1024 * 1024, min = FILE_SIZE_LIMIT_MIN, max = FILE_SIZE_LIMIT_MAX, step = 1, onChange, } = props;
    ...
    const onNumberChange = useCallback((val) => {
        onChange?.(limitSize(val == null ? val : val * vOption.value, min, max));
    }, [vOption.value]);
    const onUnitChange = useCallback((val) => {
        onChange?.(limitSize(v * val, min, max));
    }, [v]);
    return (React.createElement(Space.Compact, null,
        React.createElement(InputNumber, { value: v, onChange: onNumberChange, defaultValue: `${dv}`, step: step }),
        React.createElement(Select, { options: UnitOptions, value: vOption.value, onChange: onUnitChange, className: "auto-width" })));
}
