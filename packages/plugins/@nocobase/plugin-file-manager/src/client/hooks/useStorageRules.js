...
export function useAttachmentFieldProps() {
    ...
    const action = `${collectionField?.target}:create${collectionField?.storage ? `?attachmentField=${collectionField.collectionName}.${collectionField.name}` : ''}`;
        ...
        if (recordId && collection?.name && collectionField?.name && !isAutoSavingRef.current) {
            ...
            try {
                ...
                const attachmentIds = Array.isArray(value)
                    ? value.map((v) => v?.id || v).filter((id) => typeof id === 'number' || typeof id === 'string')
                    : value?.id
                        ? [value.id]
                        : [];
...
//# sourceMappingURL=useStorageRules.js.map