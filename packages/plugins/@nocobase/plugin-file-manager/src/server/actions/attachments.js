...
import Plugin from '..';
import { FILE_FIELD_NAME, FILE_SIZE_LIMIT_DEFAULT, FILE_SIZE_LIMIT_MIN, LIMIT_FILES, STORAGE_TYPE_LOCAL, } from '../../constants';
...
async function multipart(ctx, next) {
    ...
    const StorageClass = ctx.app.pm.get(Plugin).storageTypes.get(storage.type);
    if (!StorageClass) {
...
export async function createMiddleware(ctx, next) {
    ...
    const storageName = resourceName === 'attachments'
        ? ctx.db.getFieldByPath(attachmentField)?.options?.storage
        : collection.options.storage;
    const plugin = ctx.app.pm.get(Plugin);
    const storage = Array.from(plugin.storagesCache.values()).find((storage) => storageName ? storage.name === storageName : storage.default);
    if (!storage) {
