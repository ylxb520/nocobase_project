    ...
    describe('create / upload', () => {
        describe('default storage', async () => {
            it('should be create file record', async () => {
                const Plugin = app.pm.get(PluginFileManagerServer);
            ...
            it('upload file should be ok', async () => {
                ...
                const destPath = path.resolve(path.isAbsolute(documentRoot) ? documentRoot : path.join(process.cwd(), documentRoot), storage.path || '');
        ...
        describe('specific storage', () => {
            ...
            it('upload file with wrong extension but correct mime type should be ok', async () => {
                ...
                const destPath = path.resolve(path.isAbsolute(documentRoot) ? documentRoot : path.join(process.cwd(), documentRoot), imageStorage.path || '');
            ...
            it('path longer than 255', async () => {
                const BASE_URL = `/storage/uploads/another`;
                const urlPath = 'extreme-test/max-long-path-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890-1234567890';
                // 动态添加 storage
                const storage = await StorageRepo.create({
                    values: {
                        name: 'local_private',
