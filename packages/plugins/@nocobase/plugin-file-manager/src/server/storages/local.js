...
export function getDocumentRoot(storage) {
    const { documentRoot = process.env.LOCAL_STORAGE_DEST || path.join(process.cwd(), 'storage', 'uploads') } = storage.options || {};
...
export function resolveSafePath(documentRoot, filePath, filename) {
...
export default class extends StorageType {
    ...
    async delete(records) {
        ...
        let count = 0;
        const undeleted = [];
        await records.reduce((promise, record) => promise.then(async () => {
            try {
