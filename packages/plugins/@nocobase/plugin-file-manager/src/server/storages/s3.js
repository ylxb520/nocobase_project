...
export default class default_1 extends StorageType {
    static defaults() {
        return {
            title: 'AWS S3',
            name: 'aws-s3',
    ...
    async delete(records) {
        const { Deleted } = await this.deleteS3Objects(this.storage.options.bucket, records.map((record) => this.getFileKey(record)));
