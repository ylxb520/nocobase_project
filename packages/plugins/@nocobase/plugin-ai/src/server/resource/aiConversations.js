  ...
  actions: {
    ...
    async getMessages(ctx, next) {
      ...
      const toolCallIds = data
        .filter((row) => row?.toolCalls?.length ?? 0 > 0)
      ...
      const toolMessageMap = new Map(
        toolMessages.map((toolMessage) => [toolMessageKey(toolMessage.messageId, toolMessage.toolCallId), toolMessage]),
      );
    ...
    async sendMessages(ctx, next) {
      const { sessionId, aiEmployee: employeeName, messages, editingMessageId, model, webSearch, stream = true } =
        ctx.action.params.values || {};
      ...
      try {
        ...
        if (!conversation.title) {
          const textUserMessage = messages.find(
            (message) => message.role === 'user' && message.content?.type === 'text' && message.content?.content,
          );
          if (textUserMessage) {
    ...
    async updateUserDecision(ctx, next) {
      ...
      const toolMessageMap = new Map(toolMessages.map((toolMessage) => [toolMessage.toolCallId, toolMessage]));
      const toolsList = await plugin.ai.toolsManager.listTools();
      const toolsMap = new Map(toolsList.map((t) => [t.definition.name, t]));
...
//# sourceMappingURL=aiConversations.js.map
