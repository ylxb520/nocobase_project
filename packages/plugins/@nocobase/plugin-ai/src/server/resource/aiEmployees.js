...
export const listByUser = async (ctx, next) => {
  const plugin = ctx.app.pm.get('ai');
  ...
  if (!roles?.includes('root')) {
    ...
    where = {
      ...where,
      username: aiEmployees.map((item) => item.aiEmployee),
    };
  }
  ...
  ctx.body = rows.map((row) => {
    const skillSettings = row.skillSettings ?? { skills: [] };
...
export const updateUserPrompt = async (ctx, next) => {
...
export const getTemplates = async (ctx, next) => {
