...
const docsIndexLoading = new Map();
...
export async function searchDocsModule(moduleKey, keywords, limit = 5) {
  ...
  const keywordHits = await Promise.all(
    searchTerms.map(async (keyword) =>
      ((await flexIndex.searchAsync(keyword, { limit: perKeywordLimit })) || []).map((id) => String(id)),
    ),
  );
...
export async function readDocEntry(docPath) {
  ...
  if (stat.isDirectory()) {
    ...
    const entries = await Promise.all(
      names.sort().map(async (name) => {
        ...
        const entryType = childStat.isDirectory() ? 'directory' : 'file';
        return {
          name,
          type: entryType,
          path: childPath,
...
export function createReadDocEntryTool() {
  return {
    scope: 'GENERAL',
    defaultPermission: 'ALLOW',
    introduction: {
...
function normalizeDocInput(input) {
...
async function getOrLoadDocsIndex(entry) {
  const now = Date.now();
