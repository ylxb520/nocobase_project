...
export function sendSSEError(ctx, error, errorName) {
...
export function stripToolCallTags(content) {
  if (typeof content !== 'string') {
    return content;
  }
  return content.replace(/<[|｜]tool▁(?:calls▁begin|calls▁end|call▁begin|call▁end|sep)[|｜]>/g, '');
}
export function parseResponseMessage(row) {
...
export async function encodeLocalFile(url) {
  url = path.join(process.cwd(), url);
  const data = await fs.promises.readFile(url);
  return Buffer.from(data).toString('base64');
}
export async function encodeFile(ctx, url) {
...
export async function parseVariables(ctx, value) {
  const re = /\{\{\$user\.([^}]+)\}\}/g;
