...
import { ToolDefinition } from '../llm-providers/provider';
...
export class AIEmployee {
  ...
  async buildChatContext({ messageId, userMessages, userDecisions }) {
  ...
  async stream({ messageId, userMessages = [], userDecisions }) {
    try {
      ...
      const { stream, signal } = await this.prepareChatStream({
        chatContext,
        provider,
        config: { ...config, callbacks: [responseMetadataCollector] },
        state,
      });
  ...
  async invoke({ messageId, userMessages = [], userDecisions }) {
    try {
  ...
  async createAgent({ provider, systemPrompt, tools, middleware }) {
  ...
  async agentStream(provider, context, config, state) {
  ...
  async prepareChatStream({ chatContext, provider, config, state }) {
  ...
  getEmployeeDataSourceContext() {
    const dataSourceSettings = this.employee.dataSourceSettings;
...
If information is missing, clearly state it in the summary.</Important>`;
  ...
  async updateToolCallDone(messageId, toolCallId, result) {
    const [updated] = await this.aiToolMessagesModel.update(
      {
        invokeStatus: 'done',
  ...
  async confirmToolCall(transaction, messageId, toolCallIds) {
    const [updated] = await this.aiToolMessagesModel.update(
      {
        invokeStatus: 'confirmed',
      },
  ...
  async getToolCallResult(messageId, toolCallId) {
    return (
  ...
  async updateThread(transaction, { sessionId, thread }) {
  ...
  async formatMessages({ messages, provider }) {
    ...
    // 截断过长的内容
    const truncate = (text, maxLen = 50000) => {
  ...
  async forkCurrentThread(provider) {
    let retTry = 3;
...
class AgentThread {
  ...
  static newThread(sessionId, thread) {
...
class ChatStreamProtocol {
  ...
  toolCallStatus({ toolCall, invokeStatus, status, interruptAction }) {
    this.write({
...
class ResponseMetadataCollector extends BaseCallbackHandler {
  ...
  getResponseMetadata(id) {
    return this.responseMetadata.get(id);
  }
}
//# sourceMappingURL=ai-employee.js.map
