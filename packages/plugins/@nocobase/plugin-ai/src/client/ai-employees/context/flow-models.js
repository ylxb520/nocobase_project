...
const parseFlowModel = async (model) => {
  if (!model) {
    return {};
  }
...
const toCollection = async (model) => {
...
const toSimplifyComponentTree = async (model) => {
  const result = {
    uid: model.uid,
    title: model.title,
    component: model.use,
  ...
  result.data = await FlowUtils.getResource(model);
  return result;
};
const handleSelect = (ctx, onAdd) => {
...
export const FlowModelsContext = {
  name: 'flow-model',
  menu: {
    icon: React.createElement(BuildOutlined, null),
    Component: () => {
      const t = useT();
      return React.createElement('div', null, t('Pick block'));
    },
  ...
  tag: {
    Component: ({ item }) => {
      ...
      return React.createElement(
        ...
        React.createElement('span', null, model?.title || item?.title || ''),
      );
    },
  },
...
//# sourceMappingURL=flow-models.js.map
