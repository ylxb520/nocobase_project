...
export function replaceTagBlockByIndex(input, tagName, indexToReplace, newInnerStr) {
...
const EditModal = ({ open, setOpen, optionStr, message, index }) => {
  const t = useT();
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        option
          ...
          : React.createElement(Alert, { type: 'error', showIcon: true, message: t('Invalid configuration') }),
      ),
    ),
  );
};
export const Echarts = (props) => {
  ...
  try {
    ...
    return React.createElement(
      ...
      React.createElement(
        SyntaxHighlighter,
        { ...rest, PreTag: 'div', language: 'json', style: isDarkTheme ? dark : defaultStyle },
