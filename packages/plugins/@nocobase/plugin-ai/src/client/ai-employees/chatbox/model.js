...
export const getAllModels = (services) =>
...
export const isValidModel = (value, allModels) =>
  !!value && allModels.some((item) => item.llmService === value.llmService && item.model === value.model);
export const isSameModel = (a, b) => a?.llmService === b?.llmService && a?.model === b?.model;
export const resolveModel = (api, username, allModels, currentOverride) => {
...
export const ensureModel = async ({ api, aiConfigRepository, username, currentOverride, onResolved }) => {
