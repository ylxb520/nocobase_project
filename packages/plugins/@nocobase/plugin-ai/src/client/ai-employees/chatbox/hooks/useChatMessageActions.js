...
export const useChatMessageActions = () => {
  ...
  const messagesServiceRef = useRef();
  messagesServiceRef.current = messagesService;
  const processStreamResponse = async (stream, sessionId, aiEmployee) => {
          ...
          try {
            ...
            if (data.type === 'content' && data.body && typeof data.body === 'string') {
              ...
              updateLastMessage((last) => ({
                ...
                content: {
                  ...last.content,
                  content: last.content.content + data.body,
                },
                loading: false,
              }));
            }
  ...
  const resendMessages = async ({ sessionId, messageId, aiEmployee, important }) => {
  ...
  const resumeToolCall = useCallback(
    async ({ sessionId, messageId, aiEmployee, toolCallIds, toolCallResults }) => {
      setResponseLoading(true);
      // Read model from store at call time to avoid stale closure.
      // If not ready yet, resolve it through shared model rules.
      let model = useChatBoxStore.getState().model;
  ...
  const startEditingMessage = useCallback((msg) => {
