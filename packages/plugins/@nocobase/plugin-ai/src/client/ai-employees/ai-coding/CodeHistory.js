...
import { Avatar, Button, Flex, Layout, List } from 'antd';
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(Title, { level: 5, style: { margin: 0 } }, t('Code history')),
      ),
    ),
    React.createElement(Content, null, React.createElement(CodeHistoryList, null)),
  );
};
const CodeHistoryList = () => {
  ...
  return React.createElement(List, {
    ...
    renderItem: (item, index) =>
      React.createElement(List.Item, { key: index }, React.createElement(CodeHistoryListItem, { ...item })),
  });
};
const CodeHistoryListItem = ({ message, workContext }) => {
  const currentEmployee = useChatBoxStore.use.currentEmployee();
  const content = workContext.content;
  const avatarProps = {
    size: 33,
    shape: 'circle',
    style: {
  ...
  return React.createElement(
    ...
    message.role === 'user'
      ? React.createElement(Avatar, { ...avatarProps, icon: React.createElement(UserOutlined, null) })
      : React.createElement(Avatar, { ...avatarProps, src: avatars(currentEmployee.avatar) }),
    React.createElement(
      'div',
      {
        className: css`
      ...
      React.createElement(CodeBasic, { className: `language-${content.language}` }, content.code),
    ),
  );
};
