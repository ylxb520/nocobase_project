...
export class Radar extends EChart {
  ...
  getProps({ data, general, advanced, fieldProps }) {
    ...
    if (variableType === 'dimension') {
      variables = Array.from(new Set(data.map((row) => row[xField]))).map((value) => value || 'null');
      data = yFields.map((field) => ({
        name: fieldProps[field]?.label,
        value: data.map((row) => row[field]),
      }));
    } else {
      variables = yFields.map((field) => fieldProps[field]?.label);
      data = data.map((row) => ({
        name: row[xField] || 'null',
        value: yFields.map((field) => row[field]),
      }));
    }
