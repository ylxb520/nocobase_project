...
export class EChart extends Chart {
  static lightThemes = ['default', 'light'];
  static darkThemes = ['dark'];
  series;
  ...
  static registerTheme(name, theme, mode = 'light') {
  ...
  getProps({ data, general, advanced, fieldProps }) {
    ...
    let seriesName = yFields.map((field) => fieldProps[field]?.label || field);
    if (seriesField) {
      seriesName = Array.from(new Set(data.map((row) => row[seriesField]))).map((value) => value || 'null');
      yFields = seriesName;
    }
    return {
      legend: {
