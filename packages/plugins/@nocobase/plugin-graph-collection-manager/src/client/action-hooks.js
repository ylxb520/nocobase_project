...
import { useAPIClient, useActionContext, useCollectionFieldFormValues, useCollectionManager_deprecated, useCompile, useRequest, } from '@nocobase/client';
...
export const SourceCollection = observer(() => {
    ...
    return (React.createElement("div", null,
        React.createElement(Select, { popupMatchSelectWidth: false, disabled: true, value: record.name, options: [{ value: record.name, label: compile(record.title) }] })));
...
export const useCreateAction = (collectionName, targetId) => {
    ...
    return {
        async run() {
            ...
            const { data: { data }, } = await api.resource('collections.fields', collectionName).create({
                values: formValues,
            });
            targetId &&
...
export const useAsyncDataSource = (service) => {
    return (field, { targetScope }) => {
        field.loading = true;
        service(targetScope)
...
//# sourceMappingURL=action-hooks.js.map