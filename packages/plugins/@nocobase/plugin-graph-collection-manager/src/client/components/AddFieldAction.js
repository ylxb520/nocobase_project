...
export const AddFieldAction = ({ item: record, database }) => {
    return (React.createElement(AddCollectionFieldAction, { trigger: ['click'], align: {
            overflow: {
                adjustY: false, // 关闭溢出位置调整
            },
        }, item: record, database: database, scope: {
            useCancelAction,
            useCreateCollectionField: () => useCreateCollectionField(record),
        }, getContainer: getPopupContainer },
        React.createElement(PlusOutlined, { className: "btn-add", id: "graph_btn_add_field" })));
};
