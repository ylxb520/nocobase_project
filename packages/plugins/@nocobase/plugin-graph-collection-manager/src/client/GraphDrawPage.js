...
import { APIClientProvider, ApplicationContext, CollectionCategoriesContext, CurrentAppInfoContext, DataSourceApplicationProvider, SchemaComponent, SchemaComponentOptions, Select, useAPIClient, useApp, useCollectionManager_deprecated, useCompile, useCurrentAppInfo, useDataSource, useDataSourceManager, useGlobalTheme, } from '@nocobase/client';
...
import { cleanGraphContainer, collection, formatData, getChildrenCollections, getDiffEdge, getDiffNode, getInheritCollections, getPopupContainer, useGCMTranslation, } from './utils';
...
function optimizeEdge(edge) {
    const { store: { data: { connectionType }, }, } = edge;
...
const handelResetLayout = (isTemporaryLayout) => {
    ...
    if (nodeWithEdges.length) {
        maxY = targetGraph
            .getCellById(maxBy(nodeWithEdges, (k) => {
            return targetGraph.getCellById(k).position().y;
        }))
            .position().y;
        minX = targetGraph
            .getCellById(minBy(nodeWithEdges, (k) => {
        ...
        const referenceNode = targetGraph
...
export const GraphDrawPage = React.memo(() => {
    ...
    const initGraphCollections = () => {
        ...
        register({
            ...
            component: (props) => {
                return (React.createElement(CurrentAppInfoContext.Provider, { value: currentAppInfo },
                    React.createElement(DataSourceApplicationProvider, { dataSourceManager: dm, dataSource: dataSource?.key },
                        React.createElement(APIClientProvider, { apiClient: api },
                            React.createElement(SchemaComponentOptions, { inherit: true, scope: scope, components: components },
                                React.createElement(CollectionCategoriesContext.Provider, { value: categoryCtxRef.current },
                                    React.createElement(ConfigProvider, { theme: theme },
                                        React.createElement("div", { style: { height: 'auto' } },
                                            React.createElement(App, null,
                                                React.createElement(ApplicationContext.Provider, { value: app },
                                                    React.createElement(Entity, { ...props, setTargetNode: setTargetNode, targetGraph: targetGraph })))))))))));
            },
        });
        targetGraph.use(new Scroller({
            autoResize: true,
            enabled: true,
            pannable: true,
            pageVisible: true,
    ...
    const handleCleanHighlight = (key, currentDirection, currentConnectionType) => {
        ...
        for (let i = 0; i < length; i++) {
            ...
            const renltionshipFlag = key && targetGraph.filterConfig?.key === key ? connectionType !== currentConnectionType : true;
            if (nodes[i].id !== key) {
                setTimeout(() => {
                    filterFlag &&
    ...
    const handleFiterCollections = (value) => {
        ...
        const relationshipBothFlag = value === filterConfig?.key &&
    ...
    const handleSetEdgeVisible = (type) => {
        ...
        const edges = targetGraph.getEdges();
        edges.forEach((v) => {
            const { store: { data: { connectionType }, }, } = v;
    ...
    return (React.createElement(Layout, null,
        React.createElement("div", { className: styles.graphCollectionContainerClass },
            React.createElement(Spin, { spinning: loading },
                React.createElement(DataSourceApplicationProvider, { dataSourceManager: dm },
                    React.createElement(CollapsedContext.Provider, { value: { collectionList, handleSearchCollection } },
                        React.createElement("div", { className: cx(styles.collectionListClass) },
                                                ...
                                                actions: {
                                                    ...
                                                    properties: {
                                                        ...
                                                        autoLayout: {
                                                            ...
                                                            'x-component-props': {
                                                                component: () => {
                                                                    return (React.createElement(Tooltip, { title: t('Auto layout'), getPopupContainer: getPopupContainer },
                                                        ...
                                                        selectMode: {
                                                            type: 'void',
                                                            'x-component': () => {
                                                                return (React.createElement(Tooltip, { title: t('Selection') },
                ...
                React.createElement("div", { id: "container", style: { width: '100vw' } }),
                React.createElement("div", { id: "graph-minimap", className: styles.graphMinimap, style: { width: '300px', height: '250px', right: '10px', bottom: '20px', position: 'fixed' } })))));
});
GraphDrawPage.displayName = 'GraphDrawPage';
//# sourceMappingURL=GraphDrawPage.js.map