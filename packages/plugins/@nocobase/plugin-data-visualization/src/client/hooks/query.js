...
import { DEFAULT_DATA_SOURCE_KEY, useACLRoleContext, useDataSourceManager, usePlugin } from '@nocobase/client';
...
export const useChartDataSource = (dataSource) => {
...
export const useFields = (collectionFields) => {
...
export const useFieldsWithAssociation = (dataSource, collection) => {
    ...
    () =>
      fields.map((field) => {
        ...
        if (filterable?.children?.length) {
          const children = filterable.children.map((child) => ({
            ...child,
...
export const useChartFields = (fields) => (field) => {
...
export const useFormatters = (fields) => (field) => {
...
export const useOrderFieldsOptions = (defaultOptions, fields) => (field) => {
  const query = field.query('query').get('value') || {};
...
export const useOrderReaction = (defaultOptions, fields) => (field) => {
  const query = field.query('query').get('value') || {};
  const { measures = [] } = query;
  const hasAgg = measures.some((measure) => measure.aggregation);
...
export const useData = (data, dataSource, collection) => {
  ...
  for (const field of selectedFields) {
    ...
    formatters[field.value] = (value) => valueFormatter(field, value);
  }
  return (service?.data || data || []).map((record) => {
    const processed = {};
...
export const useCollectionFieldsOptions = (dataSource, collectionName, maxDepth = 2, excludes = []) => {
...
export const useCollectionFilterOptions = (dataSource, collection) => {
  const { cm, fim, collection: _collection } = useChartDataSource(dataSource);
  return useMemo(() => {
