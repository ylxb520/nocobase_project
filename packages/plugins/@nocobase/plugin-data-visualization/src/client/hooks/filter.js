...
export const useCustomFieldInterface = () => {
  ...
  return {
    getSchemaByInterface: (fieldInterface) => {
...
export const useChartFilter = () => {
  ...
  const getChartFilterFields = ({ dataSource, collection, cm, fim }) => {
    const fields = cm.getCollectionFields(collection.name);
    const field2item = (field, title, name, fieldName) => {
    ...
    const children2item = (child, title, name, fieldName) => {
    ...
    const field2option = (field, depth, title, name, fieldName) => {
  ...
  const hasFilter = (chart, filterValues) => {
    if (!chart) {
      return false;
    }
    ...
    return (
      ...
      (filterValues['custom'] && parameters?.find(({ key }) => lodash.has(filterValues['custom'], key)))
    );
  };
  const appendFilter = (chart, filterValues) => {
    const { dataSource, collection, query } = chart;
    let newQuery = { ...query };
    ...
    if (filterValues['custom'] && parameters?.find((param) => filterValues['custom'][param.key])) {
  ...
  const getTranslatedTitle = useMemoizedFn((title) => {
    return title
      .split(' / ')
      .map((item) => i18n.t(Schema.compile(item, { t: i18n.t })))
      .join(' / ');
  });
...
export const useChartFilterSourceFields = () => {
  ...
  const field2option = (cm, field, depth) => {
    if (!field.interface) {
      return;
    }
    const fieldInterface = fim.getFieldInterface(field.interface);
    ...
    if (nested) {
      ...
      return {
        value: field.name,
        label: t(field?.uiSchema?.title || field.name),
        children: items.filter((item) => item),
      };
    }
...
export const useCollectionJoinFieldTitle = (dataSource, name) => {
  const { t } = useChartsTranslation();
  const dm = useDataSourceManager();
