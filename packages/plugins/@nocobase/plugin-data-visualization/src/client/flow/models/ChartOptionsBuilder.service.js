...
// 允许保留的公共键（不随类型清理掉）
const COMMON_FIELD_KEYS = ['type', 'legend', 'tooltip', 'label', 'boundaryGap', 'xAxisLabelRotate', 'yAxisSplitLine'];
// 根据类型自动填充的主字段规则
const AUTO_FILL_RULES = {
...
function normalizeByType(type, builder = {}, columns = []) {
  const next = stripInvalidColumns(builder, columns);
  next.type = type;
  const specs = TYPE_FIELD_SPECS[type] || TYPE_FIELD_SPECS.line;
  const allowed = new Set([...COMMON_FIELD_KEYS, ...specs.map((fs) => fs.name)]);
  // 根据类型自动填充主字段
  ...
  specs.forEach((fs) => {
    if (Object.prototype.hasOwnProperty.call(fs, 'default')) {
      const v = next[fs.name];
      if (v == null) next[fs.name] = fs.default;
    }
  });
  // 清理掉不属于该类型的字段（保留公共键）
...
const normalizeLine = (builder = {}, columns) => normalizeByType('line', builder, columns);
const normalizeBar = (builder = {}, columns) => normalizeByType('bar', builder, columns);
const normalizeBarHorizontal = (builder = {}, columns) => normalizeByType('barHorizontal', builder, columns);
const normalizePie = (builder = {}, columns) => normalizeByType('pie', builder, columns);
const normalizeDoughnut = (builder = {}, columns) => normalizeByType('doughnut', builder, columns);
const normalizeFunnel = (builder = {}, columns) => normalizeByType('funnel', builder, columns);
const normalizeScatter = (builder = {}, columns) => normalizeByType('scatter', builder, columns);
const normalizeArea = (builder = {}, columns) => normalizeByType('area', builder, columns);
...
export function getChartFormSpec(type) {
  const specs = TYPE_FIELD_SPECS[type] || TYPE_FIELD_SPECS.line;
  return specs.map((fs) => {
    if (fs.valueType === 'enum') {
      return {
        ...
        options: (fs.options || []).map((v) => ({ labelKey: v, value: v })),
      };
    }
    ...
    if (fs.valueType === 'boolean') {
      return {
        kind: 'switch',
        name: fs.name,
        labelKey: BOOL_LABEL_KEY[fs.name] || fs.name,
      };
    }
    const limits = NUMBER_LIMITS[fs.name] || {};
...
const pickFirstTwo = (columns) => ({ a: columns[0], b: columns[1] });
// 从 columns 构建字段候选项
export function buildFieldOptions(columns = []) {
  return (columns || []).map((c) => ({ label: c, value: c }));
}
// 清理引用了无效列的字段
export function stripInvalidColumns(builder = {}, columns = []) {
  const next = { ...builder };
...
const genRawBarHorizontal = (builder) => {
  const {
    xField,
    yField,
    seriesField,
...
//# sourceMappingURL=ChartOptionsBuilder.service.js.map
