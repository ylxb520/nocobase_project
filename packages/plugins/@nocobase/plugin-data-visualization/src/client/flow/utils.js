...
export function convertDatasetFormats(data) {
...
export function normalizeEChartsOption(option) {
  ...
  const pieSeries = seriesArr.filter((s) => s?.type === 'pie');
  ...
  const replaceC = (tpl) => (tpl.includes('{c}') ? tpl.replace(/\{c\}/g, '{@' + valueDim + '}') : tpl);
  pieSeries.forEach((s) => {
...
export function appendColon(label, lang) {
