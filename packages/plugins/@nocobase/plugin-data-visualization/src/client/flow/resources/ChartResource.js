...
export class ChartResource extends BaseRecordResource {
  resourceName = 'charts';
  refreshTimer = null;
  request = {
    url: 'charts:query',
  ...
  setFilter(filter) {
    ...
    if (
      ...
      Array.isArray(cleanedRoot.$and)
    ) {
      const andItems = cleanedRoot.$and
        .map((item) => removeUnparsableFilter(item))
        .filter(Boolean)
        .flatMap((item) => (item && Array.isArray(item.$and) ? item.$and : [item]));
  ...
  async run() {
    ...
    const mode = data.mode ?? (data.sql ? 'sql' : undefined);
    if (!mode) {
    ...
    return await this.runAction('query', this.getRefreshRequestOptions());
  }
  // debounce 刷新数据
  async refresh() {
...
//# sourceMappingURL=ChartResource.js.map
