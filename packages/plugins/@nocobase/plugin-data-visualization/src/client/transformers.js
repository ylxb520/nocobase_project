...
const transformers = {
  general: {
    Prefix: {
      ...
      fn: (val, prefix) => (prefix ? `${prefix}${val}` : val),
    },
    Suffix: {
      schema: {
        'x-component': 'Input',
      },
      fn: (val, suffix) => (suffix ? `${val}${suffix}` : val),
    },
  },
  datetime: {
    Format: {
  ...
  time: {
    ...
    'hh:mm:ss': {
      ...
      fn: (val) => dayjs(val).format('hh:mm:ss'),
    },
    'hh:mm': {
      label: 'hh:mm (Deprecated, use Format instead)',
      fn: (val) => dayjs(val).format('hh:mm'),
    },
    hh: {
      label: 'hh (Deprecated, use Format instead)',
      fn: (val) => dayjs(val).format('hh'),
    },
  },
  number: {
    Precision: {
      ...
      fn: (val, precision) => Number(val.toFixed(precision)),
    },
    Divide: {
      schema: {
    ...
    Exponential: (val) => (+val)?.toExponential(),
    Abbreviation: {
      schema: {
        'x-component': 'Select',
        enum: [
      ...
      fn: (val, locale = 'en-US') => new Intl.NumberFormat(locale, { notation: 'compact' }).format(val),
    },
  },
  string: {
    'Type conversion': {
