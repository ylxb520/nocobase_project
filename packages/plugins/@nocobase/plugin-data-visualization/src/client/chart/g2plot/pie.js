...
export class Pie extends G2PlotChart {
  ...
  getProps({ data, general, advanced, fieldProps }) {
    ...
    const angleFieldProps = fieldProps[angleField] || {};
    const hasMinus = data.some((item) => item[angleField] < 0);
    if (hasMinus) {
      const min = Math.min(...data.map((item) => item[angleField]));
      data = data.map((item) => ({
        ...item,
        [angleField]: item[angleField] - min,
      }));
      const transformer = angleFieldProps.transformer;
      angleFieldProps.transformer = (value) => {
        const v = value + min;
