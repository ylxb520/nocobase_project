    ...
    async load() {
        this.app.resourceManager.registerActionHandler('customVariables:parse', async (ctx, next) => {
            ...
            try {
                ...
                const { collection, params: { field, filter, distinct }, aggregator, precision = 2, } = options;
                ...
                // 5. 对查询结果进行精度处理
                const processedResult = round((aggregator === 'avg' ? Number(result) : result) || 0, Math.max(0, Math.min(precision, 14)));
                // 6. 将处理后的结果返回给前端
