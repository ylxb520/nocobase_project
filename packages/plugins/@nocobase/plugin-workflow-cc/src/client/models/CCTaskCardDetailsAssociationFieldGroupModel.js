...
const isExcluded = (fieldPath, { whitelist = [], blacklist = [] } = {}) => {
...
export class CCTaskCardDetailsAssociationFieldGroupModel extends DetailsAssociationFieldGroupModel {
    ...
    static defineChildren(ctx) {
        ...
        const displayAssociationFields = (targetCollection, fieldPath = '', depth = 0) => {
            return targetCollection
                .getToOneAssociationFields()
                .map((field) => {
                const fPath = fieldPath ? `${fieldPath}.${field.name}` : field.name;
                if (!field.targetCollection) {
                    console.error(`AssociationFieldGroupModel: target collection ${field.target} not found for field ${field.name}`);
                    return;
                }
                if (isExcluded(fPath)) {
                    return;
...
//# sourceMappingURL=CCTaskCardDetailsAssociationFieldGroupModel.js.map