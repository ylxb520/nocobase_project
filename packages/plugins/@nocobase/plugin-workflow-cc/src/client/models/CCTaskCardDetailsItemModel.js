...
export const getEligibleTempAssociationSources = (sources = [], collection) => {
...
export const updateWorkflowCcTaskAssociationFields = ({ flowEngine, workflow, nodes, tempAssociationSources, }) => {
    if (!flowEngine)
        return;
    ...
    associations
        ...
        .forEach((association) => {
        collection.addField(new CCTaskCardTempAssociationField({
            type: 'belongsTo',
    ...
    static defineChildren(ctx) {
        ...
        return collection
            ...
            if (EXCLUDED_FIELDS.includes(field.name))
                return null;
            if (field.name.startsWith(TEMP_ASSOCIATION_PREFIX))
                return null;
            ...
            return {
                ...
                createModelOptions: () => ({
                    ...
                    subModels: {
                        field: {
                            use: fieldModel,
                            props: typeof binding.defaultProps === 'function' ? binding.defaultProps(ctx, field) : binding.defaultProps,
                        },
                    },
                }),
            };
    ...
    onInit(options) {
        ...
        this.context.defineProperty('collectionField', {
            get: () => {
                ...
                const collectionField = this.context.dataSourceManager.getCollectionField(`${params.dataSourceKey}.${params.collectionName}.${params.fieldPath}`);
                if (!collectionField && this.context.workflow) {
                    ...
                    const refreshedField = this.context.dataSourceManager.getCollectionField(`${params.dataSourceKey}.${params.collectionName}.${params.fieldPath}`);
...
//# sourceMappingURL=CCTaskCardDetailsItemModel.js.map