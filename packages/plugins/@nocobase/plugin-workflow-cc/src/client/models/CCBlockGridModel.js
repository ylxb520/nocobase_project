...
const CCAddBlockButton = ({ model }) => {
    ...
    const items = useMemo(() => {
        const dataBlocksChildren = [];
        // Trigger Data
        const triggerData = _.castArray(trigger.getCreateModelMenuItem?.({ config: workflow.config, nodeType: 'cc' })).filter(Boolean);
    ...
    return (React.createElement(AddSubModelButton, { model: model, subModelKey: "items", items: items },
        React.createElement(FlowSettingsButton, { icon: React.createElement(PlusOutlined, null) }, t('Add block'))));
};
export class CCBlockGridModel extends BlockGridModel {
    renderAddSubModelButton() {
        ...
        return React.createElement(CCAddBlockButton, { model: this });
    }
}
//# sourceMappingURL=CCBlockGridModel.js.map