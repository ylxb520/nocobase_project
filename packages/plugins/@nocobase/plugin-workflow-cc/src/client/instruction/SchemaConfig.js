...
import { ActionContextProvider, css, gridRowColWrap, parseCollectionName, SchemaComponent, SchemaComponentProvider, SchemaInitializer, useActionContext, useAPIClient, usePlugin, useRequest, useSchemaOptionsContext, } from '@nocobase/client';
import PluginWorkflowClient, { SimpleDesigner, updateNodeConfig, useAvailableUpstreams, useFlowContext, useNodeContext, useTrigger, useWorkflowExecuted, } from '@nocobase/plugin-workflow/client';
import { useTempAssociationSources } from '../hooks/useTempAssociationSources';
import { FlowModelRenderer, useFlowEngine, useFlowEngineContext, useFlowViewContext, } from '@nocobase/flow-engine';
import { NAMESPACE } from '../../common/constants';
import { lang, usePluginTranslation } from '../locale';
import { updateWorkflowCcTaskAssociationFields, } from '../models/CCTaskCardDetailsItemModel';
...
function SchemaContent({ value, onChange }) {
    ...
    return (React.createElement(SchemaComponentProvider, { scope: scope, components: components, designable: !executed },
...
const ConfigChangedContext = React.createContext({});
const ForceReadHint = observer(() => {
    const { initialValues, values } = useForm();
    return initialValues.forceRead != values.forceRead ? (React.createElement(Typography.Paragraph, { type: "secondary" },
        React.createElement(Typography.Text, { type: "warning" }, lang('Force read changes unsaved, click "Submit" to save.')))) : null;
});
export function SchemaConfigButton(props) {
    ...
    return (React.createElement(ConfigChangedContext.Provider, { value: { setFormValueChanged } },
        React.createElement(Button, { type: "primary", onClick: () => setVisible(true), disabled: false }, t(executed ? 'View user interface' : 'Configure user interface')),
        React.createElement(ActionContextProvider, { value: { visible, setVisible, formValueChanged: false } }, props.children),
        React.createElement(ForceReadHint, null)));
}
...
function CCInterfaceDrawerContent({ form, dataSourceKey, collectionName }) {
    ...
    return React.createElement(FlowModelRenderer, { model: model, hideRemoveInSettings: true, showFlowSettings: false });
}
...
export function CCInterfaceConfig({ children }) {
    ...
    const openV2 = ({ needConfirm } = {}) => {
        const doOpen = () => {
            ctx.viewer.open({
                ...
                content: (React.createElement(CCInterfaceDrawerContent, { form: form, dataSourceKey: dataSourceName, collectionName: collectionName })),
            });
        ...
        if (!ccUid && needConfirm) {
            Modal.confirm({
                ...
                content: t('Before using the v2 UI, you must delete the existing v1 UI. Are you sure you want to delete the v1 UI?', { ns: NAMESPACE }),
                onOk: async () => {
                    await deleteV1();
                    doOpen();
                },
    ...
    const items = [
        {
            key: 'v1',
            label: (React.createElement("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', minWidth: 120 } },
                React.createElement("span", { style: { flex: 1 } }, lang('v1 (Legacy)')),
                ccDetailValue && (React.createElement(DeleteOutlined, { onClick: (e) => {
                        e.stopPropagation();
                        handleDeleteV1(e);
    ...
    if (ccDetailValue) {
        return (React.createElement(ConfigChangedContext.Provider, { value: configChangedValue },
            React.createElement("div", { style: { display: 'flex', alignItems: 'center' } },
                React.createElement(Dropdown, { menu: { items }, trigger: ['hover'] },
                    React.createElement(Button, { type: "primary", icon: React.createElement(SettingOutlined, null) },
                        lang('Go to configure'),
                        " ",
                        React.createElement(DownOutlined, null)))),
            React.createElement(ActionContextProvider, { value: { visible: v1Visible, setVisible: setV1Visible, formValueChanged: false } },
                React.createElement(SchemaConfig, null))));
    }
    return (React.createElement(Button, { icon: React.createElement(SettingOutlined, null), type: "primary", onClick: openV2, disabled: false }, lang('Go to configure')));
}
...
function CCTaskCardDrawerContent({ uid, onUidChange, formDisabled, workflow, nodes, form, node, setFormValueChanged }) {
    ...
    const nodesSignature = useMemo(() => nodes
        ?.map((item) => item?.id)
        .filter(Boolean)
        .join('|') ?? '', [nodes]);
    const syncTempAssociationFields = useCallback((fields) => {
        if (formDisabled || !form || !node)
            return;
        if (isEqual(form.values.tempAssociationFields, fields))
            return;
    ...
    return (React.createElement(FlowModelRenderer, { model: model, hideRemoveInSettings: true, showFlowSettings: formDisabled
            ? false
            : {
...
export function CCTaskCardConfigButton() {
    ...
    const { setFormValueChanged } = useActionContext();
    const t = ctx.t;
    const onUidChange = useCallback((uid) => {
    ...
    const openConfig = useCallback(() => {
        ...
        ctx.viewer.open({
            ...
            content: (React.createElement(CCTaskCardDrawerContent, { uid: form.values.taskCardUid, onUidChange: onUidChange, formDisabled: form.disabled, workflow: flowContext.workflow, nodes: upstreamNodes, form: form, node: node, setFormValueChanged: setFormValueChanged })),
        });
    }, [
        cleanupTempAssociationFields,
        ctx.viewer,
    ...
    return (React.createElement(Button, { icon: React.createElement(CreditCardOutlined, null), type: "default", onClick: openConfig, disabled: false }, lang('Go to configure')));
}
