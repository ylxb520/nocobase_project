...
import { SchemaComponentContext, useCollectionRecordData, SchemaComponent, css, useAPIClient, usePlugin, useFormBlockContext, RemoteSchemaComponent, CollectionRecordProvider, useActionContext, useListBlockContext, } from '@nocobase/client';
import PluginWorkflowClient, { DetailsBlockProvider, FlowContext, linkNodes, NodeContext, useAvailableUpstreams, usePopupRecordContext, useTasksCountsContext, WorkflowTitle, } from '@nocobase/plugin-workflow/client';
...
function FlowContextProvider() {
    ...
    const [record, setRecord] = useState(useCollectionRecordData());
    ...
    if (ccUid && node && flowContext) {
        ...
        return (React.createElement(CollectionRecordProvider, { record: record },
            React.createElement(FlowContext.Provider, { value: flowContext },
                React.createElement(NodeContext.Provider, { value: node },
                    React.createElement(RemoteFlowModelRenderer, { uid: ccUid, onModelLoaded: (model) => {
    ...
    return node && flowContext ? (React.createElement(CollectionRecordProvider, { record: record },
        React.createElement(FlowContext.Provider, { value: flowContext },
            React.createElement(NodeContext.Provider, { value: node },
                React.createElement(RemoteSchemaComponent, { components: {
                        DetailsBlockProvider,
                        ...nodeComponents,
                    }, scope: {
                        useDetailsBlockProps,
                    }, uid: node.config?.ccDetail, noForm: true }))))) : (React.createElement(Spin, null));
}
function Drawer() {
    ...
    return (React.createElement(ConfigProvider, { theme: {
        ...
        React.createElement(Descriptions, { ...props, column: 1, items: [
                ...
                },
                {
                    ...
                    children: (React.createElement(SchemaComponent, { components: {
                        // TaskStatusColumn,
                        }, schema: {
                            name: 'status',
                            type: 'number',
...
const getUpstreams = (currentNode, nodes) => {
...
function TaskItem() {
    ...
    const onModelLoaded = useCallback((model) => {
    ...
    if (taskCardUid) {
        return (React.createElement(RemoteFlowModelRenderer, { uid: taskCardUid, onModelLoaded: onModelLoaded, mapModel: mapModel, reloadKey: taskCardReloadKey }));
    }
    // V1: 使用默认 Card 渲染
...
function TodoExtraActions(props) {
    return (React.createElement(SchemaComponent, { scope: {
            useReadAllAction,
...
export const ccTodo = {
    ...
    Item: TaskItem,
    Detail: Drawer,
    getPopupRecord: (apiClient, { params }) => apiClient.resource('workflowCcTasks').get({
        ...params,
