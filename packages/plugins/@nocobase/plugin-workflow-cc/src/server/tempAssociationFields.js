...
const normalizeTempAssociationConfigs = (configs) => {
    ...
    return configs
        ...
        .filter((config) => config.nodeId !== undefined &&
...
const getConfigAppends = (config) => normalizeAppends(config?.params?.appends ?? config?.appends);
export async function appendTempAssociationFields(context) {
    const body = context.body;
    const bodyData = Array.isArray(body) ? body : body?.rows;
    if (!Array.isArray(bodyData) || bodyData.length === 0) {
        return;
    }
    const tasksWithConfig = new Map();
    const executionIds = new Set();
    const tasksByNodeId = new Map();
    const ccNodeConfigMap = new Map();
    const ccNodeIdsToFetch = new Set();
    ...
    if (!tasksWithConfig.size)
        return;
    const workflowIdsNeedingConfig = new Set();
    for (const [task, configs] of tasksWithConfig.entries()) {
        if (!configs.some((config) => config.nodeType === 'workflow'))
            continue;
...
//# sourceMappingURL=tempAssociationFields.js.map