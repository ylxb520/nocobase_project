...
import { useActionContext, useBlockRequestContext, useCollection, useCollectionRecordData, useDataBlockRequestGetter, useDataBlockResource, useDestroyActionProps, usePlugin, } from '@nocobase/client';
...
export function useDeleteActionProps() {
...
export const NotificationVariableProvider = ({ value, children }) => {
    return React.createElement(NotificationVariableContext.Provider, { value: value }, children);
};
export const useNotificationTypes = () => {
    ...
    for (const [key, val] of plugin.channelTypes.getEntities()) {
        const title = Schema.compile(val.title, { t });
        const type = {
            ...val,
            name: val.type,
            key: val.type,
