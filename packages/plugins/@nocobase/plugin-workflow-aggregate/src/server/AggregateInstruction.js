...
export default class extends Instruction {
    async run(node, input, processor) {
        ...
        const repo = associated
            ? collectionManager.getRepository(`${association?.associatedCollection}.${association.name}`, processor.getParsedValue(association?.associatedKey, node.id))
        ...
        return {
            result: round((options.dataType === DataTypes.DOUBLE ? Number(result) : result) || 0, Math.max(0, Math.min(precision, 14))),
