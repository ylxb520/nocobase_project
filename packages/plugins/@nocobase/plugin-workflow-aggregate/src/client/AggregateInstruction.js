...
import { SchemaComponentContext, css, joinCollectionName, parseCollectionName, useCollectionDataSource, useCollectionFilterOptions, useCollectionManager_deprecated, useCompile, } from '@nocobase/client';
import { FieldsSelect, FilterDynamicComponent, ValueBlock, BaseTypeSets, defaultFieldNames, nodesOptions, triggerOptions, Instruction, } from '@nocobase/plugin-workflow/client';
...
function AssociatedConfig({ value, onChange, ...props }) {
    ...
    const onSelectChange = useCallback((path, option) => {
        if (!path?.length) {
    ...
    return (React.createElement(Cascader, { ...props, value: p, options: options, changeOnSelect: true, onChange: onSelectChange, loadData: loadData }));
}
// based on collection:
// { collection, field }
...
export default class extends Instruction {
    ...
    icon = (React.createElement(BarChartOutlined, { style: {} }));
    fieldset = {
        aggregator: {
            type: 'string',
...
//# sourceMappingURL=AggregateInstruction.js.map