 ...
 */
import { SchemaSettings, SchemaSettingsSelectItem, useDesignable, SchemaSettingsBlockHeightItem, SchemaSettingsModalItem, useOpenModeContext, SchemaSettingsLinkageRules, LinkageRuleCategory, } from '@nocobase/client';
...
const ActionPanelLayout = () => {
    ...
    return (React.createElement(SchemaSettingsSelectItem, { title: t('Layout', { ns: 'block-workbench' }), options: [
            { label: t('Grid', { ns: 'block-workbench' }), value: WorkbenchLayout.Grid },
            { label: t('List', { ns: 'block-workbench' }), value: WorkbenchLayout.List },
        ], value: fieldSchema?.['x-component-props']?.layout || WorkbenchLayout.Grid, onChange: (value) => {
...
export const ellipsisSettingsItem = {
    name: 'ellipsis',
    type: 'switch',
    useComponentProps() {
        const fieldSchema = useFieldSchema();
...
export function ActionPanelItemsPerRow() {
    ...
    const { dn } = useDesignable();
    const { t } = useTranslation();
    return (React.createElement(SchemaSettingsModalItem, { title: t('Items per row', { ns: 'block-workbench' }), schema: {
            type: 'object',
            properties: {
                itemsPerRow: {
                    type: 'number',
...
//# sourceMappingURL=workbenchBlockSettings.js.map