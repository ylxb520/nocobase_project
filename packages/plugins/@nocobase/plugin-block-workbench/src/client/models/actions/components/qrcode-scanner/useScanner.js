...
export function useScanner({ onScannerSizeChanged, elementId, onScanSuccess, app: appCtx, navigate }) {
    ...
    const mobileManager = targetApp.pm.get?.(PluginMobileClient);
    ...
    const startScanCamera = useCallback(async (scanner) => {
    ...
    const stopScanner = useCallback(async (scanner) => {
    ...
    const startScanFile = useCallback(async (file) => {
