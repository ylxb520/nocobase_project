...
export default class PluginVerficationServer extends Plugin {
    ...
    async load() {
        ...
        Object.entries(verifiersActions).forEach(([action, handler]) => this.app.resourceManager.registerActionHandler(`verifiers:${action}`, handler));
        ...
        this.verificationManager.addSceneRule((scene, verificationType) => ['auth-sms', 'unbind-verifier'].includes(scene) && verificationType === SMS_OTP_VERIFICATION_TYPE);
    ...
    async install() {
        const { DEFAULT_SMS_VERIFY_CODE_PROVIDER, INIT_ALI_SMS_ACCESS_KEY, INIT_ALI_SMS_ACCESS_KEY_SECRET, INIT_ALI_SMS_ENDPOINT = 'dysmsapi.aliyuncs.com', INIT_ALI_SMS_VERIFY_CODE_TEMPLATE, INIT_ALI_SMS_VERIFY_CODE_SIGN, } = process.env;
        if (DEFAULT_SMS_VERIFY_CODE_PROVIDER &&
