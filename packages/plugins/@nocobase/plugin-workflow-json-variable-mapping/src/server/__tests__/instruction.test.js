...
describe('workflow > instructions > JSONVariableMapping', () => {
    ...
    beforeEach(async () => {
        ...
        db = app.db;
        engine = app.pm.get(WorkflowPlugin);
    ...
    describe('JSONVariableMapping', () => {
        ...
        it('complex object with parsing array', async () => {
            ...
            expect(JSONJob.result.eq72xyzu23x).toStrictEqual(variable.a.b[0].c);
            expect(JSONJob.result['8jgtkcxmwe5']).toStrictEqual(variable.a.b[0].d);
            expect(JSONJob.result.g80ivg5werx).toStrictEqual(variable.a.b[1]);
            expect(JSONJob.result.bjtdmh5jp7e).toStrictEqual(variable.a.b[1].c);
        ...
        it('array with parsing array', async () => {
            ...
            expect(JSONJob.result.ph4sne0rzag).toStrictEqual(variable[0].c);
            expect(JSONJob.result['1838l1wqykf']).toStrictEqual(variable[0].d);
            expect(JSONJob.result.c1kjugwk88r).toStrictEqual(variable[1]);
            expect(JSONJob.result['1mx1pws62k2']).toStrictEqual(variable[1].c);
            expect(JSONJob.result['76fouy3vbpl']).toStrictEqual(variable[1].f);
            expect(JSONJob.result.xo9fol3qyhl).toStrictEqual(variable[2]);
            expect(JSONJob.result['ylqc55ufmq1']).toStrictEqual(variable[2][0]);
            expect(JSONJob.result['e2m0xvylq0b']).toStrictEqual(variable[2][0].c);
            expect(JSONJob.result['mmuk5wu8hoc']).toStrictEqual(variable[2][0].d);
            expect(JSONJob.result['tcbghieuroc']).toStrictEqual(variable[2][1]);
            expect(JSONJob.result['0qf1k5q846w']).toStrictEqual(variable[2][1].e);
            expect(JSONJob.result['4bo6b9ujea7']).toStrictEqual(variable[2][1].d);
        });
