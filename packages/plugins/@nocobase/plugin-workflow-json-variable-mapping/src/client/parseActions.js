...
function transformArray(arr, prefix = '', paths = [], options = { parseArray: false, keys: [], key: '' }) {
    ...
    if (parseArray) {
        ...
        const currentPath = `${prefix ? prefix + (key === ARRAY_KEY_IN_PATH ? '' : '.') : ''}${key ? (key === ARRAY_KEY_IN_PATH ? ARRAY_KEY_IN_TITLE : key + ARRAY_KEY_IN_TITLE) : ''}`;
        key !== ARRAY_KEY_IN_PATH &&
            paths.push({
                key: uid(),
    ...
    return {
        async run() {
            try {
                ...
                const bodyField = form.query('variables').take();
...
//# sourceMappingURL=parseActions.js.map