...
export class AdjacencyListRepository extends Repository {
  ...
  async queryPathByNode({ nodePks, dataSourceName, transaction }) {
    const collection = this.collection;
  ...
  async queryPathByRoot({ rootPks, dataSourceName, transaction }) {
    const collection = this.collection;
    const pathTableName = `${dataSourceName}_${collection.name}_path`;
    const repo = this.database.getRepository(pathTableName);
    if (repo) {
...
//# sourceMappingURL=adjacency-list-repository.js.map
