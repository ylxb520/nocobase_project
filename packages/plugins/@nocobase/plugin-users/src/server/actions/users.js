...
export async function updateProfile(ctx, next) {
    ...
    for (const field of requiredFields) {
        if (!values[field]) {
            ...
            throw new ValidationError(`${field} can not be null`, [
                new ValidationErrorItem(`${field} can not be null`, 
                // @ts-ignore
                'notNull violation', field, null, user, 'is_null', null, null),
            ]);
        }
    }
...
export async function updateLang(ctx, next) {
...
export const listExcludeRole = async (ctx, next) => {
...
export const getSystemSettings = async (ctx, next) => {
    const systemSettings = ctx.db.getRepository('systemSettings');
    const settings = await systemSettings.findOne();
    ctx.body = {
...
export const updateSystemSettings = async (ctx, next) => {
