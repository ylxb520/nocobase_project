...
export class Restorer extends AppMigrator {
  ...
  async getImportMeta() {
    ...
    return JSON.parse(await fsPromises.readFile(metaFile, 'utf8'));
  }
  async checkMeta() {
  ...
  async importCollection(options) {
    ...
    const rawAttributes = lodash.mapValues(meta.attributes, (attr, key) => {
      ...
      const DataTypeClass = DataTypes[db.options.dialect][attr.type] || DataTypes[attr.type];
      const obj = {
        ...attr,
  ...
  async importDb(options) {
    ...
    const queries = Object.values(sqlContent);
