 ...
 */
import { CollectionGroupManager as DBCollectionGroupManager } from '@nocobase/database';
...
export class Dumper extends AppMigrator {
  ...
  async dumpDb(options) {
    for (const collection of this.app.db.collections.values()) {
      const collectionOnDumpOption = this.app.db.collectionFactory.collectionTypes.get(collection.constructor)?.onDump;
  ...
  async dumpCollection(options) {
    ...
    // @ts-ignore
    const columns = [...new Set(lodash.map(attributes, 'field'))];
