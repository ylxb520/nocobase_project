...
function RemoteModelRenderer({ options }) {
  ...
  const { data, loading } = useRequest(
    async () => {
      const model = await ctx.engine.loadOrCreateModel(options, { delegateToParent: false, delegate: ctx });
      return model;
    },
    {
  ...
  if (loading || !data?.uid) {
    return React.createElement(SkeletonFallback, { style: { margin: 16 } });
  }
  return React.createElement(FlowModelRenderer, {
    model: data,
    fallback: React.createElement(SkeletonFallback, { style: { margin: 16 } }),
  });
}
...
function EditFormContent({ model }) {
  ...
  return React.createElement(
    ...
    React.createElement(RemoteModelRenderer, {
      options: {
  ...
  steps: {
    duplicateMode: {
      ...
      uiSchema(ctx) {
        ...
        const duplicateValues = cloneDeep(ctx.model.props?.duplicateFields || []);
                ...
                'a',
                {
                  onClick: async () => {
                    if (popupUid) {
                      ...
                      const model = await ctx.engine.loadModel({ parentId: ctx.model.uid, refresh: true });
                      if (!model) {
                        return;
                      }
                      run(model.subModels.items[0].subModels.grid);
...
async function fetchTemplateData(resource, template) {
    ...
    openView: {
      ...
      async handler(ctx, params) {
        ...
        ctx.viewer.open({
          ...
          content: () => React.createElement(EditFormContent, { model: ctx.model }),
          styles: {
            content: {
              padding: 0,
...
//# sourceMappingURL=DuplicateActionModel.js.map
