...
\`await ctx.getVar('ctx.popup.record')\`.`,
        schema: z.object({
            path: z
                .string()
                .optional()
                .describe('Dot-notated variable path. Supports progressive drilling. Prefer the most specific path when only a subset of data is needed.'),
            depth: z
                .number()
                .optional()
                .describe('Maximum traversal depth when exploring variables. Default is 3. Use smaller depth when the target structure is known.'),
        }),
    },
...
//# sourceMappingURL=getContextVars.js.map