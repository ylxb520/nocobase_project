 ...
 */
export function adjustSelectsForCollection(koaCtx, dataSourceKey, collection, fields, appends) {
    const ds = koaCtx.app.dataSourceManager.get(dataSourceKey || 'main');
    const cm = ds.collectionManager;
    const coll = cm?.db?.getCollection?.(collection);
    const assocKeys = Object.keys(coll?.model?.associations || {});
    const rawAttrs = coll?.model?.rawAttributes || {};
