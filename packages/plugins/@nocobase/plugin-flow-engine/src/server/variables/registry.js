...
import { ServerBaseContext } from '../template/contexts';
...
class VariableRegistry {
    ...
    extractUsage(template) {
        ...
        return extractUsedVariablePaths(template);
    }
    validate(template, contextParams) {
        ...
        for (const varName of Object.keys(usage)) {
            ...
            for (const spec of def.requiredParams) {
                if (!spec.required)
                    continue;
                if (params && typeof params[spec.name] !== 'undefined')
                    continue;
...
export const variables = g[GLOBAL_KEY];
/** 仅测试使用：重置变量注册表为内置默认集 */
 ...
 */
export function inferSelectsFromUsage(paths = [], _params) {
...
async function fetchRecordWithRequestCache(koaCtx, dataSourceKey, collection, filterByTk, fields, appends, strictSelects, preferFullRecord, associationName, sourceId) {
    try {
        ...
        if (!kctx.state['__varResolveBatchCache']) {
            kctx.state['__varResolveBatchCache'] = new Map();
        }
        ...
        if (!cm?.db)
            return undefined;
        const repo = associationName && typeof sourceId !== 'undefined'
            ? cm.db.getRepository(associationName, sourceId)
        ...
        const pkIsValid = pkAttr && modelInfo?.rawAttributes && Object.prototype.hasOwnProperty.call(modelInfo.rawAttributes, pkAttr);
        const collectionInfo = repo?.collection;
        ...
        const extraKeys = getExtraKeyFieldsForSelect(filterByTk, {
            ...
            pkAttr,
            pkIsValid,
            rawAttributes: modelInfo?.rawAttributes || undefined,
        });
        ...
        const cacheKeyFields = preferFullRecord && pkIsValid
            ? undefined
        ...
        if (cache) {
            ...
            if (!strictSelects) {
                ...
                // 则认为该字段已被关联载入，可视为满足。
                const needFields = !preferFullRecord && Array.isArray(fieldsWithExtras) ? [...new Set(fieldsWithExtras)] : undefined;
...
function attachGenericRecordVariables(flowCtx, koaCtx, usage, contextParams) {
    ...
    for (const varName of Object.keys(usage)) {
        ...
        const deepRecordMap = new Map(); // relativePath -> recordParams
        const cp = contextParams;
        if (cp && typeof cp === 'object') {
            const cpRec = cp;
            for (const key of Object.keys(cpRec)) {
                if (!key || (key !== varName && !key.startsWith(`${varName}.`)))
                    continue;
                if (key === varName)
                    continue;
                const val = cpRec[key];
                if (!isRecordParams(val))
                    continue;
        ...
        if (isRecordParams(topParams)) {
            ...
            const usedPathsForBase = deepRecordMap.size
                ? (usedPaths || []).filter((p) => {
                    ...
                    for (const relative of deepRecordMap.keys()) {
                        if (!relative)
                            continue;
                        if (p === relative || p.startsWith(relative + '.') || p.startsWith(relative + '['))
                            return false;
                    }
                    return true;
                })
            ...
            flowCtx.defineProperty(varName, {
                get: async () => {
                    ...
                    if (Array.isArray(topParams?.fields))
                        generatedFields = topParams.fields;
                    if (Array.isArray(topParams?.appends))
                        generatedAppends = topParams.appends;
                    const fixed = adjustSelectsForCollection(koaCtx, dataSourceKey, topParams.collection, generatedFields, generatedAppends);
                    const base = await fetchRecordWithRequestCache(koaCtx, dataSourceKey, topParams.collection, topParams.filterByTk, fixed.fields, fixed.appends, strictSelects, hasDirectRefTop, topParams.associationName, topParams.sourceId);
                    if (!deepRecordMap.size)
                        return base;
                    ...
                    const buildNestedPromise = (recordParams, relative) => {
                        ...
                        if (Array.isArray(recordParams?.fields))
                            generatedFields = recordParams.fields;
                        if (Array.isArray(recordParams?.appends))
                            generatedAppends = recordParams.appends;
                        const fixed = adjustSelectsForCollection(koaCtx, dataSourceKey, recordParams.collection, generatedFields, generatedAppends);
                        return fetchRecordWithRequestCache(koaCtx, dataSourceKey, recordParams.collection, recordParams.filterByTk, fixed.fields, fixed.appends, strictSelects, hasDirectRef, recordParams.associationName, recordParams.sourceId);
                    };
        ...
        const oneLevelRecordChildren = segEntries.filter(([seg]) => {
            ...
            const nestedObj = _.get(contextParams, [varName, seg]) ?? (idx ? _.get(contextParams, [varName, idx]) : undefined);
            const dotted = (contextParams || {})[`${varName}.${seg}`] ?? (idx ? (contextParams || {})[`${varName}.${idx}`] : undefined);
            return isRecordParams(nestedObj) || isRecordParams(dotted);
        });
        if (!oneLevelRecordChildren.length && deepRecordMap.size === 0)
            continue;
        flowCtx.defineProperty(varName, {
            get: () => {
                const root = new ServerBaseContext();
                const definedFirstLevel = new Set();
                ...
                const defineRecordGetter = (container, key, recordParams, subPaths = [], preferFull) => {
                    ...
                    container.defineProperty(key, {
                        get: async () => {
                            const dataSourceKey = recordParams?.dataSourceKey || 'main';
                            const fixed = adjustSelectsForCollection(koaCtx, dataSourceKey, recordParams.collection, generatedFields, generatedAppends);
                            return await fetchRecordWithRequestCache(koaCtx, dataSourceKey, recordParams.collection, recordParams.filterByTk, fixed.fields, fixed.appends, strictSelects, preferFull || (subPaths?.length ?? 0) === 0, recordParams.associationName, recordParams.sourceId);
                        },
                        cache: true,
                    });
                };
                ...
                for (const [seg, remainders] of oneLevelRecordChildren) {
                    const idx = parseIndexSegment(seg);
                    const recordParams = _.get(contextParams, [varName, seg]) ??
                    ...
                    if (!effRemainders.length) {
                        const all = usedPaths
                            .map((p) => p.startsWith(`${seg}.`) ? p.slice(seg.length + 1) : p.startsWith(`${seg}[`) ? p.slice(seg.length) : '')
                            .filter((x) => !!x);
                        if (all.length)
                            effRemainders = all;
                    }
            ...
            flowCtx.defineProperty('user', {
                get: async () => {
                    ...
                    if (typeof uid === 'undefined' || uid === null)
                        return undefined;
                    return await fetchRecordWithRequestCache(koaCtx, 'main', 'users', uid, generatedFields, generatedAppends, false, undefined, undefined, undefined);
                },
                cache: true,
            });
        },
