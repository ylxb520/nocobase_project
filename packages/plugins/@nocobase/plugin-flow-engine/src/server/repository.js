...
export function transaction(transactionAbleArgPosition) {
...
export class FlowModelRepository extends Repository {
    ...
    async remove(uid, options) {
        ...
        await this.database.sequelize.query(this.sqlAdapter(`DELETE FROM ${this.flowModelsTableName} WHERE "uid" IN (
        ...
        await this.database.sequelize.query(` DELETE FROM ${this.flowModelTreePathTableName}
            WHERE descendant IN (
    ...
    async duplicate(modelUid, options) {
        ...
        for (const n of sorted) {
            ...
            const optionsObj = this.replaceStepParamsModelUids(lodash.isPlainObject(n.options) ? n.options : JSON.parse(n.options), uidMap);
            if (newParentUid) {
    ...
    dedupeNodesForDuplicate(nodes, rootUid) {
        ...
        const seenUidsInQueryOrder = new Set();
        for (const row of nodes) {
            const uid = row?.uid;
            if (!uid || seenUidsInQueryOrder.has(uid))
                continue;
    ...
    async insertNewSchema(schema, options) {
        ...
        const nodes = FlowModelRepository.schemaToSingleNodes(schema);
        // insert schema fist
        await this.database.sequelize.query(this.sqlAdapter(`INSERT INTO ${this.flowModelsTableName} ("uid", "name", "options") VALUES ${nodes
            .map((n) => '(?)')
            .join(',')};`), {
        ...
        // insert tree path
        await this.database.sequelize.query(this.sqlAdapter(`INSERT INTO ${this.flowModelTreePathTableName} (ancestor, descendant, depth, type, async, sort) VALUES ${treePathData.map((item) => '(?)').join(',')}`), {
...
WHERE TreeTable.depth = 1 AND  TreeTable.ancestor = :ancestor and TreeTable.sort >= :sort and NodeInfo.type = :type`;
    ...
    async childrenCount(uid, transaction) {
        const db = this.database;
        const countResult = await db.sequelize.query(`SELECT COUNT(*) as count FROM ${this.flowModelTreePathTableName} where ancestor = :ancestor and depth  = 1`, {
            replacements: {
                ancestor: uid,
            },
            type: 'SELECT',
    ...
    async findNodesById(uid, options) {
        const db = this.database;
        const treeTable = this.flowModelTreePathTableName;
        const rawSql = `
        SELECT "SchemaTable"."uid" as "uid", "SchemaTable"."name" as name, "SchemaTable"."options" as "options" ,
    ...
    async schemaExists(schema, options) {
        ...
        const { transaction } = options;
        const result = await this.database.sequelize.query(this.sqlAdapter(`select "uid" from ${this.flowModelsTableName} where "uid" = :uid`), {
            type: 'SELECT',
            replacements: {
    ...
    static modelToSingleNodes(model, parentChildOptions = null) {
        ...
        if (Object.keys(subModels || {}).length > 0) {
            for (const [subKey, subItems] of Object.entries(subModels)) {
                const items = _.castArray(subItems);
                let sort = 0;
    ...
    async findModelById(uid, options) {
        const nodes = await this.findNodesById(uid, options);
    ...
    async attach(uid, attachOptions, options) {
        const { transaction } = options || {};
        ...
        // 环检测：禁止将祖先挂到其子孙节点下
        const cycle = await this.database.sequelize.query(`SELECT 1 as v
       FROM ${treeTable}
       ...
       LIMIT 1`, {
        ...
        if (subType === 'object') {
            const conflict = await this.database.sequelize.query(`SELECT TreeTable.descendant as uid
        ...
        const position = subType === 'object' ? 'last' : normalizePosition(attachOptions?.position);
        ...
        if (typeof position === 'object') {
            ...
            const ok = await this.database.sequelize.query(`SELECT 1 as v
         FROM ${treeTable} as TreeTable
                  LEFT JOIN ${treeTable} as NodeInfo
                            ON NodeInfo.descendant = TreeTable.descendant
                                AND NodeInfo.depth = 0
...
__decorate([
    transaction()
], FlowModelRepository.prototype, "clearAncestor", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "patch", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "initializeActionContext", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "batchPatch", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "remove", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertAdjacent", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "duplicate", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insert", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertNewSchema", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertBeside", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertInner", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertAfterBegin", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertBeforeEnd", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertBeforeBegin", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertAfterEnd", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertNodes", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "insertModel", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "updateSingleNode", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "upsertModel", null);
__decorate([
    transaction()
], FlowModelRepository.prototype, "attach", null);
