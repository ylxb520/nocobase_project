...
describe('server template resolver: dashed keys in dot-only path', () => {
    it('resolves dashed keys in dot-only expressions', async () => {
        ...
        const out = await resolveJsonTemplate('{{ ctx.formValues.roles.a-b }}', ctx);
        expect(out).toBe(123);
    });
    it('resolves dashed keys inside template strings', async () => {
        ...
        const out = await resolveJsonTemplate('prefix {{ ctx.formValues.roles.a-b }} suffix', ctx);
    ...
    it('keeps subtraction working (fallback to JS evaluation)', async () => {
        ...
        const out = await resolveJsonTemplate('{{ ctx.aa.bb-ctx.cc }}', ctx);
        expect(out).toBe(5);
    });
});
//# sourceMappingURL=resolver.dashed-keys.test.js.map