 ...
 */
export async function resolveJsonTemplate(template, ctx) {
...
async function evaluate(expr, ctx) {
    try {
        ...
        const dotOnly = raw.match(/^ctx\.([a-zA-Z_$][a-zA-Z0-9_$]*)(?:\.([a-zA-Z_$][a-zA-Z0-9_$-]*(?:\.[a-zA-Z_$][a-zA-Z0-9_$-]*)*))?$/);
        if (dotOnly) {
 ...
 * @returns 改写后的表达式字符串
 */
export function preprocessExpression(expression) {
    let out = '';
    let i = 0;
    ...
    while (i < n) {
        ...
        if (expression[j] === '.') {
            ...
            varName = (m[2] ?? m[3]); // unescaped content
            j += m[0].length;
        }
        else {
