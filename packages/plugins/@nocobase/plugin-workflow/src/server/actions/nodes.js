...
export async function create(context, next) {
    ...
    const workflowPlugin = context.app.pm.get(WorkflowPlugin);
    context.body = await db.sequelize.transaction(async (transaction) => {
        const workflow = workflowPlugin.enabledCache.get(Number.parseInt(context.action.sourceId, 10)) ||
            (await repository.getSourceModel(transaction));
...
export async function duplicate(context, next) {
    const { db } = context;
    const repository = utils.getRepositoryFromParams(context);
    ...
    context.body = await db.sequelize.transaction(async (transaction) => {
        ...
        const workflow = workflowPlugin.enabledCache.get(origin.workflowId) ||
...
export async function destroy(context, next) {
    ...
    await db.sequelize.transaction(async (transaction) => {
        ...
        const keepBranchHead = keepBranchIndex != null
...
export async function destroyBranch(context, next) {
    const { db } = context;
    const repository = utils.getRepositoryFromParams(context);
    ...
    await db.sequelize.transaction(async (transaction) => {
        ...
        if (shift) {
            ...
            await Promise.all(headsToShift.map((item) => item.update({
...
export async function move(context, next) {
    const { db } = context;
    const repository = utils.getRepositoryFromParams(context);
...
export async function test(context, next) {
    ...
    const plugin = context.app.pm.get(WorkflowPlugin);
