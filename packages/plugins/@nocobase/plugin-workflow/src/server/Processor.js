...
export default class Processor {
    ...
    async prepare() {
        const { execution, options: { plugin }, } = this;
    ...
    async exec(instruction, node, prevJob) {
        ...
        try {
            ...
            this.logger.error(`execution (${this.execution.id}) run instruction [${node.type}] for node (${node.id}) failed: `, { error: err, workflowId: node.workflowId });
            job = {
    ...
    async recall(node, job) {
        ...
        if (typeof instruction.resume !== 'function') {
            return Promise.reject(new Error(`"resume" method should be implemented for [${node.type}] instruction of node (#${node.id})`));
        }
        ...
        if (this.jobsToSave.size) {
            ...
            for (const job of this.jobsToSave.values()) {
                if (job.isNewRecord) {
                    ...
                    if (changes.length) {
                        await this.options.plugin.db.sequelize.query(`UPDATE ${JobCollection.quotedTableName()} SET ${changes.map(([key]) => `${key} = ?`)} WHERE id='${job.id}'`, { replacements: changes.map(([, value]) => value), transaction: this.mainTransaction });
                    }
            ...
            if (newJobs.length) {
                ...
                await JobsModel.bulkCreate(newJobs.map((job) => job.toJSON()), {
