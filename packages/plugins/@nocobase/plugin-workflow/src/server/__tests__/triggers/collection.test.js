...
describe('workflow > triggers > collection', () => {
    ...
    beforeEach(async () => {
        ...
        db = app.db;
        plugin = app.pm.get(PluginWorkflowServer);
    ...
    describe('model context', () => {
        ...
        it('trigger on association', async () => {
            ...
            const CategoryRepo = db.getRepository('posts.category', post.id);
    ...
    describe('execute', () => {
        it('disabled could be executed', async () => {
            ...
            const e1s = await workflow.getExecutions();
            expect(e1s.length).toBe(0);
            const { status, body: { data }, } = await agent.resource('workflows').execute({
