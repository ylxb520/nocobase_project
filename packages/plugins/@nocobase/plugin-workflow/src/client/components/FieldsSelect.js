...
function FieldOption({ label, value }) {
    return (React.createElement(Space, null,
        ...
        React.createElement(Tag, { bordered: false }, value)));
}
export const FieldsSelect = observer((props) => {
    ...
    const options = useMemo(() => fields.filter(filter).map((field) => ({
    ...
    return (React.createElement(Select, { popupMatchSelectWidth: false, ...others, options: options, filterOption: onSearch, optionRender: FieldOption }));
