...
test('Collection event add data trigger, filter single line text field not empty, common table single line text field data, set single line text field constant data', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    ...
    await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), updateNodeAppendText)
        .collections);
    ...
    const updateNodeCollectionRecordThree = updateNodeFieldDisplayName +
    ...
    await page
        .getByLabel(`block-item-AssignedField-${updateNodeCollectionName}-${updateNodeCollectionName}.${updateNodeFieldName}-${updateNodeFieldDisplayName}`)
        .getByRole('textbox')
...
test('Collection event add data trigger, filter single line text field not empty, common table single line text field data, set trigger node single line text field variable', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    const triggerNodeAppendText = 'a' + Math.random().toString(36).substring(2, 12);
    const updateNodeAppendText = 'b' + Math.random().toString(36).substring(2, 12);
    //创建触发器节点数据表
    ...
    await expect(page
        .getByLabel(`block-item-AssignedField-${updateNodeCollectionName}-${updateNodeCollectionName}.${updateNodeFieldName}-${updateNodeFieldDisplayName}`)
        .getByLabel('variable-tag')).toHaveText(`Trigger variables / Trigger data / ${triggerNodeFieldDisplayName}`);
    await updateRecordNode.submitButton.click();
    const updateRecordNodefieldData = updateNodeFieldDisplayName + dayjs().format('YYYYMMDDHHmmss.SSS').toString();
    // 查看更新前数据
    let updateNodeCollectionData = await apiGetList(updateNodeCollectionName);
...
test('Collection event add data trigger, filter multi-line text field not empty, common table multi-line text field data, set multi-line text field constant data', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    const triggerNodeAppendText = 'a' + Math.random().toString(36).substring(2, 12);
    const updateNodeAppendText = 'b' + Math.random().toString(36).substring(2, 12);
    //创建触发器节点数据表
...
test('Collection event add data trigger, filter multiline text field not empty, common table multiline text field data, set trigger node multiline text field variable', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    const triggerNodeAppendText = 'a' + Math.random().toString(36).substring(2, 12);
    const updateNodeAppendText = 'b' + Math.random().toString(36).substring(2, 12);
    //创建触发器节点数据表
...
test('Collection event add data trigger, filter integer field not null, common table integer field data, set integer field constant data', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    const triggerNodeAppendText = 'a' + Math.random().toString(36).substring(2, 12);
    const updateNodeAppendText = 'b' + Math.random().toString(36).substring(2, 12);
    //创建触发器节点数据表
...
//# sourceMappingURL=updateInABatch1.test.js.map