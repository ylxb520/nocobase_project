...
test('Collection event add data trigger, single row text fields for common tables, set constant data', async ({ page, mockCollections, mockRecords, }) => {
    ...
    await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), createNodeAppendText)
        .collections);
    //添加工作流
    ...
    await page
        .getByLabel(`block-item-AssignedField-${createNodeCollectionName}-${createNodeCollectionName}.${createNodeFieldName}-${createNodeFieldDisplayName}`)
        .getByRole('textbox')
