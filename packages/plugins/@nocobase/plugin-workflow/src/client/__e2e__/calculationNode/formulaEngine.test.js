...
import { AggregateNode, CalculationNode, CollectionTriggerNode, CreateRecordNode, QueryRecordNode, apiCreateWorkflow, apiCreateWorkflowNode, apiDeleteWorkflow, apiGetWorkflow, apiGetWorkflowNode, apiGetWorkflowNodeExecutions, apiUpdateWorkflowNode, apiUpdateWorkflowTrigger, appendJsonCollectionName, generalWithNoRelationalFields, } from '@nocobase/plugin-workflow-test/e2e';
import { expect, test } from '@nocobase/test/e2e';
import { dayjs } from '@nocobase/utils';
test('Collection event add data trigger, get trigger node single line text variable', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
...
test('Collection event add data triggers, get predecessor Formula engine arithmetic node data', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    //创建触发器节点数据表
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
...
test('Collection events add data triggers, get single line of text data for front query data nodes', async ({ page, mockCollections, mockRecords, }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    //创建触发器节点数据表
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
...
test('Collection event add data trigger, get front added data node single line text data', async ({ page, mockCollections, mockRecords, }) => {
    //创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
    ...
    await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), createRecordNodeAppendText)
        .collections);
    //添加工作流
    const workFlowName = faker.string.alphanumeric(5) + triggerNodeAppendText;
    const workflowData = {
...
test('Collection event adds data triggers, fetches data from front-end aggregation query nodes', async ({ page, mockCollections, mockRecords, }) => {
    //创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
...
test.skip('Collection event add data trigger, get front manual node add form single line text data', async ({ page, mockCollections, mockRecords, }) => {
    // 创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
    ...
    await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), manualNodeAppendText)
        .collections);
    //添加工作流
    const workFlowName = faker.string.alphanumeric(5) + triggerNodeAppendText;
    const workflowData = {
...
//# sourceMappingURL=formulaEngine.test.js.map