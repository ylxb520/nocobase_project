...
import { useFieldSchema, useForm, useFormEffects } from '@formily/react';
import { DataSourceProvider, joinCollectionName, RemoteSelect, SchemaSettingsActionModalItem, useCollection, useCollection_deprecated, useCompile, useDataSourceKey, useDesignable, useFormBlockContext, } from '@nocobase/client';
...
function WorkflowSelect({ formAction, buttonAction, actionType, ...props }) {
    ...
    const [workflowCollection, setWorkflowCollection] = useState(() => {
        ...
        return joinCollectionName(dataSourceKey, target ? baseCollection.collectionManager.getCollection(target).name : baseCollection.name);
    });
    const compile = useCompile();
    const workflowPlugin = usePlugin('workflow');
    const triggerOptions = workflowPlugin.useTriggersOptions();
    const workflowTypes = useMemo(() => triggerOptions
        .filter((item) => {
        return (typeof item.options.isActionTriggerable_deprecated === 'function' ||
            item.options.isActionTriggerable_deprecated === true);
    })
        .map((item) => item.value), [triggerOptions]);
    useFormEffects(() => {
    ...
    const optionFilter = useCallback(({ key, type, config }) => {
        if (key === props.value) {
            return true;
        }
    ...
    return (React.createElement(DataSourceProvider, { dataSource: "main" },
        React.createElement(RemoteSelect, { manual: false, placeholder: t('Select workflow', { ns: 'workflow' }), fieldNames: {
                label: 'title',
                value: 'key',
            }, service: {
                resource: 'workflows',
            ...
            }, optionFilter: optionFilter, optionRender: ({ label, data }) => {
                const typeOption = triggerOptions.find((item) => item.value === data.type);
                return typeOption ? (React.createElement(Flex, { justify: "space-between" },
                    React.createElement("span", null, label),
                    React.createElement(Tag, { color: typeOption.color }, compile(typeOption.label)))) : (label);
            }, ...props })));
}
export function BindWorkflowConfig() {
    ...
    const description = {
        ...
        'customize:save': t('Support pre-action event (local mode), post-action event (local mode), and approval event here.', {
            ns: 'workflow',
        }),
        'customize:update': t('Support pre-action event (local mode), post-action event (local mode), and approval event here.', { ns: 'workflow' }),
        'customize:triggerWorkflows': t('Workflow will be triggered directly once the button clicked, without data saving. Only supports to be bound with "Custom action event".', { ns: '@nocobase/plugin-workflow-custom-action-trigger' }),
        'customize:triggerWorkflows_deprecated': t('"Submit to workflow" to "Post-action event" is deprecated, please use "Custom action event" instead.', { ns: 'workflow' }),
    ...
    return (React.createElement(SchemaSettingsActionModalItem, { title: t('Bind workflows', { ns: 'workflow' }), scope: {
            fieldFilter(field) {
                                ...
                                properties: {
                                    context: {
                                        ...
                                        'x-component-props': {
                                            ...
                                            collection: `${collection.dataSource && collection.dataSource !== 'main' ? `${collection.dataSource}:` : ''}${collection.name}`,
                                            filter: '{{ fieldFilter }}',
                                            rootOption: {
        ...
        }, initialValues: { group: fieldSchema?.['x-action-settings']?.triggerWorkflows }, onSubmit: ({ group }) => {
...
//# sourceMappingURL=BindWorkflowConfig.js.map