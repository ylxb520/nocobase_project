...
export class NodeValueModel extends BlockModel {
    ...
    renderComponent() {
        return React.createElement(Result, { dataSource: this.dataSource, execution: this.execution, defaultValue: this.defaultValue });
    }
}
...
function Result({ dataSource, execution, defaultValue }) {
    ...
    const result = parse(dataSource)({
        $jobsMapByNodeKey: (execution.jobs ?? []).reduce((map, job) => Object.assign(map, { [job.nodeKey]: job.result }), {}),
    }) || defaultValue;
    return (React.createElement("pre", { style: {
            margin: 0,
        } }, JSON.stringify(result, null, 2)));
}
//# sourceMappingURL=NodeValueModel.js.map