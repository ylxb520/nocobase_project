...
import { css, cx, SchemaComponent, useActionContext, useAPIClient, useCancelAction, useResourceActionContext, useToken, } from '@nocobase/client';
            ...
            try {
                await apiClient.resource('flow_nodes').update({
                    ...
                    values: {
                        config: {
                            ...node.config,
                            conditions: node.config.conditions.map((item, index) => index === branchIndex - 1 ? { ...item, ...form.values } : item),
                        },
                    },
                });
                setVisible(false);
...
function getTooltipByResult(result) {
...
function ConditionHeaderCard(props) {
    ...
    const content = (React.createElement(Space, { className: css `
        padding: 0.5em 1em;
        border-radius: 2em;
...
function ConditionConfiguration({ condition, index }) {
    const node = useNodeContext();
    const apiClient = useAPIClient();
    const { refresh } = useResourceActionContext();
    ...
    return (React.createElement(ConditionHeader, null,
        executed <= 0 && node.config.conditions.length > 1 ? (React.createElement(Tooltip, { title: lang('Delete branch') },
            React.createElement(Button, { type: "text", shape: "circle", icon: React.createElement(DeleteOutlined, null), onClick: onRemove, className: cx('workflow-node-remove-button', 'workflow-branch-remove-button', css `
                display: none;
                position: absolute;
                right: -2em;
                top: 1.35em;
              `), size: "small" }))) : null,
        React.createElement(ConditionHeaderCard, null,
                                    ...
                                    properties: {
                                        ...
                                        engine: {
                                            ...
                                            'x-component-props': {
                                                ...
                                                ].reduce((result, [value, options]) => result.concat({ value, ...options }), []),
                                            },
                                            required: true,
                                            default: 'basic',
                                        },
...
function ConditionBranch({ condition, index }) {
    ...
    return (React.createElement(Branch, { from: node, entry: entry, branchIndex: index, controller: React.createElement(ConditionConfiguration, { condition: condition, index: index }), className: css `
        ...
        React.createElement("div", { className: styles.nodeSubtreeClass },
            React.createElement("div", { className: styles.branchBlockClass },
                conditions.map((item, index) => {
                    ...
                    return React.createElement(ConditionBranch, { key: item.uid, condition: item, index: branchIndex });
                }),
                React.createElement(Branch, { from: data, entry: defaultEntry, branchIndex: 0, controller: React.createElement(ConditionHeader, null,
                        React.createElement(ConditionHeaderCard, null, lang('Otherwise'))), end: !data.config.continueOnNoMatch })),
            React.createElement(Tooltip, { title: lang('Add branch'), className: css `
          ...
          ` },
            ...
            `, size: "small", onClick: onAddBranch, disabled: executed > 0 })))));
}
export default class extends Instruction {
    ...
    icon = (React.createElement(ClusterOutlined, null));
    fieldset = {
        continueOnNoMatch: {
            type: 'boolean',
...
//# sourceMappingURL=multi-conditions.js.map