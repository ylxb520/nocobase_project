...
import { CollectionProvider, SchemaComponent, SchemaComponentContext, useActionContext, useApp, usePlugin, useRecord, useResourceActionContext, useResourceContext, } from '@nocobase/client';
...
function SyncOptionSelect(props) {
    ...
    return React.createElement(RadioWithTooltip, { ...props });
}
...
function WorkflowEnabledSwitch() {
    ...
    const onChange = useCallback(async (nextChecked) => {
        if (!record?.id) {
            return;
        }
        setLoading(true);
    ...
    return (React.createElement(Switch, { checked: Boolean(record?.enabled), checkedChildren: lang('On'), unCheckedChildren: lang('Off'), disabled: loading || !record?.id, loading: loading, onClick: (val, e) => e?.stopPropagation?.(), onChange: onChange }));
}
export function WorkflowPane() {
    const ctx = useContext(SchemaComponentContext);
    const { useTriggersOptions } = usePlugin(WorkflowPlugin);
    return (React.createElement(SchemaComponentContext.Provider, { value: { ...ctx, designable: false } },
        React.createElement(SchemaComponent, { schema: workflowSchema, components: {
                CollectionProvider,
                WorkflowLink,
...
//# sourceMappingURL=WorkflowPane.js.map