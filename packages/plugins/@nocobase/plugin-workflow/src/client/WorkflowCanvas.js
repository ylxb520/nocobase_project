...
import { ActionContextProvider, ResourceActionProvider, SchemaComponent, cx, useActionContext, useApp, useCancelAction, useDocumentTitle, useNavigateNoUpdate, useResourceActionContext, useResourceContext, useCompile, css, useRequest, } from '@nocobase/client';
...
function ExecutionResourceProvider({ request, filter = {}, ...others }) {
    ...
    return React.createElement(ResourceActionProvider, { ...props });
}
function ExecutedStatusMessage({ data, option }) {
    const compile = useCompile();
    const statusText = compile(option.label);
    return (React.createElement(Trans, { ns: NAMESPACE, values: { statusText } },
        'Workflow executed, the result status is ',
        React.createElement(Tag, { color: option.color }, '{{statusText}}'),
        React.createElement(Link, { to: getWorkflowExecutionsPath(data.id) }, "View the execution")));
}
function getExecutedStatusMessage({ id, status }) {
    ...
    return {
        type: 'info',
        content: React.createElement(ExecutedStatusMessage, { data: { id }, option: option }),
    };
}
function useExecuteConfirmAction() {
    ...
    return {
        async run() {
            ...
            await form.submit();
            const { data: { data }, } = await resource.execute({
...
function ActionDisabledProvider({ children }) {
    ...
    return message ? React.createElement(Tooltip, { title: message }, children) : children;
}
function ExecuteActionButton() {
    const { workflow } = useFlowContext();
    const executed = useWorkflowExecuted();
    const trigger = useTrigger();
    return (React.createElement(CurrentWorkflowContext.Provider, { value: workflow },
        React.createElement(HideVariableContext.Provider, { value: true },
...
function WorkflowMenu() {
    ...
    const onDelete = useCallback(async () => {
        const content = workflow.current
            ? lang('This is a main version, delete it will cause the whole workflow to be deleted (including all other revisions).')
    ...
    const onMenuCommand = useCallback(({ key }) => {
        switch (key) {
            case 'refresh':
                refresh();
                return;
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Dropdown, { menu: {
            ...
            React.createElement(Button, { "aria-label": "more", type: "text", icon: React.createElement(EllipsisOutlined, null) })),
        React.createElement(ActionContextProvider, { value: { visible: historyVisible, setVisible: setHistoryVisible } },
            React.createElement(SchemaComponent, { schema: executionSchema, components: {
...
function RevisionsDropdown() {
    ...
    const navigate = useNavigate();
    const { workflow } = useFlowContext();
    const onSwitchVersion = useCallback(({ key }) => {
        if (key != workflow.id) {
    ...
    const loadRevisions = useCallback((visible) => {
        if (visible) {
            run();
        }
    }, [run]);
    const revisions = data?.data ?? [];
    return (React.createElement(Dropdown, { className: "workflow-versions", trigger: ['click'], onOpenChange: loadRevisions, menu: {
            ...
            items: revisions
                ...
                key: `${item.id}`,
                icon: item.current ? React.createElement(RightOutlined, null) : null,
                className: cx({
                ...
                label: (React.createElement(React.Fragment, null,
                    React.createElement("strong", null, `#${item.id}`),
                    React.createElement("time", null, dayjs(item.createdAt).fromNow()))),
            })),
        } },
        React.createElement(Button, { type: "text", "aria-label": "version" },
            React.createElement("label", null, lang('Version')),
            React.createElement("span", null, workflow?.id ? `#${workflow.id}` : null),
...
export function WorkflowCanvas() {
    ...
    if (!data?.data) {
        ...
        return (React.createElement(Result, { status: "404", title: "Not found", extra: React.createElement(Button, { onClick: () => navigate(-1) }, lang('Go back')) }));
    }
    const entry = nodes.find((item) => !item.upstream);
    return (React.createElement(FlowContext.Provider, { value: {
            workflow,
            nodes,
            refresh,
        } },
        React.createElement("div", { className: "workflow-toolbar" },
            React.createElement("header", null,
                ...
                workflow.sync ? (React.createElement(Tag, { color: "orange" }, lang('Synchronously'))) : (React.createElement(Tag, { color: "cyan" }, lang('Asynchronously')))),
            React.createElement("aside", null,
                React.createElement(ExecuteActionButton, null),
                React.createElement(RevisionsDropdown, null),
                React.createElement(Switch, { checked: enabled, onChange: onToggle, checkedChildren: lang('On'), unCheckedChildren: lang('Off'), loading: switchLoading }),
                React.createElement(WorkflowMenu, null))),
        React.createElement(AddNodeContextProvider, null,
            React.createElement(RemoveNodeContextProvider, null,
                React.createElement(NodeDragContextProvider, null,
                    React.createElement(NodeClipboardContextProvider, null,
