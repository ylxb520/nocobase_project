...
const NodeDragContext = createContext(null);
export function useNodeDragContext() {
    return useContext(NodeDragContext);
}
function collectDownstreams(start, branchChildrenMap, visited = new Set()) {
...
function collectBranchSubtree(root, branchChildrenMap) {
    const result = new Set();
    if (!root) {
        return result;
    }
...
export function NodeDragContextProvider(props) {
    ...
    const dragSubtreeRef = useRef(new Set());
    ...
    const clearSuppressTimer = useRef(null);
    ...
    const updateActiveDropRef = useRef(() => { });
    const autoScrollRef = useRef({ raf: null, vx: 0, vy: 0, container: null });
    ...
    const { nodeDepsMap, dependentsMap } = useMemo(() => {
        ...
        nodes.forEach((node) => {
            ...
            nodeDeps.forEach((depKey) => {
                const list = dependents.get(depKey) ?? new Set();
                list.add(node);
    ...
    const getTargetDownstream = useCallback((upstream, branchIndex, currentNode) => {
        if (!nodes) {
            return null;
        }
        if (!upstream) {
    ...
    const getDropImpact = useCallback((target) => {
        ...
        const downstreamSet = targetDownstream
            ...
            : new Set();
        const deps = nodeDepsMap.get(node.id) ?? new Set();
        const impactedSelf = [];
        deps.forEach((depKey) => {
        ...
        const dependents = dependentsMap.get(String(node.key)) ?? new Set();
        const impactedDependents = [];
    ...
    const moveNode = useCallback(async (nodeId, target, options) => {
        if (!nodeId) {
            return false;
        }
        const upstream = target?.upstream ?? null;
    ...
    const updateNodeConfigs = useCallback(async (items) => {
        let updated = false;
    ...
    const onNodeMouseDown = useCallback((node, event) => {
    ...
    const registerDropZone = useCallback((target, element) => {
        if (!element || !target) {
            return () => { };
        }
    ...
    return React.createElement(NodeDragContext.Provider, { value: value }, props.children);
}
//# sourceMappingURL=NodeDragContext.js.map