...
function CommonRepeatField({ value, onChange, disabled }) {
    ...
    return (React.createElement(InputNumber, { value: value / option.value, onChange: (v) => {
            if (!v) {
                return;
            }
            onChange(v * option.value);
        }, min: 1, addonBefore: t('Every'), addonAfter: t(option.unitText), className: "auto-width", disabled: disabled }));
}
export function RepeatField({ value = null, onChange, disabled }) {
    ...
    return (React.createElement("fieldset", { className: css `
      ...
      ` },
        React.createElement(Select, { value: typeValue, onChange: onTypeChange, className: "auto-width", disabled: disabled }, RepeatOptions.map((item) => (React.createElement(Select.Option, { key: item.value, value: item.value }, t(item.text))))),
        typeof typeValue === 'number' ? (React.createElement(CommonRepeatField, { value: value, onChange: onChange, disabled: disabled })) : null,
        typeValue === 'cron' ? (React.createElement(Cron, { value: value.trim().split(/\s+/).slice(1).join(' '), setValue: (v) => onChange(`0 ${v}`), clearButton: false, locale: window['cronLocale'], disabled: disabled })) : null));
}
