...
describe('user-data-resource-manager', () => {
    ...
    it('register resource error', async () => {
        try {
            ...
            expect(resourceManager.registerResource(errResource)).toThrowError('"name" for user data synchronize resource is required');
            const errResource2 = new ErrorResource(db, app.logger);
            errResource2.name = 'error';
            expect(resourceManager.registerResource(errResource2)).toThrowError('"accepts" for user data synchronize resource is required');
        }
        catch (error) {
            // ...
        }
