...
describe('inapp message channels', () => {
    ...
    describe('myInappChannels', async () => {
        ...
        test('user can get own channels and messages', async () => {
            ...
            await createMessages({ messagesRepo }, { unreadNum: 2, readNum: 2, channelName: channelsRes[0].name, startTimeStamp: Date.now(), userId: users[0].id });
            await createMessages({ messagesRepo }, { unreadNum: 2, readNum: 2, channelName: channelsRes[0].name, startTimeStamp: Date.now(), userId: users[1].id });
        ...
        test('filter channel by status', async () => {
            ...
            await createMessages({ messagesRepo }, { unreadNum: 0, readNum: 4, channelName: allReadChannel.name, startTimeStamp: Date.now(), userId: currUserId });
            await createMessages({ messagesRepo }, {
                unreadNum: 4,
                readNum: 0,
