...
export const messageMapObs = observable({ value: {} });
export const isFecthingMessageObs = observable({ value: false });
...
const filterMessageByStatus = (message) => {
...
const filterMessageByUserId = (message) => {
...
export const selectedMessageListObs = observable.computed(() => {
    if (selectedChannelNameObs.value) {
        const filteredMessages = messageListObs.value.filter((message) => message.channelName === selectedChannelNameObs.value && filterMessageByStatus(message) && filterMessageByUserId);
...
export const fetchMessages = async (params = { limit: 30 }) => {
...
export const markAllMessagesAsRead = async ({ channelName }) => {
    const apiClient = getAPIClient();
    await apiClient.request({
        resource: InAppMessagesDefinition.name,
        action: 'updateMyOwn',
...
export const unreadMsgsCountObs = observable({ value: null });
