...
const useChats = () => {
    ...
    const addChats = useCallback((groups) => {
        setGroupMap(produce((draft) => {
            groups.forEach((group) => {
                ...
                if (!draft[group.id].msgMap)
                    draft[group.id].msgMap = {};
            });
        }));
    }, []);
    const requestChats = useCallback(async ({ filter = {}, limit = 30 }) => {
    ...
    const addMessagesToGroup = useCallback(async (groupId, messages) => {
        ...
        if (group)
            setGroupMap(produce((draft) => {
                ...
                if (!draft[groupId].msgMap)
                    draft[groupId].msgMap = {};
    ...
    const fetchChats = useCallback(async ({ filter = {}, limit = 30 }) => {
        const res = await apiClient.request({
            url: 'myInAppChannels:list',
            method: 'get',
            params: { filter, limit },
    ...
    const fetchMessages = useCallback(async ({ filter }) => {
        const res = await apiClient.request({
