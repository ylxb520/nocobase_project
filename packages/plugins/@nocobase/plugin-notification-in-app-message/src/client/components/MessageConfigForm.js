...
export const MessageConfigForm = ({ variableOptions }) => {
    ...
    return (React.createElement(SchemaComponent, { scope: { t }, components: { UsersSelect, UsersAddition }, schema: {
            type: 'void',
            properties: {
                receivers: {
                    type: 'array',
                    title: `{{t("Receivers")}}`,
                    description: tval('When select receivers from node result, only support ID of user (or IDs array of users). Others will not match any user.', { ns: NAMESPACE }),
...
//# sourceMappingURL=MessageConfigForm.js.map