 ...
 * @returns
 */
export function fillParentFields(appends) {
...
export const parseAssociationNames = (dataSourceKey, collectionName, app, fieldSchema) => {
    let appends = new Set([]);
    ...
    const _getAssociationAppends = (schema, str) => {
        ...
        const customReducer = (pre, s, key) => {
            ...
            const collection = dataSource.collectionManager.getCollection(s?.['x-collection-field']?.split('.')?.[0] || collectionName);
            if (!collection) {
