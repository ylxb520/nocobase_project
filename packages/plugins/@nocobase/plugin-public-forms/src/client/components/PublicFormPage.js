...
import { ACLCustomContext, Action, APIClientProvider, AssociationField, CollectionManager, DataSource, DataSourceApplicationProvider, DataSourceManager, DatePicker, GlobalThemeProvider, PoweredBy, SchemaComponent, SchemaComponentContext, useApp, useCompile, useRequest, VariablesProvider, } from '@nocobase/client';
...
function PublicPublicFormProvider(props) {
    ...
    return (React.createElement("div", null,
        React.createElement(DataSourceApplicationProvider, { dataSource: dataSource.key, dataSourceManager: dataSourceManager, instance: collectionManager }, props.children)));
}
function PublicAPIClientProvider({ children }) {
    ...
    return React.createElement(APIClientProvider, { apiClient: app.apiClient }, children);
}
...
export const PublicFormMessageContext = createContext({});
...
const PublicFormMessageProvider = ({ children }) => {
    ...
    return (React.createElement(PublicFormMessageContext.Provider, { value: { showMessage, setShowMessage } }, children));
};
...
function InternalPublicForm() {
    ...
    if (error?.['response']?.status === 401 || data?.data?.passwordRequired) {
        return (React.createElement("div", null,
            React.createElement(Modal, { centered: true, title: "Password", open: true, cancelButtonProps: {
                    hidden: true,
                }, onOk: () => {
                    run({
                        password: pwd,
                ...
                React.createElement(Input.Password, { onChange: (e) => {
                        setPwd(e.target.value);
                    } }))));
    }
    if (error?.['response']?.status === 500) {
        return React.createElement(UnFoundFormPlaceholder, null);
    }
    if (loading) {
        return React.createElement(Spin, null);
    }
    if (!data?.data) {
        return React.createElement(UnEnabledFormPlaceholder, null);
    }
    const components = isMobile ? mobileComponents : {};
    return (React.createElement(ACLCustomContext.Provider, { value: { allowAll: true } },
        React.createElement(PublicAPIClientProvider, null,
            React.createElement("div", { style: {
              ...
              padding-top: 0px;
            ` },
                    React.createElement(PublicPublicFormProvider, { dataSource: data?.data?.dataSource },
                        React.createElement(SchemaComponentContext.Provider, { value: { ...ctx, designable: false } },
                            React.createElement(SchemaComponent, { schema: data?.data?.schema, scope: {
...
export function PublicFormPage() {
    return (React.createElement(GlobalThemeProvider, { theme: {
            token: {
                marginBlock: 18,
                borderRadiusBlock: 0,
        ...
        React.createElement(VariablesProvider, { filterVariables: (v) => {
            ...
            React.createElement(InternalPublicForm, null))));
}
//# sourceMappingURL=PublicFormPage.js.map