...
export class PluginErrorHandlerServer extends Plugin {
    ...
    registerSequelizeValidationErrorHandler() {
        ...
        this.errorHandler.register((err) => err?.errors?.length &&
            (err.name === 'SequelizeValidationError' || err.name === 'SequelizeUniqueConstraintError'), (err, ctx) => {
            ctx.body = {
    ...
    registerSQLErrorHandler() {
        ...
        const isSqlReferenceError = (error) => {
            ...
            if (/(unknown column|no such column|has no column named|no such table|unknown table|ambiguous column)/i.test(message)) {
                return true;
            }
            return false;
        };
        this.errorHandler.register((err) => isSqlSyntaxError(err), (err, ctx) => {
            ctx.body = {
                errors: [
                    {
        ...
        this.errorHandler.register((err) => isSqlReferenceError(err), (err, ctx) => {
            ctx.body = {
                errors: [
                    {
