...
describe('202503091240-fix-roles-mobile-routes', () => {
    let app, role1, role2, mobileRoutesRepo, rolesMobileRoutesRepo;
    beforeEach(async () => {
    ...
    it('should create miss child routes', async () => {
        ...
        const migration = new Migration({ db: app.db, app });
        await migration.up();
    ...
    it('should create miss child routes no includes hidden = false', async () => {
        ...
        expect(rolesMobileRoutes.map((x) => x.get('mobileRouteId'))).toStrictEqual(expect.arrayContaining([models[1].id, models[3].id]));
        const role2Routes = await rolesMobileRoutes.filter((x) => x.roleName === role2.get('name'));
...
//# sourceMappingURL=202503091240-fix-roles-mobile-routes.test.js.map