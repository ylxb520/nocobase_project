...
describe('MobileTabBar.Item', () => {
    test('Basic', () => {
        render(React.createElement(BasicApp, null));
        expect(screen.getByText('Test')).toBeInTheDocument();
    });
    test('With Icon: string', () => {
        render(React.createElement(WithIconApp, null));
        expect(screen.getByText('Test')).toBeInTheDocument();
        expect(screen.queryByRole('img')).toBeInTheDocument();
    });
    test('With Icon: React.Node', () => {
        render(React.createElement(WithIconReactNodeApp, null));
        expect(screen.getByText('Test')).toBeInTheDocument();
        expect(screen.queryByRole('img')).toBeInTheDocument();
    });
    test('Selected', () => {
        render(React.createElement(SelectedApp, null));
        expect(screen.getByText('Test')).toBeInTheDocument();
        expect(document.querySelector('.adm-tab-bar-item-active')).toBeInTheDocument();
    });
    test('Selected Icon', () => {
        render(React.createElement(SelectedIconApp, null));
        expect(screen.getByText('Test')).toBeInTheDocument();
        expect(document.querySelector('.adm-tab-bar-item-active')).toBeInTheDocument();
        expect(screen.queryByRole('img')).toHaveAttribute('aria-label', 'appstore-add');
    });
    test('onClick', async () => {
        render(React.createElement(OnClickApp, null));
        expect(screen.getByText('Test')).toBeInTheDocument();
        await act(async () => {
...
//# sourceMappingURL=MobileTabBar.Item.test.js.map