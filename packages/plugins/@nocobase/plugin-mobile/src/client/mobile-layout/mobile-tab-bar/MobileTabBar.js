...
import { cx, DndContext, SchemaComponent, useDesignable } from '@nocobase/client';
...
export const MobileTabBar = ({ enableTabBar = true }) => {
    ...
    if (!activeTabBarItem && validRouteList.length > 0)
        return null;
    return (React.createElement("div", { className: cx(componentCls, hashId, 'mobile-tab-bar'), "data-testid": "mobile-tab-bar" },
        React.createElement("div", { className: "mobile-tab-bar-content" },
            React.createElement(DndContext, { onDragEnd: handleDragEnd },
                React.createElement("div", { className: "mobile-tab-bar-list", style: {
                        maxWidth: designable ? 'calc(100% - 58px)' : '100%',
                    } }, routeList.map((item) => {
                    if (item.hideInMenu)
                        return null;
                    return React.createElement(SchemaComponent, { key: item.id, schema: getMobileTabBarItemSchema(item) });
                }))),
            React.createElement(MobileTabBarInitializer, null)),
...
//# sourceMappingURL=MobileTabBar.js.map