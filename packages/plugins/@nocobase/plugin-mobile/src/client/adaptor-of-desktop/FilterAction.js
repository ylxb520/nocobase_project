...
export const FilterAction = withDynamicSchemaProps((props) => {
    return (React.createElement(OriginFilterAction, { ...props, Container: (props) => {
            ...
            return (React.createElement(ConfigProvider, { theme: theme },
                props.children,
                React.createElement(Popup, { className: `${componentCls} ${hashId}`, visible: visiblePopup, onClose: closePopup, onMaskClick: closePopup, getContainer: getContainer, bodyStyle: bodyStyle, maskStyle: zIndexStyle, style: zIndexStyle, closeOnSwipe: true },
                    React.createElement("div", { className: "nb-mobile-action-drawer-header" },
                        ...
                        React.createElement("span", null, t('Filter')),
                        React.createElement("span", { className: "nb-mobile-action-drawer-close-icon", onClick: closePopup },
                            React.createElement(CloseOutline, null))),
                    React.createElement("div", { style: { padding: 12 } }, props.content),
 ...
 * @returns
 */
export const usePopupContainer = (visible, animationDelay = POPUP_ANIMATION_DELAY) => {
    const [mobileContainer] = useState(() => document.querySelector('.mobile-container') || document.body);
    // 仅需在桌面端创建弹窗容器
    if (!isDesktop) {
        return {
            visiblePopup: visible,
            popupContainerRef: { current: mobileContainer },
        };
    }
    ...
    // 创建弹窗容器的样式对象
    const createPopupContainerStyles = React.useCallback((zIndex) => ({
