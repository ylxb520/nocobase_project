 ...
 */
import { AdminProvider, AllDataBlocksProvider, AntdAppProvider, AssociationFieldMode, AssociationFieldModeProvider, BlockTemplateProvider, GlobalThemeProvider, OpenModeProvider, useAssociationFieldModeContext, usePlugin, usePopupSettings, zIndexContext, } from '@nocobase/client';
...
const CommonDrawer = (props) => {
    ...
    if (isPopupVisibleControlledByURL()) {
        return React.createElement(MobileActionPage, { ...props });
    }
...
export const Mobile = () => {
    ...
    React.useEffect(() => {
        if (!isDesktop) {
            ...
            viewportMeta.setAttribute('content', 'width=device-width,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no,viewport-fit=cover');
    ...
    const modeToComponent = React.useMemo(() => {
        return {
            PopoverNester: (props) => {
                ...
                return React.createElement(InternalPopoverNesterUsedInMobile, { ...props, OriginComponent: OriginComponent });
            },
        };
    }, []);
    return (React.createElement(AdminProviderComponent, null,
        React.createElement(DesktopComponent, null,
            React.createElement(GlobalThemeProvider, { theme: {
                    token: {
                ...
                React.createElement(AntdAppProvider, { className: `mobile-container ${componentCls} ${hashId}` },
                    React.createElement(OpenModeProvider, { defaultOpenMode: "page", isMobile: true, hideOpenMode: true, openModeToComponent: openModeToComponent },
                        React.createElement(BlockTemplateProvider, { componentNamePrefix: "mobile-" },
                            React.createElement(MobileAppProvider, null,
                                React.createElement(ResetSchemaOptionsProvider, null,
                                    React.createElement(AssociationFieldModeProvider, { modeToComponent: modeToComponent },
                                        React.createElement(zIndexContext.Provider, { value: 100 },
                                            React.createElement(AllDataBlocksProvider, null,
