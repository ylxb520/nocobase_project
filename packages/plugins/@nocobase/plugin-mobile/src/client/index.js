 ...
 */
import { PagePopups, Plugin, createRouterManager } from '@nocobase/client';
...
import { MobileLayout, MobileTabBar, mobileTabBarInitializer, mobileTabBarLinkSettings, mobileTabBarPageSettings, } from './mobile-layout';
import { MobileHomePage, MobileNavigationActionBar, MobileNavigationBarAction, MobileNotFoundPage, MobilePage, MobilePageContent, MobilePageHeader, MobilePageNavigationBar, MobilePageProvider, MobilePageTabs, mobileAddBlockInitializer, mobileNavigationBarActionsInitializer, mobileNavigationBarLinkSettings, mobilePageSettings, mobilePageTabsSettings, mobilePagesTabInitializer, useMobileNavigationBarLink, } from './pages';
...
export class PluginMobileClient extends Plugin {
    ...
    async load() {
        // 弃用警告
        console.warn('[NocoBase] @nocobase/plugin-mobile is deprecated and may be removed in future versions. Please migrate to the new mobile solution.');
    ...
    addInitializers() {
        this.app.schemaInitializerManager.add(mobileAddBlockInitializer, mobileTabBarInitializer, mobilePagesTabInitializer, mobileNavigationBarActionsInitializer);
    }
    addSettings() {
        this.app.schemaSettingsManager.add(mobilePageSettings, mobileTabBarPageSettings, mobileTabBarLinkSettings, mobilePageTabsSettings, mobileNavigationBarLinkSettings, MobileSettingsBlockSchemaSettings);
    }
    addComponents() {
    ...
    setMobileRouter() {
        const router = createRouterManager(this.options?.config?.router || { type: 'browser', basename: this.mobileBasename }, this.app);
    ...
    addPermissionsSettingsUI() {
        this.app.pm.get(PluginACLClient)?.settingsUI.addPermissionsTab(({ t, TabLayout, activeKey, currentUserRole }) => {
            ...
            return {
                ...
                sort: 25,
                children: (React.createElement(TabLayout, null,
                    React.createElement(MobileAllRoutesProvider, { active: activeKey === 'mobile-menu' },
                        React.createElement(MenuPermissions, { active: activeKey === 'mobile-menu' })))),
            };
        });
    }
}
