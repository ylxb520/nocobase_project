...
// 更新工作流
export const apiUpdateWorkflow = async (id, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 查询工作流
export const apiGetWorkflow = async (id) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
    const headers = getHeaders(state);
    const result = await api.get(`/api/workflows:get?filterByTk=${id}&appends[]=stats.executed&appends[]=versionStats.executed`, {
        headers,
    });
    if (!result.ok()) {
        throw new Error(await result.text());
...
// 更新工作流触发器节点
export const apiUpdateWorkflowTrigger = async (id, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 添加工作流节点
export const apiCreateWorkflowNode = async (workflowId, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 查询工作流节点
export const apiGetWorkflowNode = async (id) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 更新工作流节点配置
export const apiUpdateWorkflowNode = async (id, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 查询节点执行历史
export const apiGetWorkflowNodeExecutions = async (id) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 更新业务表单条数据
export const apiUpdateRecord = async (collectionName, id, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 查询业务表单条数据
export const apiGetRecord = async (collectionName, id) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 查询业务表list
export const apiGetList = async (collectionName) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 查询业务表list
export const apiFilterList = async (collectionName, filter) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
export const apiCreateRecordTriggerFormEvent = async (collectionName, triggerWorkflows, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 提交至工作流触发工作流表单事件
export const apiSubmitRecordTriggerFormEvent = async (triggerWorkflows, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 添加业务表单条数据触发工作流表单事件,triggerWorkflows=key1!field,key2,key3!field.subfield
export const apiCreateRecordTriggerActionEvent = async (collectionName, triggerWorkflows, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 添加业务表单条数据触发工作流表单事件,triggerWorkflows=key1!field,key2,key3!field.subfield
export const apiTriggerCustomActionEvent = async (collectionName, triggerWorkflows, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 审批中心发起审批
export const apiApplyApprovalEvent = async (data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
// 新增字段
export const apiCreateField = async (collectionName, data) => {
    const api = await request.newContext({
        storageState: process.env.PLAYWRIGHT_AUTH_FILE,
    });
    const state = await api.storageState();
...
export const userLogin = async (browser, approvalUserEmail, approvalUser) => {
