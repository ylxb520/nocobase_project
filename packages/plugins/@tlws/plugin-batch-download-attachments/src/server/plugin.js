...
import fs from 'fs';
export class BatchDownloadAttachmentsServer extends Plugin {
    async afterAdd() {
    ...
    async load() {
        ...
        this.app.resourcer.registerActions({
            'attachments:batchDownload': async (ctx, next) => {
                ...
                const { filter, collection, attachmentFields = [], // 用户选择的附件字段，空数组表示下载所有
                ...
                try {
                    ...
                    // 自动检测所有附件字段
                    const allAttachmentFields = [];
                    for (const [fieldName, field] of fieldEntries) {
                        const isAttachment = (field?.type === 'belongsToMany' && field?.target === 'attachments') ||
                    ...
                    const targetAttachmentFields = attachmentFields && attachmentFields.length > 0
                        ? attachmentFields.filter((f) => allAttachmentFields.includes(f))
                        : allAttachmentFields;
                    ...
                    ctx.set('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(zipFileName)}`);
                    ctx.respond = false;
    ...
    async addFileToArchive(archive, attachment, fieldName, recordName, groupPrefix = '') {
        try {
            let filePath;
