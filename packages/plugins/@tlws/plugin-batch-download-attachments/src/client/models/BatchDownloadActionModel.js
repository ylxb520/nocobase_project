...
function getCollectionName(ctx) {
 ...
 */
function getCollectionFromContext(ctx) {
    // 在设置界面中，通过 model.context.blockModel 获取
    return ctx.model?.context?.blockModel?.collection;
}
/**
 * 获取集合中的所有附件字段
 */
function getAttachmentFields(collection) {
    if (!collection)
        return [];
 ...
 * 支持多选，可以组合多个字段命名
 */
function getNamingFields(collection) {
    if (!collection)
        return [];
    const fields = collection.getFields?.() || [];
    const namingFields = [];
 ...
 * 单选，选择后启用分组
 */
function getGroupByFields(collection) {
    if (!collection)
        return [];
    const fields = collection.getFields?.() || [];
    const groupByFields = [];
        ...
        download: {
            ...
            async handler(ctx, params) {
                ...
                try {
                    ...
                    const ids = selectedRows.map((r) => r[filterTargetKey] ?? r.id).filter((v) => v != null);
...
//# sourceMappingURL=BatchDownloadActionModel.js.map